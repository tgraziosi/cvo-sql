...
CREATE PROCEDURE  [dbo].[glappstd_grp_sp]
	@app_id  		smallint,
	@app_post_date 		int,
	@process_id 		smallint, 
	@user_id 		smallint,
...
BEGIN
	...
	EXEC glpsupd_group2_sp @process_ctrl_num, @company_code
...
SELECT	@batch_code = '' 
	...
	BEGIN
		...
		SELECT @batch_code = MIN(batch_ctrl_num_group) 
		FROM #Group_batch 
		WHERE process_group_num = @process_ctrl_num
		AND batch_ctrl_num_group > @batch_code
		
		IF ( @batch_code IS NULL or @batch_code = '')--AQUI
			break
		-------------------------------------		
		


		EXEC	@result = glpsprep_grp_sp	@batch_code, @debug
		IF ( @result != 0 )
		BEGIN
			EXEC	pctrlupd_sp	@process_ctrl_num,
						2
		...
		EXEC	@result = glpshold_grp_sp	@batch_code,
						@debug
		IF ( @result != 0 )
		BEGIN
			EXEC	pctrlupd_sp	@process_ctrl_num,
		...
		EXEC	@result = glpspost_grp_sp	@batch_code,
						@debug
		IF ( @result != 0 )
		BEGIN
			EXEC	pctrlupd_sp	@process_ctrl_num,
...
GO
GRANT EXECUTE ON  [dbo].[glappstd_grp_sp] TO [public]
GO
