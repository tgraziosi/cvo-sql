...
CREATE PROCEDURE [dbo].[cc_query_list_d_rpt] 
	@from_cust		varchar(8) = '',
	@thru_cust		varchar(8) = '',	
	@from_terr		varchar(8) = '',
	@thru_terr		varchar(8) = '',	
...
IF @all_org_flag = 0
	...
	EXEC ('INSERT #final_table
		SELECT 	"",
					status,
					status_datetime = CASE WHEN status_date > 0	THEN CONVERT(datetime,DATEADD(dd, status_date - 639906, "01/01/1753")) ELSE NULL	END,
					customer_code,
					customer_name,
					balance,	
					aging_datetime = CASE WHEN aging_date > 0	THEN convert(datetime,DATEADD(dd, aging_date - 639906, "01/01/1753")) ELSE	NULL	END,
					territory_code,
					1,
					org_id
			FROM #invoices i 
			WHERE i.status_type = 1 ' + @where_clause1
			+ ' ORDER BY customer_code, status, balance, status_date')
	...
	IF @all_workload_flag = '1'
		SELECT 	doc_ctrl_num,
			...
			'to_org' = @to_org,
			org_id
		FROM #final_table f, arco c
		ORDER BY f.status_type, customer_code, status, balance, status_datetime
	ELSE
		SELECT 	doc_ctrl_num,
			status,
			status_datetime,
		...
		ORDER BY f.status_type, f.customer_code, status, balance, status_datetime
...
DROP TABLE #final_table
	...
	SET NOCOUNT OFF
GO
GRANT EXECUTE ON  [dbo].[cc_query_list_d_rpt] TO [public]
GO
