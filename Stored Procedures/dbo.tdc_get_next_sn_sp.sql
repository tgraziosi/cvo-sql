...
CREATE PROCEDURE [dbo].[tdc_get_next_sn_sp]
	@partno varchar(30),
	@qty integer = 1,
	@location varchar(10) = ''
AS
...
SET NOCOUNT ON
	...
	BEGIN
		...
		IF EXISTS(SELECT * FROM tdc_serial_no_track WHERE part_no = @partno AND serial_no = @serial)
			SELECT @qty = @qty + 1
		--Otherwise insert it into the temp table
		ELSE INSERT INTO #serial_no (serial_no, serial_raw) VALUES (@serial, @serial)
...
GO
GRANT EXECUTE ON  [dbo].[tdc_get_next_sn_sp] TO [public]
GO
