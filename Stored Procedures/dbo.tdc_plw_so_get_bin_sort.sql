...
SET ANSI_NULLS ON
GO
CREATE PROC [dbo].[tdc_plw_so_get_bin_sort]
...
ELSE IF (EXISTS (SELECT * FROM tdc_config   
	    	  WHERE [function] = 'alloc_bin_sort_so' 
	      	    AND active     = 'Y' 
...
BEGIN
	SELECT @dist_cust_pick = value_str FROM tdc_config (NOLOCK) WHERE [function] = 'dist_cust_pick'

	SELECT @bin_sort = CASE	@dist_cust_pick				 
				WHEN '1' THEN 'LIFO' 
				WHEN '2' THEN 'FIFO'
...
RETURN						 
GO
GRANT EXECUTE ON  [dbo].[tdc_plw_so_get_bin_sort] TO [public]
GO
