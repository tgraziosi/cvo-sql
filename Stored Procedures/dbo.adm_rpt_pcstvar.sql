...
create procedure [dbo].[adm_rpt_pcstvar] @range varchar(8000) = '0=0', @vstatus varchar(255) = '= ''R''', 
@order varchar(1000) = 'receipts.receipt_no'
as
begin
  select @range = replace(@range,'"','''')
  select @vstatus = replace(@vstatus,'"','''')
  select @order = replace(@order,'"','''')

CREATE TABLE  #rpt_pcstvar ( 
item_no varchar(30),  
quantity float,  
...
receipts.unit_measure 
 ...
 receipts.po_no = pur_list.po_no  and  
 pur_list.line = case when isnull(receipts.po_line,0)=0 then pur_list.line else receipts.po_line end and 
 receipts.status ' + @vstatus + '
 and ' + @range + '
 order by ' + @order
...
GO
GRANT EXECUTE ON  [dbo].[adm_rpt_pcstvar] TO [public]
GO
