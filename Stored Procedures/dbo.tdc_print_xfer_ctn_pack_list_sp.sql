...
CREATE PROCEDURE [dbo].[tdc_print_xfer_ctn_pack_list_sp](
			@user_id    varchar(50),
...
INSERT INTO #PrintData (data_field, data_value) VALUES ('LP_CARTON_NO',     	CAST  (@carton_no AS VARCHAR(20)))
...
EXEC @return_value = tdc_print_label_sp 'PPS', 'XFERCTNPACKTKT', 'VB', @station_id

-- IF label hasn't been set up for the station id, try finding a record for the user id
IF @return_value != 0
BEGIN
	EXEC @return_value = tdc_print_label_sp 'PPS', 'XFERCTNPACKTKT', 'VB', @user_id
END
...
BEGIN
	...
	BEGIN
		...
		SELECT @max_details_on_page = ISNULL((SELECT value_str FROM tdc_config (NOLOCK) WHERE [function] = 'XFERCTNPACKTKT'), 4) 
	END
	...
	BEGIN
		...
		SELECT @UPC_Code = upc_code, @Item_Description  = [description]
		  FROM inv_master 
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[tdc_print_xfer_ctn_pack_list_sp] TO [public]
GO
