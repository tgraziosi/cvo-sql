...
CREATE PROCEDURE [dbo].[APVOProcessGLEntries5_sp]    
...
BEGIN
	...
	--sp_helptext appgetstring_sp
--select * from appstrings
	...
	IF ((select top 1 isnull(e.a_sales_tax_acct_code ,1)
               FROM #apvochg_work b
			--INNER JOIN adm_pomchtax c ON  b.trx_ctrl_num = cast(c.match_ctrl_int as char(16))     
			INNER JOIN #tmpinptax c ON b.trx_ctrl_num = c.trx_ctrl_num
	...
	DECLARE @sumtax TABLE(
	 ...
	 INSERT @sumtax
	 SELECT t.trx_ctrl_num, amt_final_tax=SUM(t.amt_final_tax), t.account_code,  d.rec_company_code, d.org_id, d.reference_code   
	  FROM #apvotaxdtl_work t
		INNER JOIN #apvochg_work h ON t.trx_ctrl_num = h.trx_ctrl_num 
		INNER JOIN #apvocdt_work d ON t.trx_ctrl_num = d.trx_ctrl_num  AND t.detail_sequence_id =  d.sequence_id	
	 WHERE t.recoverable_flag = 0 --0
	 GROUP BY t.trx_ctrl_num, t.account_code, d.rec_company_code, d.org_id, d.reference_code					
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[APVOProcessGLEntries5_sp] TO [public]
GO
