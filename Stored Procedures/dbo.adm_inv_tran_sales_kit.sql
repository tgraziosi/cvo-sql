...
CREATE PROCEDURE [dbo].[adm_inv_tran_sales_kit] @action varchar(30), @i_tran_no int, @i_tran_ext int, @i_tran_line int,
...
BEGIN
  ...
  begin
    if @i_status != 'S' or left(@i_tran_data,1) != 'P'
      select @i_update_ind = -1

    if @i_status != 'S' and left(@i_tran_data,1) = 'P'
      select @update_typ = 'H'
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              from #cost_lots c
              join lot_cost_layers l on l.tran_code = 'K' and l.tran_no = @orig_no and l.tran_ext = @orig_ext
  ...
  begin
    ...
    begin
      ...
      where tran_code = 'K' and tran_no = @i_tran_no and tran_ext = @i_tran_ext
      ...
      select @i_location,@i_part_no,lot_ser,'K',@i_tran_no,@i_tran_ext,@i_tran_line,qty,
...
END
GO
GRANT EXECUTE ON  [dbo].[adm_inv_tran_sales_kit] TO [public]
GO
