...
CREATE PROCEDURE [dbo].[tdc_print_xfer_pack_list_sp](
			@user_id    varchar(50),
...
DECLARE
	...
	@addr5	           	varchar(60),
-- add total ship qty for XFER Pack List DMoon 01/31/2011
	@Tot_qty_ship	varchar(30)
-- end
...
--V2.0	-- Get Total Packed copied from order based_pack List DMoon 1/31/2011
	SELECT @tot_qty_ship = ISNULL((SELECT CAST(SUM(a.pack_qty) AS varchar(20))),'') 
	FROM tdc_carton_detail_tx a (NOLOCK), tdc_carton_tx b (NOLOCK)  
     WHERE a.carton_no = b.carton_no and a.order_no = @xfer_no AND b.order_type = 'T'  
-- select * from tdc_carton_tx where 
...
EXEC @return_value = tdc_print_label_sp 'PPS', 'XFERPACKTKT', 'VB', @station_id

-- IF label hasn't been set up for the station id, try finding a record for the user id
IF @return_value != 0
BEGIN
	EXEC @return_value = tdc_print_label_sp 'PPS', 'XFERPACKTKT', 'VB', @user_id
END
...
DECLARE Cartons_on_Order_Cursor CURSOR FOR
	...
	SELECT tdc_carton_tx.carton_no 
	 ...
	 WHERE tdc_carton_tx.order_no         = @xfer_no
	   ...
	   AND tdc_carton_tx.order_type	      = 'T'
...
BEGIN
	...
	BEGIN
		...
		SELECT @max_details_on_page = ISNULL((SELECT value_str FROM tdc_config (NOLOCK) WHERE [function] = 'XFERPACKTKT_Detl_Cnt'), 4) 
	END
...
GO
GRANT EXECUTE ON  [dbo].[tdc_print_xfer_pack_list_sp] TO [public]
GO
