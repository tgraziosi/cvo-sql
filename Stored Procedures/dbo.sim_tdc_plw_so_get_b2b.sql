...
CREATE PROC [dbo].[sim_tdc_plw_so_get_b2b]	@location 		varchar(10),
										@part_no 		varchar(30),
...
AS
BEGIN
	-- NOTE: Routine based on tdc_plw_so_get_b2b v1.0 - All changes must be kept in sync
	...
	WHERE a.location     	 = @location
	AND a.usage_type_code IN ('REPLENISH','OPEN')
	...
	BEGIN
		...
		SELECT @pick_queue_qty = ISNULL(SUM(qty_to_process), 0) FROM #sim_tdc_pick_queue WHERE trans = 'PLWB2B'     AND location = @location AND part_no = @part_no AND bin_no = @target_bin	
...
END
GO
GRANT EXECUTE ON  [dbo].[sim_tdc_plw_so_get_b2b] TO [public]
GO
