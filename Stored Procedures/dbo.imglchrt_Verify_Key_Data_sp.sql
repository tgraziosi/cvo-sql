...
GO
    ...
    CREATE procedure 
[dbo].[imglchrt_Verify_Key_Data_sp] (@debug_level INT = 0,
                             @userid INT = 0)
    AS
    ...
    SET @Routine_Name = 'imglchrt_Verify_Key_Data_sp'        
    IF @debug_level >= 3
    ...
    SELECT DISTINCT [company_code]
            FROM [CVO_Control]..[imglchrt]
            WHERE [company_code] NOT IN (SELECT DISTINCT [company_code] FROM [glcomp_vw])
                    AND ([User_ID] = @userid OR @userid = 0)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_SELECT + ' [imglchrt] 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    IF @Row_Count > 0
        BEGIN
        INSERT INTO [imlog] VALUES (getdate(), 'GLCHART', 1, '', '', '', 'General Ledger Chart of Accounts -- Error: Staging header table contains one or more invalid company codes.', @userid)
        INSERT INTO [imlog]
                ([now], [module], [text], [User_ID])
                SELECT DISTINCT GETDATE(), 'GLCHART', 'General Ledger Chart of Accounts --     Invalid company code: ' + ISNULL([company_code], 'NULL'), @userid
                        FROM [CVO_Control]..[imglchrt]
                        WHERE [company_code] NOT IN (SELECT DISTINCT [company_code] FROM [glcomp_vw])
                                AND ([User_ID] = @userid OR @userid = 0)
    ...
    SELECT DISTINCT [company_code]
            ...
            WHERE [company_code] NOT IN (SELECT DISTINCT [company_code] FROM [glcomp_vw])
                    AND ([User_ID] = @userid OR @userid = 0)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_SELECT + ' [imglseg] 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    IF @Row_Count > 0
        BEGIN
        INSERT INTO [imlog] VALUES (getdate(), 'GLCHART', 1, '', '', '', 'General Ledger Chart of Accounts -- Error: Staging detail table contains one or more invalid company codes.', @userid)
        INSERT INTO [imlog]
                ([now], [module], [text], [User_ID])
                SELECT DISTINCT GETDATE(), 'GLCHART', 'General Ledger Chart of Accounts --     Invalid company code: ' + ISNULL([company_code], 'NULL'), @userid
                        FROM [CVO_Control]..[imglseg]
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imglchrt_Verify_Key_Data_sp] TO [public]
GO
