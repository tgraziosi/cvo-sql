...
GO
    ...
    CREATE procedure 
[dbo].[imapint01_Validate_sp] @imapint01_Validate_sp_Batch_Number INT = 0,
                      @imapint01_Validate_sp_Starting_Record_Number INT = 0,
                      @imapint01_Validate_sp_Ending_Record_Number INT = 0,
                      @imapint01_Validate_sp_Flag_Bits INT = 0,
                      @debug_level INT = 0,
                      @imapint01_Validate_sp_Record_Type INT = 4091,
                      @imapint01_Validate_sp_Import_Identifier INT = 0,
                      @userid INT = 0,
                      @imapint01_Validate_sp_Application_Name VARCHAR(30) = 'Import Manager',
                      @imapint01_Validate_sp_Override_User_Name VARCHAR(30) = ''
    AS
    ...
    SET @Routine_Name = 'imapint01_Validate_sp'        
    IF @debug_level >= 3
    ...
    IF (@imapint01_Validate_sp_Flag_Bits & 0x1) > 0
        SET @close_batch_flag = 1
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imapint01_sp.'
    IF @imapint01_Validate_sp_Import_Identifier = 0    
            OR @imapint01_Validate_sp_Import_Identifier IS NULL
        BEGIN        
        EXEC @SP_Result = imapint01_sp @method_flag = 0,
                                       @post_flag = 0,
                                       @invoice_flag = @imapint01_Validate_sp_Record_Type,
                                       @close_batch_flag = 0,
                                       @db_userid = '',
                                       @db_password = '',
                                       ...
                                       @perf_level = 0,
                                       @userid = @userid,
                                       @imapint01_sp_process_ctrl_num_Validation = @process_ctrl_num OUTPUT,
                                       @imapint01_sp_process_ctrl_num_Posting = @Dummy OUTPUT,
                                       @imapint01_sp_Import_Identifier = @imapint01_Validate_sp_Import_Identifier OUTPUT,
                                       @imapint01_sp_Override_User_Name = @imapint01_Validate_sp_Override_User_Name
    ...
    ELSE    
        ...
        EXEC @SP_Result = imapint01_sp @method_flag = 0,
                                       ...
                                       @imapint01_sp_process_ctrl_num_Validation = @process_ctrl_num OUTPUT,
                                       @imapint01_sp_process_ctrl_num_Posting = @Dummy OUTPUT,
                                       @imapint01_sp_Import_Identifier = @imapint01_Validate_sp_Import_Identifier,
                                       @imapint01_sp_Override_User_Name = @imapint01_Validate_sp_Override_User_Name
    ...
    SET @Import_Identifier = @imapint01_Validate_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imapint01_Validate_sp] TO [public]
GO
