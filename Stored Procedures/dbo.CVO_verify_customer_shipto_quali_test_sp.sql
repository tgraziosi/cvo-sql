...
CREATE PROCEDURE [dbo].[CVO_verify_customer_shipto_quali_test_sp]	@promo_id	VARCHAR(20), @promo_level VARCHAR(30), @customer VARCHAR(30), 
		...
		*/
		...
		BEGIN
			...
			BEGIN
				...
				select * from #orders
				select '@past_promo_id',@past_promo_id
				select '@past_promo_level',@past_promo_level
				select '@achived-before',@achived
				...
				BEGIN
					...
					-- END v1.1
				END
				select '@achived-after',@achived
				select '@fail_reason',@fail_reason
			END
			-- END v3.6
		...
		IF @sub_check = 0
		BEGIN
			SELECT r as code, CASE WHEN r = 1 THEN '' ELSE @fail_reason END as reason FROM #t	-- v1.1
		END
		-- END v2.7
...
GO
GRANT EXECUTE ON  [dbo].[CVO_verify_customer_shipto_quali_test_sp] TO [public]
GO
