...
-- exec cvo_multiloc_cust_sales_sp '20201', 'year to date'
...
SELECT ccdc.code, ccdc.description, ccdc.customer_code, ISNULL(sbm.ship_to,'') ship_to, 
	SUM(CASE WHEN yyyymmdd >= @df THEN anet ELSE 0 end) NetsalesTY,
	SUM(CASE WHEN yyyymmdd <  @df THEN anet ELSE 0 end) NetsalesLY
