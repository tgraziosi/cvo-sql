...
CREATE PROCEDURE [dbo].[gltc_voucher_det_sp]
	...
	select @posted_flag = posted_flag from gltcrecon where trx_ctrl_num = @trx_ctrl_num
			...
			if (@posted_flag = 1)
				insert #gltcdocdet
					...
					[qty_received],[qty_ordered],[tax_code], [po_ctrl_num], [unit_code], [unit_price],
					[amt_discount], [amt_freight], [amt_tax], [amt_misc], [amt_extended],[gl_exp_acct], 
					[code_1099],[line_desc], [rec_company_code], [reference_code]  
					from apvodet where trx_ctrl_num = @trx_ctrl_num
...
GO
GRANT EXECUTE ON  [dbo].[gltc_voucher_det_sp] TO [public]
GO
