...
CREATE PROCEDURE [dbo].[pr_customer_payment_sp]		@contract_ctrl_num		VARCHAR(16),
...
AS
	...
	BEGIN TRAN
		...
		SELECT @sequence_id = MAX(sequence_id) + 1 FROM pr_customer_payments

		INSERT [pr_customer_payments] (	[contract_ctrl_num],
																	[sequence_id],
																	[customer_code],
																	[void_flag],
																	[check_num],
		...
		IF ( ISNULL( DATALENGTH( LTRIM( RTRIM( @price_code ) ) ),0 ) > 0 )
			BEGIN
				UPDATE pr_customer_class
				SET amount_paid_to_date_home = ISNULL(amount_paid_to_date_home,0) + @home_amount,
						amount_paid_to_date_oper = ISNULL(amount_paid_to_date_oper,0) + @oper_amount
				FROM pr_customer_class
				WHERE	price_class = @price_code
				AND	contract_ctrl_num = @contract_ctrl_num
...
GO
GRANT EXECUTE ON  [dbo].[pr_customer_payment_sp] TO [public]
GO
