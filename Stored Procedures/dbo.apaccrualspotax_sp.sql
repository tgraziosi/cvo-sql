...
CREATE PROC [dbo].[apaccrualspotax_sp]  
as  
...
select @tmp_ctrl_num = MIN(po_no) from #purchaseorders_tmp  
...
BEGIN  
 ...
 SELECT p.po_no, @h_doc_type, @trx_type, tc_companycode, getdate(), '', '',  
   0,                    '', p.vendor_no,                 '',           3,     '',   
 ...
 FROM   purchase p JOIN apmaster_all v (nolock) ON (v.vendor_code = p.vendor_no)  
 JOIN Organization_all o (nolock) ON (p.organization_id = o.organization_id )  
 WHERE po_no = @tmp_ctrl_num  
 ...
 SELECT pd.po_no, pd.line, ph.curr_key ,  
   c.curr_precision, pd.tax_code, (pd.qty_ordered - pd.qty_received), pd.unit_cost,   
   (SIGN(pd.unit_cost * (pd.qty_ordered - pd.qty_received)) * ROUND(ABS(pd.unit_cost * (pd.qty_ordered - pd.qty_received)) + 0.0000001, c.curr_precision)), pd.line  
 FROM purchase ph join pur_list pd on (ph.po_no = pd.po_no)  
  join glcurr_vw c (nolock) on (ph.curr_key = c.currency_code)  
 WHERE ph.po_no = @tmp_ctrl_num  
 ...
 select @tmp_ctrl_num = MIN(po_no)   
 from #purchaseorders_tmp where po_no > @tmp_ctrl_num  
...
FROM #tmpinptaxdtl dtl   
 JOIN pur_list cdt (nolock) ON (cdt.po_no = dtl.trx_ctrl_num  
        AND cdt.line  = dtl.detail_sequence_id )  
/**/                                                
GO
GRANT EXECUTE ON  [dbo].[apaccrualspotax_sp] TO [public]
GO
