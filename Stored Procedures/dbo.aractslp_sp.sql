...
CREATE PROC [dbo].[aractslp_sp]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/aractslp.sp" + ", line " + STR( 46, 5 ) + " -- ENTRY: "

IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 48, "entry aractslp_sp", @PERF_time_last OUTPUT
...
BEGIN
	UPDATE	#aractslp_work
	SET	update_flag = 1
	FROM	#aractslp_work a, aractslp b
	WHERE	a.salesperson_code = b.salesperson_code 

	SELECT	@status = @@error

	IF ( @perf_level >= 3 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 65, "update #aractslp_work", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 124, "insert aractslp", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	FROM	aractslp p, #aractslp_work t
	WHERE	p.salesperson_code = t.salesperson_code

	SELECT	@status = @@error

	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 191, "update aractslp: delta values", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	AND	p.salesperson_code = t.salesperson_code 
	
	
	SELECT	@status = @@error

	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 240, "update aractslp: avg_days_overdue", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 257, "update aractslp: avg_days_pay", @PERF_time_last OUTPUT

END

IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractslp.sp", 261, "exit aractslp_sp", @PERF_time_last OUTPUT

RETURN @status

GO
GRANT EXECUTE ON  [dbo].[aractslp_sp] TO [public]
GO
