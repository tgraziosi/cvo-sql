...
BEGIN -- CVO_ARAGING_SP DATA      
 ...
 --AND  (a.doc_ctrl_num = b.doc_ctrl_num	
	--	OR a.DOC_CTRL_NUM = b.order_ctrl_num)
...
--select * from #invoices  
 ...
 --UPDATE #FINAL     
 --SET     
 --YTDCREDS = ISNULL(sls.ytdcreds,0),
 --YTDSALES = ISNULL(sls.ytdsales,0),   
 --LYRSALES = ISNULL(sls.lyrsales,0),   
 --r12Sales = ISNULL(sls.r12sales,0)
 --from #final F   
 --LEFT OUTER JOIN (
 --SELECT customer, SUM(CASE WHEN yyyymmdd >= @CURRYRSTART THEN ISNULL(areturns,0) ELSE 0 end) ytdcreds,
	--	SUM(CASE WHEN yyyymmdd >=@curryrstart THEN ISNULL(anet,0) ELSE 0 end) ytdsales,
	...
	--	WHERE yyyymmdd >=@LASTYRSTART
	--	GROUP BY sd.customer
 --) sls ON sls.customer = f.CUSTOMER_CODE 
 ...
 --HOLD      
 --UPDATE f SET f.HOLD = C.STATUS_CODE      
 --FROM #FINAL F
 --LEFT OUTER JOIN
 --(SELECT c.customer_code, c.status_code
 --FROM  CC_CUST_STATUS_HIST C      
 --WHERE C.CLEAR_DATE IS NULL      
