...
CREATE PROCEDURE [dbo].[gltc_cm_det_sp]
...
if exists (select * from sysobjects where id = object_id('#gltcdocdet'))
   DROP TABLE #gltcdocdet
	...
	select @posted_flag = posted_flag from gltcrecon where trx_ctrl_num = @trx_ctrl_num
			...
			if (@posted_flag = 1)
				insert #gltcdocdet
					select trx_ctrl_num, sequence_id, trx_type, location_code, item_code, unit_code, return_code 
					tax_code, discount_prc, discount_amt, extended_price, unit_price, 
					line_desc, gl_rev_acct, qty_shipped, qty_returned, rma_num, weight 
...
GO
GRANT EXECUTE ON  [dbo].[gltc_cm_det_sp] TO [public]
GO
