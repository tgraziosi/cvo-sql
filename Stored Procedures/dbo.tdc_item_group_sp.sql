...
CREATE PROCEDURE [dbo].[tdc_item_group_sp]
AS

	DECLARE @recid int, @err int, @available_qty decimal(20,8), @temp_child int, 
		@avail_qty_this_line decimal(20,8), @done int, @order_ext int, @line_no int
	DECLARE @parent int, @method varchar(2), @order int, @part varchar(30), @lot varchar(25), 
		@bin varchar(12), @qty decimal(20,8), @type varchar(3), @new_type varchar(3),
		@reccnt int, @func char(1)
	...
	BEGIN
		...
		-- Error message: No data in processing
		UPDATE #dist_group SET err_msg = '' WHERE row_id = @recid
		return @err
	END
	...
	BEGIN
		...
		-- Error message: Qty has not been picked
		UPDATE #dist_group SET err_msg = '' WHERE row_id = @recid
		return @err
	END
	...
	/* quantity from this child first.  If there was not enough in this child	*/
	...
	DECLARE group_cursor CURSOR FOR 
		SELECT	child_serial_no, quantity, line_no FROM tdc_dist_item_pick 
		WHERE	method = @method 
			AND order_no = @order 
			AND order_ext = @order_ext
			AND part_no = @part 
	...
	FETCH NEXT FROM group_cursor INTO @temp_child, @avail_qty_this_line, @line_no
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				...
				/* Update status in ADM */
--				SELECT @order_ext = 
--					(SELECT order_ext FROM tdc_dist_item_pick WHERE child_serial_no = @temp_child AND [function] = @func)
...
--						(SELECT order_ext FROM tdc_dist_item_pick WHERE child_serial_no = @temp_child AND [function] = @func)
	...
	BEGIN
		...
		-- Error message: Qty not available
		UPDATE #dist_group SET err_msg = '' WHERE row_id = @recid
...
RETURN @err
GO
GRANT EXECUTE ON  [dbo].[tdc_item_group_sp] TO [public]
GO
