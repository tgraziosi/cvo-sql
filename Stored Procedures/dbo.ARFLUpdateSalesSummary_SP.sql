...
CREATE PROC [dbo].[ARFLUpdateSalesSummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arfluss.sp", 68, "Entering ARFLUpdateSalesSummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluss.sp" + ", line " + STR( 71, 5 ) + " -- ENTRY: "
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluss.sp" + ", line " + STR( 105, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	
	
	UPDATE	#arsumslp_work
	SET	num_fin_chg = (	SELECT COUNT(trx.salesperson_code)
				 	FROM	#artrx_work trx
				 	WHERE	trx.trx_type = 2061
					AND	trx.salesperson_code = #arsumslp_work.salesperson_code
				 ),
		num_late_chg= (	SELECT COUNT(trx.salesperson_code)
					FROM	#artrx_work trx
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluss.sp" + ", line " + STR( 126, 5 ) + " -- EXIT: "
		RETURN 34563
	END

	IF ( @debug_level >= 2 )
	BEGIN
		SELECT "dumping #arsumslp_work...."
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluss.sp" + ", line " + STR( 142, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARFLUpdateSalesSummary_SP] TO [public]
GO
