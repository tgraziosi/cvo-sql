...
BEGIN
    ...
    (
        SELECT DISTINCT
               cust_code,
               CASE
                   WHEN co.promo_level = '1' THEN
                       44.99
                   WHEN co.promo_level IN ( '2', '3' ) THEN
                       39.99
                   WHEN co.promo_level = 'renew1' THEN
                       ISNULL(
                       (
                           SELECT TOP (1)
                                  rate
                           FROM c_quote
                           WHERE date_expires = '12/31/2018'
                                 AND note LIKE 'sunps 2018%'
                                 AND customer_key = cust_code
                                 AND style = ''
                           ORDER BY item
                       ),
                       9999.99
                   ...
                   ELSE
                       9999.99
               END CONTRACT_PRICE,
               'SUNPS 2019 Contract Pricing' + CASE
                                                   WHEN co.promo_level = '3' THEN
                                                       ' - Renewal'
                                                   ELSE
                                                       ''
                                               END Note
    ...
    (
        SELECT DISTINCT
               ...
               CASE
                   WHEN cdc.code = 'SUN191' THEN
                       44.99
                   WHEN cdc.code = 'SUN192' THEN
                       39.99
                   ELSE
                       9999.99
               END CONTRACT_PRICE,
        ...
        WHERE cdc.code IN ( 'sun191', 'sun192' )
    ) AS t
        CROSS JOIN
        (SELECT 'BCBG' brand UNION SELECT 'SM' UNION SELECT 'IZOD') AS b
    WHERE NOT EXISTS
