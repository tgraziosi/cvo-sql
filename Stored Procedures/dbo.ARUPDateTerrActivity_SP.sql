...
CREATE PROC [dbo].[ARUPDateTerrActivity_SP] 
...
BEGIN
	...
	SELECT territory_code, 
		...
		0, 0, 0
	FROM	artrxage
	WHERE	( LTRIM(territory_code) IS NOT NULL AND LTRIM(territory_code) != " " )
	GROUP BY territory_code	
		
	CREATE CLUSTERED INDEX #cust_bal_ind ON #terr_bal(territory_code)
...
(
	...
	EXEC	ARUPDateActvityAmtonOrder_SP	3 

	
	UPDATE	#existing_orders
	SET	exists_flag = 1
	FROM	#terr_bal
	WHERE	#terr_bal.territory_code = #existing_orders.key_code1
	...
	IF EXISTS (	SELECT territory_code
			FROM	arinpchg
			WHERE	trx_type <= 2031
			AND	hold_flag = 0 )
	BEGIN
		...
		SET	exists_flag = 1
		FROM	#terr_bal
		WHERE	#terr_bal.territory_code = #unposted_trx.territory_code
		
		
		INSERT #terr_bal
		SELECT	territory_code, 0.0, 0.0, 
	...
	SET	exists_flag = 1
	FROM	aractter
	WHERE	aractter.territory_code = #terr_bal.territory_code
	...
	SET	amt_balance 		= bal.amt_balance_home,
		...
		last_trx_time		 = 1
	FROM	#terr_bal	bal
	WHERE	bal.territory_code = aractter.territory_code
	AND	exists_flag = 1

	
	INSERT	aractter (	territory_code,	date_last_inv,	date_last_cm,
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[ARUPDateTerrActivity_SP] TO [public]
GO
