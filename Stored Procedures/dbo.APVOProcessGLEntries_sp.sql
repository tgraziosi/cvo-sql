...
SELECT 	--dbo.IBAcctMask_fn( b.tax_rounding_acct,a.org_id),
		CASE WHEN @ib_flag = 0 THEN b.tax_rounding_acct
...
SELECT 	--dbo.IBAcctMask_fn( b.ap_acct_code,a.org_id),
		CASE WHEN @ib_flag = 0 THEN b.ap_acct_code
...
ELSE IF @gl_desc_def = 4
   BEGIN
	   ...
	   UPDATE #gldetail
	   --SET gl_desc = SUBSTRING(#gldetail.vendor_code + '/' + b.line_desc,1,40)			--v1.0
	   SET gl_desc = SUBSTRING(IsNull(v.vendor_short_name,'') + '/' + b.line_desc,1,40)		--v1.0
	   FROM #gldetail
		INNER JOIN #apvocdt_work b ON #gldetail.trx_ctrl_num = b.trx_ctrl_num AND #gldetail.sequence_id = b.sequence_id
		LEFT OUTER JOIN apvend v ON #gldetail.vendor_code = v.vendor_code					--v1.0
	   WHERE #gldetail.sequence_id > 0
...
IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'apvopgle.cpp' + ', line ' + STR( 735, 5 ) + ' -- MSG: ' + 'Create accrual Journal header record'
	...
	EXEC 	@result = gltrxcrh_sp 
			...
			2,
			@journal_type,
			@accrual_journal_ctrl_num OUTPUT,
			'To offset accrual vouchers.',
			@current_date,
			@next_period,
			0,
			0,
...
SELECT
 @accrual_journal_ctrl_num,
 a.rec_company_code,
 a.account_code,
 a.gl_desc,
 a.doc_ctrl_num,
 a.trx_ctrl_num,
 (SIGN(-a.amount * ( SIGN(1 + SIGN(a.rate_home))*(a.rate_home) + (SIGN(ABS(SIGN(ROUND(a.rate_home,6))))/(a.rate_home + SIGN(1 - ABS(SIGN(ROUND(a.rate_home,6)))))) * SIGN(SIGN(a.rate_home) - 1) )) * ROUND(ABS(-a.amount * ( SIGN(1 + SIGN(a.rate_home))*(a.rate_home) + (SIGN(ABS(SIGN(ROUND(a.rate_home,6))))/(a.rate_home + SIGN(1 - ABS(SIGN(ROUND(a.rate_home,6)))))) * SIGN(SIGN(a.rate_home) - 1) )) + 0.0000001, @home_precision)),
 a.reference_code,
 -a.amount,
 a.nat_cur_code,
 a.rate_home,
 4091,
 a.sequence_id,
 (SIGN(-a.amount * ( SIGN(1 + SIGN(a.rate_oper))*(a.rate_oper) + (SIGN(ABS(SIGN(ROUND(a.rate_oper,6))))/(a.rate_oper + SIGN(1 - ABS(SIGN(ROUND(a.rate_oper,6)))))) * SIGN(SIGN(a.rate_oper) - 1) )) * ROUND(ABS(-a.amount * ( SIGN(1 + SIGN(a.rate_oper))*(a.rate_oper) + (SIGN(ABS(SIGN(ROUND(a.rate_oper,6))))/(a.rate_oper + SIGN(1 - ABS(SIGN(ROUND(a.rate_oper,6)))))) * SIGN(SIGN(a.rate_oper) - 1) )) + 0.0000001, @oper_precision)),
 a.rate_oper,
 a.rate_type_home,
 a.rate_type_oper,
 a.org_id
...
EXEC @result = gltrxvfy_sp	@accrual_journal_ctrl_num,
							@debug_level
