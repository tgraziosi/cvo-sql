...
GO
    ...
    CREATE procedure 
[dbo].[imarpyt_Copy_sp] @imarpyt_Copy_sp_Batch_Number INT = 0,
                @imarpyt_Copy_sp_Starting_Record_Number INT = 0,
                @imarpyt_Copy_sp_Ending_Record_Number INT = 0,
                @imarpyt_Copy_sp_Dummy_1 INT = 0,
                @debug_level INT = 0,
                @imarpyt_Copy_sp_Dummy_2 INT = 0,
                @imarpyt_Copy_sp_Import_Identifier INT = 0,
                @userid INT = 0,
                @imarpyt_Copy_sp_Dummy_3 VARCHAR(40),
                @imarpyt_Copy_sp_Dummy_4 VARCHAR(40),
                @imarpyt_Copy_sp_Application_Name VARCHAR(30) = 'Import Manager',
                @imarpyt_Copy_sp_Override_User_Name VARCHAR(30) = '',
                @imarpyt_Copy_sp_Dummy_5 INT = NULL
    AS
    ...
    SET @Routine_Name = 'imarpyt_Copy_sp'        
    IF @debug_level >= 3
    ...
    --
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imarpyt_sp.'
    IF @imarpyt_Copy_sp_Import_Identifier = 0    
            OR @imarpyt_Copy_sp_Import_Identifier IS NULL
        BEGIN
        EXEC @SP_Result = imarpyt_sp @company_code = @company_code,
                                     @trial_flag = 0,
                                     @debug_level = @debug_level,
                                     @userid = @userid,
                                     @imarpyt_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                     @imarpyt_sp_dummy = @Dummy OUTPUT,
                                     @imarpyt_sp_Import_Identifier = @imarpyt_Copy_sp_Import_Identifier OUTPUT,
                                     @imarpyt_sp_Override_User_Name = @imarpyt_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imarpyt_sp 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    ELSE
        BEGIN                                 
        EXEC @SP_Result = imarpyt_sp @company_code = @company_code,
                                     ...
                                     @imarpyt_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                     @imarpyt_sp_dummy = @Dummy OUTPUT,
                                     @imarpyt_sp_Import_Identifier = @imarpyt_Copy_sp_Import_Identifier,
                                     @imarpyt_sp_Override_User_Name = @imarpyt_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imarpyt_sp' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
        BEGIN
        EXEC im_log_sp_error_sp @ILSE_Error_Code = @SP_Result,
                                @ILSE_SP_Name = 'imarpyt_sp',
                                @ILSE_String = '',
        ...
        END    
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': After imarpyt_sp.'
    SET @Import_Identifier = @imarpyt_Copy_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarpyt_Copy_sp] TO [public]
GO
