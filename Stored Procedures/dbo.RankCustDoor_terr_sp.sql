...
if ISNULL(@collection,'*ALL*') = '*ALL*'
 begin
	...
	select distinct kys from category
 end
else
 begin
	insert into #collection ([collection])
...
SELECT Right(customer,5)MergeCust, T2.*, CASE WHEN yyyymmdd between @DateFrom and @Dateto THEN 'TY' ELSE 'LY' END AS 'TYLY'
INTO #SOURCE
FROM cvo_sbm_details (nolock) t2
JOIN INV_MASTER (NOLOCK) INV ON T2.PART_NO=INV.PART_NO
inner join #collection c on c.collection = inv.category
