...
CREATE PROCEDURE [dbo].[bows_arintax_sp]    
...
SELECT @err = 0, @result = 0, @err_proc=''    
 ...
 IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'tmp/bows_arintax.sp' + ', line ' + STR( 45, 5 ) + ' -- ENTRY: '    
...
(    
 ...
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 10, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 20, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
/*------------------------------------*/        
...
/*-------------------------------------*/      
 ...
 SELECT    
  ...
  t.amt_tax,    
  t.amt_final_tax    
  FROM #TxInfo t, #arinpchg h    
  WHERE  h.trx_ctrl_num = t.control_number    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 30, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 SET db_action = 0    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 50, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
    
 UPDATE  w    
 SET w.db_action = 1    
 FROM #bows_arinptax_link w, #arinptax a    
 WHERE a.trx_ctrl_num = w.trx_ctrl_num    
 AND a.tax_type_code = w.tax_type_code    
 AND w.tax_calculated_mode = 1    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 60, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 SELECT  2000,    
  ...
  '',    
   0    
 FROM #bows_arinptax_link w    
 WHERE w.db_action = 0    
 AND w.tax_calculated_mode = 1    
    
 AND NOT (  ((w.amt_final_tax) <= (0.0) + 0.0000001) AND    
   EXISTS(SELECT 1 FROM artxtype t WHERE t.tax_type_code = w.tax_type_code) )    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 70, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 AND w.db_action = 1    
 AND w.tax_calculated_mode = 1    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 80, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 AND a.tax_type_code NOT IN    
  ...
  WHERE a.trx_ctrl_num = w.trx_ctrl_num    
  AND a.tax_type_code = w.tax_type_code)    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 90, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
 ...
 FROM #arinptax t, glcurr_vw g, #arinpchg h    
 WHERE h.nat_cur_code = g.currency_code    
 AND t.trx_ctrl_num = h.trx_ctrl_num    
 BEGIN SELECT @result = @@error IF @result<>0 BEGIN SELECT @err = 100, @err_proc='bows_arintax_sp' GOTO lbFinal END END    
...
lbFinal:    
 ...
 IF @debug_level>5 BEGIN    
  SELECT '#ARINPTAX after calculating tax'    
 ...
 IF @err<>0 BEGIN    
  ...
  SELECT     
   2000, @err,'PROC:' + @err_proc+ '.','',@result,0.0,    
   0, NULL,0,'',0    
 END    
...
DROP TABLE #txconnlineinput    
 ...
 IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'tmp/bows_arintax.sp' + ', line ' + STR( 212, 5 ) + ' -- EXIT: '    
    
RETURN @err    
GO
GRANT EXECUTE ON  [dbo].[bows_arintax_sp] TO [public]
GO
