...
CREATE PROCEDURE [dbo].[amch_vwDelete_sp]
(
	@timestamp				timestamp,
...
BEGIN
	...
	BEGIN
		...
		IF @line_id <> 1
		BEGIN
			EXEC	 	amGetErrorMessage_sp 20002, "tmp/amchdl.sp", 96, "am_ch_vw", @error_message = @message OUT
	...
	IF @ts <> @timestamp
	BEGIN
		EXEC	 	amGetErrorMessage_sp 20001, "tmp/amchdl.sp", 107, "am_ch_vw", @error_message = @message OUT
...
RETURN 0
GO
GRANT EXECUTE ON  [dbo].[amch_vwDelete_sp] TO [public]
GO
