...
create procedure [dbo].[amasttypLast_sp]
(
...
select @MSKasset_type_code = max(asset_type_code) from amasttyp
...
select @MSKasset_type_code = max(asset_type_code) from amasttyp where
	asset_type_code < @MSKasset_type_code
...
return @@error
GO
GRANT EXECUTE ON  [dbo].[amasttypLast_sp] TO [public]
GO
