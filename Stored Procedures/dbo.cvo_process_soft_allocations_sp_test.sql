...
CREATE PROC [dbo].[cvo_process_soft_allocations_sp_test]
AS
BEGIN
	-- Directives
	SET NOCOUNT ON
...
-- v5.5	AND		c.order_ext IS NULL
	...
	select * from cvo_soft_alloc_hdr (NOLOCK) where status = -1
	select * from cvo_soft_alloc_det (NOLOCK) where status = -1
...
END
GO
GRANT EXECUTE ON  [dbo].[cvo_process_soft_allocations_sp_test] TO [public]
GO
