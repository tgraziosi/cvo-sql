...
CREATE PROC [dbo].[APVOProcessGLEntries1_sp]    
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			AND h.intercompany_flag = @intercompany_flag
		...
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'apvopgle.cpp' + ', line ' + STR( 156, 5 ) + ' -- MSG: ' + 'Create Journal header record'
			...
			if (@trans_type = 1)
			  EXEC appgetstring_sp 'STR_FROM_VOUCHER_ACCRUALS', @str_msg_at  OUT	
			else if (@trans_type = 2)
			  EXEC appgetstring_sp 'STR_FROM_RECEIPTAP_ACCRUALS', @str_msg_at  OUT	
			else if (@trans_type = 4)
			  EXEC appgetstring_sp 'STR_FROM_RECEIPTADM_ACCRUALS', @str_msg_at  OUT
			else if (@trans_type = 5)	
			  EXEC appgetstring_sp 'STR_FROM_MATCHADM_ACCRUALS', @str_msg_at  OUT
			else if (@trans_type = 6)	
			  EXEC appgetstring_sp 'STR_FROM_POADM_ACCRUALS', @str_msg_at  OUT
		...
		SELECT 	a.gl_exp_acct,
				...
				a.org_id
		FROM #apvocdt_work a
			INNER JOIN #apvochg_work b ON ( a.trx_ctrl_num = b.trx_ctrl_num AND b.intercompany_flag = @intercompany_flag)
		...
		and 	a.org_id = @org_id
		and     b.a_disc_given_acct_code is null 
		and a.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		and a.intercompany_flag = @intercompany_flag
		...
		and 	a.org_id = @org_id 
		and     b.a_misc_chg_acct_code is null
		and a.intercompany_flag = @intercompany_flag		) > 0  )
		begin
			RETURN 420
		end
		...
		IF ((select top 1 isnull(b.a_sales_tax_acct_code,1)
			...
			and 	a.org_id = @org_id 
			and     b.a_sales_tax_acct_code is null
			and a.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		IF ((select top 1 isnull(e.a_sales_tax_acct_code ,1)
			...
			and 	b.org_id = @org_id 
			and     e.a_sales_tax_acct_code is null
			and b.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		and b.intercompany_flag = @intercompany_flag
		...
		and 	b.org_id = @org_id
		and     e.a_freight_acct_code is null
		and b.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		AND	d.tax_based_type = 2
		and 	b.org_id = @org_id
		and     d.sales_tax_acct_code is null
		and b.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		DECLARE @sumtax TABLE(
		 ...
		 SELECT t.trx_ctrl_num, t.trx_type, amt_final_tax=SUM(t.amt_final_tax), t.account_code,  d.rec_company_code, d.org_id, d.reference_code     
		  FROM #apvotaxdtl_work t
			INNER JOIN #apvochg_work h ON (t.trx_ctrl_num = h.trx_ctrl_num AND t.trx_type = h.trx_type and h.intercompany_flag = @intercompany_flag)
		...
		INTO 	 #amttaxincl
		FROM #apvochg_work a
			INNER JOIN #apvocdt_work b ON ( a.trx_ctrl_num = b.trx_ctrl_num AND a.intercompany_flag = @intercompany_flag )
		...
		and 	a.org_id = @org_id
		and     b.a_tax_rounding_acct is null
		and a.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end
		...
		and 	a.org_id = @org_id 
		and     b.a_freight_acct_code is null
		and a.intercompany_flag = @intercompany_flag ) > 0  )
		begin
			RETURN 420
		end	
	...
	END	
	SELECT @intercompany_flag = @intercompany_flag + 1
END	
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[APVOProcessGLEntries1_sp] TO [public]
GO
