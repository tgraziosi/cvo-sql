...
CREATE PROC [dbo].[ARINInsertValTables_SP]	
AS
...
BEGIN
	...
	SELECT age.trx_ctrl_num,	sequence_id,	  	doc_ctrl_num,	
		apply_to_num,		apply_trx_type,	age.trx_type,
		date_applied,		date_due,		date_aging,
		customer_code,	salesperson_code,	territory_code,
		price_code,		amt_due
	FROM	arinpage age, #aredtkey k
	WHERE	age.trx_ctrl_num = k.trx_ctrl_num
	AND	age.trx_type = k.trx_type
	...
	SELECT pyt.trx_ctrl_num,	doc_ctrl_num,		date_doc,
		customer_code,	payment_code,		amt_payment,
		amt_disc_taken,	cash_acct_code
	FROM	arinptmp pyt, #aredtkey k
	WHERE	pyt.trx_ctrl_num = k.trx_ctrl_num
	...
	(	trx_ctrl_num,		sequence_id,		rev_acct_code,
		apply_amt,		trx_type,		temp_flag,
		reference_code, org_id, interbranch_flag 
	)
	SELECT	rev.trx_ctrl_num,	sequence_id,		rev_acct_code,
		apply_amt,		rev.trx_type,		0,
		reference_code, org_id, 0
	FROM	arinprev rev, #aredtkey k
	WHERE	rev.trx_ctrl_num = k.trx_ctrl_num

     	RETURN 0
END
GO
GRANT EXECUTE ON  [dbo].[ARINInsertValTables_SP] TO [public]
GO
