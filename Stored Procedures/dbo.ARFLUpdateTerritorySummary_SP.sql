...
CREATE PROC [dbo].[ARFLUpdateTerritorySummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arfluts.sp", 68, "Entering ARFLUpdateTerritorySummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluts.sp" + ", line " + STR( 71, 5 ) + " -- ENTRY: "
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluts.sp" + ", line " + STR( 105, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	
	
	UPDATE	#arsumter_work
	SET	num_fin_chg = (	SELECT COUNT(trx.territory_code)
				 	FROM	#artrx_work trx
				 	WHERE	trx.trx_type = 2061
					AND	trx.territory_code = #arsumter_work.territory_code
				 ),
		num_late_chg= (	SELECT COUNT(trx.territory_code)
					FROM	#artrx_work trx
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluts.sp" + ", line " + STR( 126, 5 ) + " -- EXIT: "
		RETURN 34563
	END

	IF ( @debug_level >= 2 )
	BEGIN
		SELECT "dumping #arsumter_work...."
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluts.sp" + ", line " + STR( 142, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARFLUpdateTerritorySummary_SP] TO [public]
GO
