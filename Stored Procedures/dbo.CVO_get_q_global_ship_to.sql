...
CREATE PROCEDURE [dbo].[CVO_get_q_global_ship_to] @cust varchar(30), @sort char(1), @void char(1), @mode varchar(2) = '',   -- mls 4/12/00 SCR 21882    
											@secured_mode int = 0, @typ int = 0, @org_id varchar(30) = '', @cust_code  varchar(10) = '' -- v1.1   
AS    
BEGIN    
-- v1.0 CB 07/08/2013 - Issue #1111 - Remove org and add city
-- v1.1 CB 22/04/2016 - Issue #775 Global Ship To - Regional
...
begin    
	...
	IF (@mode = 'XX')
	BEGIN
		SELECT @sql = @sql + ' , cvo_cust_gsh gsh (NOLOCK) '
	END
	-- v1.1 End
	...
	IF (@mode = 'XX')
	BEGIN
		SELECT @sql = @sql + ' and m.customer_code = gsh.global_ship AND gsh.cust_code = ''' + @cust_code + ''''
	END
	-- v1.1 End
