...
CREATE PROC [dbo].[aractcus_sp]	@batch_ctrl_num	varchar( 16 ),
...
BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/aractcus.sp" + ", line " + STR( 42, 5 ) + " -- ENTRY: "
	IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 43, "entry aractcus_sp", @PERF_time_last OUTPUT
	...
	BEGIN
		UPDATE	#aractcus_work
		SET		update_flag = 1
		FROM	#aractcus_work a, aractcus b
		WHERE	a.customer_code = b.customer_code
	
		SELECT	@status = @@error
	
		IF ( @perf_level >= 3 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 60, "update #aractcus_work", @PERF_time_last OUTPUT
	END
	...
	BEGIN
		...
		IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 118, "insert aractcus", @PERF_time_last OUTPUT
	END
	...
	BEGIN
		...
		SET	date_last_inv = ISNULL(t.date_last_inv, p.date_last_inv),
			...
			amt_age_bracket1 = p.amt_age_bracket1 + ISNULL(t.amt_age_bracket1, 0.0),
			amt_age_bracket2 = p.amt_age_bracket2 + ISNULL(t.amt_age_bracket2, 0.0),
			amt_age_bracket3 = p.amt_age_bracket3 + ISNULL(t.amt_age_bracket3, 0.0),
			amt_age_bracket4 = p.amt_age_bracket4 + ISNULL(t.amt_age_bracket4, 0.0),
			amt_age_bracket5 = p.amt_age_bracket5 + ISNULL(t.amt_age_bracket5, 0.0),
			amt_age_bracket6 = p.amt_age_bracket6 + ISNULL(t.amt_age_bracket6, 0.0),
			amt_on_order = p.amt_on_order + ISNULL(t.amt_on_order, 0.0),
			amt_inv_unposted = p.amt_inv_unposted + ISNULL(t.amt_inv_unposted, 0.0),
			...
			amt_on_acct = p.amt_on_acct + ISNULL(t.amt_on_acct, 0.0),
			...
			amt_on_acct_oper = p.amt_on_acct_oper + ISNULL(t.amt_on_acct_oper, 0.0), 
		...
		FROM	aractcus p, #aractcus_work t
		WHERE	p.customer_code = t.customer_code
		SELECT @status = @@error

		IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 186, "update aractcus: delta values", @PERF_time_last OUTPUT
	END
	...
	BEGIN
		...
		IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 234, "update aractcus: avg_days_overdue", @PERF_time_last OUTPUT
	END
	...
	BEGIN
		...
		IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 250, "update aractcus: avg_days_pay", @PERF_time_last OUTPUT
	END


	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/aractcus.sp" + ", line " + STR( 254, 5 ) + " -- EXIT: "
	IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractcus.sp", 255, "exit aractcus_sp", @PERF_time_last OUTPUT
	RETURN @status
END
GO
GRANT EXECUTE ON  [dbo].[aractcus_sp] TO [public]
GO
