...
CREATE PROC [dbo].[ARCAUpdateTerritoryActivity_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcauta.sp", 57, "Entering ARCAUpdateTerritoryActivity_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 60, 5 ) + " -- ENTRY: "
	...
	CREATE TABLE	#max_trx
			...
			IF( @@error != 0 )
			BEGIN
				IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 98, 5 ) + " -- EXIT: "
				RETURN 34563
			END
	...
	IF( @result != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 115, 5 ) + " -- EXIT: "
		RETURN 35011
	END
	...
	SELECT	DISTINCT trx.territory_code
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 132, 5 ) + " -- EXIT: "
		RETURN 34563
	END
		

	INSERT	#max_trx (character_8, trx_ctrl_num)
	SELECT	trx.territory_code,				 
	...
	AND	( LTRIM(trx.territory_code) IS NOT NULL AND LTRIM(trx.territory_code) != " " )
	GROUP BY trx.territory_code

	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 150, 5 ) + " -- EXIT: "
		RETURN 34563
	END

		
	
	SELECT	artrx.territory_code, pdt.trx_ctrl_num, SUM(pdt.amt_applied) sum_amt_applied
	INTO	#t
	...
	AND	trx.character_8 = artrx.territory_code
	GROUP BY artrx.territory_code, pdt.trx_ctrl_num
	...
	WHERE	#aractter_work.territory_code = #t.territory_code
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 181, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	amt_balance = 
		(
			...
			AND	pdt.sub_apply_num = trx.doc_ctrl_num
			AND	pdt.sub_apply_type = trx.trx_type
			AND	#aractter_work.territory_code = trx.territory_code
			AND	pyt.non_ar_flag = 0
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 209, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 234, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	num_inv_paid = - invoice_pre.num_inv_paid + ISNULL(( 	SELECT SUM(invoice_post.paid_flag)	
										...
										WHERE	#aractter_work.territory_code = invoice_post.territory_code 
										AND	invoice_post.trx_type <= 2031 ),0.0)
	FROM	#arsumter_pre	invoice_pre
	WHERE	#aractter_work.territory_code = invoice_pre.territory_code

	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 251, 5 ) + " -- EXIT: "
		RETURN 34563
	END	
	...
	IF(@debug_level > 0)
	BEGIN
		SELECT "dumping #aractter_work after update"
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcauta.sp" + ", line " + STR( 271, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCAUpdateTerritoryActivity_SP] TO [public]
GO
