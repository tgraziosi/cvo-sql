...
CREATE PROCEDURE [dbo].[cvo_ST_Activity_log_sp]
    ...
    @detail INT = 0     -- 0 = no, any other value = yes, -1 = H-L version 072418
AS
BEGIN
    ...
    -- exec cvo_ST_Activity_log_sp '06/01/2018','06/30/2018', '20204,20203', -1 , -1
 /*
declare @startdate datetime, @enddate datetime
set @startdate = '06/01/2018'
set @enddate = '06/30/2018'

declare @territory varchar(20), @qualorder int, @detail int
select @territory = '20204,20203', @qualorder = 1, @detail = -1
 ...
 */
    ...
    SELECT o.order_no,
           ...
           WHEN o.status IN ( 'P' ) THEN
               CASE WHEN ISNULL(
                         ...
                         ''
                               ) IN ( 'F', 'S', 'X' ) THEN 'Shipped' ELSE 'Processing'
               END
           WHEN o.status IN ( 'Q' ) THEN 'Processing'
           WHEN o.status IN ( 'R', 'S', 'T' ) THEN 'Shipped'
           WHEN o.status IN ( 'V', 'x' ) THEN 'Void' ELSE ''
           END AS status_desc,
           o.who_entered,
           o.shipped_flag,
           o.hold_reason,
    ...
    SELECT DISTINCT
           ...
           total_amt_order = (CASE o.status WHEN 'T' THEN o.gross_sales ELSE o.total_amt_order END) * -1,
           (CASE o.status WHEN 'T' THEN o.total_discount ELSE o.tot_ord_disc END) * -1 AS total_discount,
           (CASE o.status WHEN 'T' THEN o.total_tax ELSE o.tot_ord_tax END) * -1 AS total_tax,
           (CASE o.status WHEN 'T' THEN o.freight ELSE o.tot_ord_freight END) * -1 AS freight,
           0 AS qty_ordered,
           0 AS qty_shipped,
           (CASE o.status WHEN 'T' THEN o.total_invoice ELSE
           (o.total_amt_order - o.tot_ord_disc + o.tot_ord_tax + o.tot_ord_freight) END
           ) * -1 AS total_invoice,
           ...
           END AS status_desc,
           o.who_entered,
           CASE WHEN o.status IN ( 'r', 's', 't' ) THEN 'Yes' ELSE 'No' END AS shipped_flag,
           o.hold_reason,
           o.orig_no,
           o.orig_ext,
           co.promo_id,
    ...
    FROM #territory AS t
        ...
        SELECT ar.territory_code,
               SUM(CASE WHEN sd.user_category LIKE 'RX%' THEN sd.anet ELSE 0 END) Net_RX,
               SUM(sd.anet) Net_sales
        FROM #territory AS t2 (NOLOCK)
            JOIN dbo.armaster ar (NOLOCK)
                ON ar.territory_code = t2.territory
    ...
    SELECT #temp.cust_code,
           ...
           CASE WHEN ca.door = 0 THEN '' ELSE ISNULL(#temp.ship_to, '') END AS ship_To_door,
           #temp.ship_to_name,
                            -- salesperson,
    ...
    ELSE IF @detail = -1
       ...
       FramesRMA,
	   0 net_rx,
	   isnull(ts.net_sales,0.0) net_sales,
       back_ord_flag,
       Cust_type,
       HS_order_no,
       source 
	From #temp f
	join #territory t on t.territory = f.territory
	        LEFT OUTER JOIN dbo.arsalesp slp (NOLOCK)
            ON slp.territory_code = f.Territory and status_type = 1
	left outer join (select ff.territory, sum(ff.total_amt_order)-sum(ff.total_discount) net_sales
	from #temp ff where ' + @where + ' group by ff.territory) ts on ts.territory = f.territory	
	 where ' + @where);
