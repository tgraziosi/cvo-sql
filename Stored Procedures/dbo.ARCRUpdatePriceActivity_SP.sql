...
CREATE PROC [dbo].[ARCRUpdatePriceActivity_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcrupa.sp", 68, "Entering ARCRUpdatePriceActivity_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 71, 5 ) + " -- ENTRY: "
	...
	SELECT	DISTINCT artrx.price_code
	...
	AND	( LTRIM(artrx.price_code) IS NOT NULL AND LTRIM(artrx.price_code) != " " )
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 92, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	amt_balance = ISNULL(amt_balance, 0.0) - 
		(	
			...
			AND	pdt.db_action = 2
			AND	#aractprc_work.price_code = artrx.price_code
			AND	pyt.non_ar_flag = 0),0.0)
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 120, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 147, 5 ) + " -- EXIT: "
		RETURN 34563
	END

	
	
	SELECT artrx.price_code, trx.trx_type, MAX(trx.trx_ctrl_num) trx_ctrl_num
	INTO	#max_trx
	...
	GROUP BY artrx.price_code, trx.trx_type	
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 172, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	AND	#aractprc_work.price_code = maxtrx.price_code
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 192, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 215, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 241, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 265, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 279, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	GROUP BY artrx.price_code
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 303, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	WHERE	#aractprc_work.price_code = a.price_code		
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 321, 5 ) + " -- EXIT: "
		RETURN 34563
	END

	DROP TABLE #invoices_paid

	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrupa.sp" + ", line " + STR( 327, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCRUpdatePriceActivity_SP] TO [public]
GO
