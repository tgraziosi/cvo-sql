...
CREATE PROC [dbo].[ARCRUpdateShipToSummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcrusts.sp", 72, "Entering ARCRUpdateShipToSummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 75, 5 ) + " -- ENTRY: "
	...
	GROUP BY trx.customer_code, trx.ship_to_code, glprd.period_start_date, glprd.period_end_date
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 118, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 135, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 163, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	num_inv_paid = 
		(
			...
			FROM	#invoices_paid
			WHERE	#invoices_paid.ship_to_code = #arsumshp_work.ship_to_code
			AND	#invoices_paid.customer_code = #arsumshp_work.customer_code
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 181, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 188, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	sum_days_to_pay_off = 
		(
			...
			AND	artrxpdt.db_action = 2
			AND	#arsumshp_work.ship_to_code = invoice.ship_to_code
			AND	#arsumshp_work.customer_code = invoice.customer_code
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 210, 5 ) + " -- EXIT: "
		RETURN 34563
	END	
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 251, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 279, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrusts.sp" + ", line " + STR( 283, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCRUpdateShipToSummary_SP] TO [public]
GO
