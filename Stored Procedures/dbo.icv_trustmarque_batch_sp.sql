...
CREATE PROCEDURE [dbo].[icv_trustmarque_batch_sp] AS
BEGIN
	...
	DECLARE @authorization		CHAR(255)
	DECLARE @ret			INT
	DECLARE @iMonth			INT
	DECLARE @iYear			INT
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @Month = CONVERT(VARCHAR(2), @iMonth)
			SELECT @Year = CONVERT(VARCHAR(4), @iYear)
			...
			BEGIN
				...
				SELECT @buf = "Nonzero return from icv_trustmarque: " + RTRIM(LTRIM(@buf))
				EXEC icv_Log_sp @buf, @LogActivity
			...
			ELSE
			BEGIN
				IF (ISNULL(DATALENGTH(RTRIM(LTRIM(@authorization))),0) = 0)
					SELECT @authorization = "TRANSACTION FAILED"
				SELECT @authorization =''
				UPDATE arinpchg SET hold_flag = 1
				 WHERE trx_ctrl_num = @trx_ctrl_num
				SELECT @arinpchg_rowcount = @@ROWCOUNT
				UPDATE arinppyt SET hold_flag = 1, prompt4_inp = @authorization
				 WHERE trx_ctrl_num = @trx_ctrl_num
				SELECT @arinppyt_rowcount = @@ROWCOUNT
				UPDATE arinptmp SET prompt4_inp = @authorization
				 WHERE trx_ctrl_num = @trx_ctrl_num
				...
				UPDATE arinptmp SET arinptmp.prompt4_inp = @authorization
				 ...
				 WHERE a.trx_ctrl_num = b.trx_ctrl_num
				   AND b.order_ctrl_num = @trx_ctrl_num
				SELECT @arinptmp2_rowcount = @@ROWCOUNT
				SELECT @buf = "Batch booking failed, @trx_ctrl_num: " + @trx_ctrl_num + ", @authorization: " + @authorization
				EXEC icv_Log_sp @buf, @LogActivity
...
END
GO
GRANT EXECUTE ON  [dbo].[icv_trustmarque_batch_sp] TO [public]
GO
