...
-- =============================================
CREATE PROCEDURE [dbo].[KidsPP_ssrs_2016_sp] 
...
, yr = CASE WHEN yyyymmdd >= @P1From THEN 'CY'
		  ...
		  ELSE 'xxx' end
, kidtype = CASE 
			WHEN i.category = 'OP' AND ia.category_2 LIKE '%adult%' THEN '3OP   '
			WHEN i.category = 'JC' AND ia.category_2 LIKE '%adult%' THEN '4JC   '
...
CASE WHEN yyyymmdd >= @P1From THEN 'CY'
		  ...
		  WHEN yyyymmdd >= @p5from THEN 'PY4'
		  ELSE 'xxx' END,
CASE  		WHEN i.category = 'OP' AND ia.category_2 LIKE '%adult%' THEN '3OP'
			WHEN i.category = 'JC' AND ia.category_2 LIKE '%adult%' THEN '4JC'
			WHEN ia.category_2 LIKE '%child%' AND i.category NOT IN ( 'FP','DD' ) THEN '1Kids'
			WHEN ia.category_2 LIKE '%child%'  AND i.category IN ( 'FP','DD' ) THEN '2Pedi'
...
, COUNT(DISTINCT s.promo_id+s.promo_level) NetSales
...
AND s.YYYYMMDD BETWEEN  @P4From and @P1To 
GROUP BY RIGHT(s.customer,5), s.ship_to,
CASE WHEN yyyymmdd >= @P1From THEN 'CY'
		  WHEN yyyymmdd >= @p2from THEN 'PY1'
		  WHEN yyyymmdd >= @p3from THEN 'PY2'
		  WHEN yyyymmdd >= @p4from THEN 'PY3'
...
GO
GRANT EXECUTE ON  [dbo].[KidsPP_ssrs_2016_sp] TO [public]
GO
