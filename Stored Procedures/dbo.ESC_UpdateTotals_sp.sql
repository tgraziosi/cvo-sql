...
CREATE procedure [dbo].[ESC_UpdateTotals_sp]  @ParRecID varchar(40)
as
...
where 		ParentRecID = @ParRecID		-- 08/21/2012 BNM - resolve issue 781, Manual cash application, exclude Manual Credits
and not exists(select 1 from #ManualCredits where #ManualCredits.ParentRecID = ESC_CashAppDet.ParentRecID and #ManualCredits.SeqID = ESC_CashAppDet.SeqID)


GO
GRANT EXECUTE ON  [dbo].[ESC_UpdateTotals_sp] TO [public]
GO
