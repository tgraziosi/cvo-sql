...
CREATE  PROCEDURE [dbo].[glpspost_sp]   @batch_code     varchar(16),
...
BEGIN
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "glpspost.cpp" + ", line " + STR( 128, 5 ) + " -- ENTRY: "
	...
	IF ( @debug_level > 2 ) SELECT "glpspost.cpp" + ", line " + STR( 243, 5 ) + " -- MSG: " + CONVERT(char,@work_time,100) + "Summing debits and credits"
	...
	IF ( @debug_level > 2 ) SELECT "glpspost.cpp" + ", line " + STR( 387, 5 ) + " -- MSG: " + CONVERT(char,@work_time,100) + "Summarizing accounts"
	...
	IF ( @debug_level > 3 ) SELECT CONVERT(char,getdate(),109) + "  " + "glpspost.cpp" + ", line " + STR( 460, 5 ) + " -- MSG: " + 'EXEC glpspstu_sp ' + CONVERT( varchar(10), @post_user_id ) + ', ' + CONVERT( varchar(10), @debug_level )

  	EXEC @result = glpspstu_sp 	@post_user_id,
                                @debug_level


	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "glpspost.cpp" + ", line " + STR( 466, 5 ) + " -- EXIT: "

	RETURN @result

END
GO
GRANT EXECUTE ON  [dbo].[glpspost_sp] TO [public]
GO
