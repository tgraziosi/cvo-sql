...
BEGIN
	...
	CREATE CLUSTERED INDEX #cust_bal_ind ON #class_bal(class_code)
	...
	BEGIN
		...
		SET	exists_flag = 1
		FROM	#class_bal
		WHERE	#class_bal.class_code = #unposted_trx.class_code
		
		
		INSERT #class_bal
		SELECT	class_code, 0.0, 0.0, 
	...
	SET	exists_flag = 1
	FROM	apactcls
	WHERE	apactcls.class_code = #class_bal.class_code
	...
	SET	amt_balance 		= bal.amt_balance_home,
		...
		last_trx_time		 = 1
	FROM	#class_bal	bal
	WHERE	bal.class_code = apactcls.class_code
	AND	exists_flag = 1
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[apuacls_sp] TO [public]
GO
