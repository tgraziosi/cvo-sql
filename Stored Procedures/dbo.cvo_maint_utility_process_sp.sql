...
BEGIN
	...
	BEGIN
		...
		WHERE	(b.location = '001' OR b.location IS NULL)
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					IF EXISTS( SELECT 1 FROM tdc_bin_part_qty (NOLOCK) WHERE location = '001' AND part_no = @part_no AND [primary] = 'Y' AND bin_no <> @bin_no)
					BEGIN
				...
				IF (@bin_repl_max > 0)
				BEGIN
					IF EXISTS( SELECT 1 FROM tdc_bin_replenishment (NOLOCK) WHERE location = '001' AND part_no = @part_no AND bin_no = @bin_no)
					BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN				
				...
				VALUES  ('001', @part_no, @bin_no, @fill_qty_max, 'Y', 0)
			END
			...
			BEGIN				
				...
				VALUES	('001', @bin_no, @part_no, @bin_repl_min, @bin_repl_max, @bin_repl_qty, GETDATE(), 'Upload', 0)
			END
			...
			BEGIN			
				...
				INSERT	tdc_bin_part_qty
				VALUES  ('001', @part_no, @bin_no, @fill_qty_max, 'N', @max_seq)
			END
