...
CREATE PROC [dbo].[ARFLUpdateTerritoryActivity_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arfluta.sp", 79, "Entering ARFLUpdateTerritoryActivity_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 82, 5 ) + " -- ENTRY: "
	...
	IF( @result != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 95, 5 ) + " -- EXIT: "
		RETURN 35011
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 113, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET amt_balance = ISNULL(amt_balance, 0.0) + 
			 	(	
					...
					WHERE	trx_type >= 2061
					AND	trx_type <= 2071
					AND	#aractter_work.territory_code = #artrx_work.territory_code
			 	)
	
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 132, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 151, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
	 	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 166, 5 ) + " -- EXIT: "
	 	RETURN 34563
	END
	...
	WHERE	trx_type >= 2061
	AND	trx_type <= 2071
	GROUP BY territory_code, trx_type

	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 181, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	WHERE	trx.territory_code = m.character_8
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 225, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 241, 5 ) + " -- EXIT: "
		RETURN 34563
	END
		
	IF ( @debug_level >= 2 )
	BEGIN
		SELECT "dumping #aractter_work..."
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arfluta.sp" + ", line " + STR( 256, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARFLUpdateTerritoryActivity_SP] TO [public]
GO
