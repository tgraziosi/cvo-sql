...
CREATE PROC [dbo].[input_plinterorg_xml]
AS
				...
				CREATE TABLE #PLHeaderXML(
					...
					SupplierName	varchar(40),		OrderID					INT,
					PONumber		varchar(18)
				)
				...
				@CompanyCode			varchar(30),		@ValidForMatch		INT,
				@LineNumber				INT
			
				declare @detail_company_id INT,
				@detail_company_name VARCHAR(30)	
				--Data for proccesing
			...
			BEGIN
					...
					SELECT  @keyLineValue = MIN(KeyLineValue) FROM  #PLXML WHERE OrderID = @OrderID AND KeyLineValue >= @keyLineValue
					...
					BEGIN
							...
							select	@SupplierPartNum = SupplierPartNum, @LoginName = LoginName, @account_code = AccountCode,
									...
									@UnitPrice = UnitPrice, @ShortDscrptn = ShortDscrptn, @UnitOfMeasure = UnitOfMeasure, @CompanyCode = CompanyCode,
									@LineNumber = LineNumber 
								FROM #PLXML WHERE KeyLineValue = @keyLineValue

						SELECT @detail_company_id = company_id,  @detail_company_name = company_name
						FROM CVO_Control..smcomp WHERE company_id = @CompanyCode
							...
							SET @count_trx_dtl = @count_trx_dtl + 1
								...
								SELECT 
									...
									@detail_company_id,			@detail_company_name,		@SupplierPartNum,		@ShortDscrptn,
...
GO
GRANT EXECUTE ON  [dbo].[input_plinterorg_xml] TO [public]
GO
