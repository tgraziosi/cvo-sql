...
/* over from there.                                                             */
/********************************************************************************/
CREATE PROCEDURE [dbo].[tdc_ins_receipt_ser_batch]  @err_msg varchar(255) output
AS
...
SELECT @sncounter = count(*) FROM #serial_no
...
--	EXEC fs_calc_receipt_tax @po, @tax_code, @qty, @cost, @tax OUT
	...
	DECLARE sn_cursor CURSOR FOR SELECT serial FROM #serial_no ORDER BY serial
	OPEN sn_cursor
...
RETURN @receipt_no
GO
GRANT EXECUTE ON  [dbo].[tdc_ins_receipt_ser_batch] TO [public]
GO
