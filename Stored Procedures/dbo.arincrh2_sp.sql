...
CREATE PROCEDURE [dbo].[arincrh2_sp]	@module_id			smallint,
					@val_mode			smallint,
...
BEGIN
	...
	SELECT	@home_currency = home_currency,
		...
		@home_override_flag = 0,
		@oper_override_flag = 0
	FROM	glco	

	
	SELECT	@home_override_flag = ISNULL(override_flag,0)
	FROM 	glcurate_vw
	...
	SELECT	@oper_override_flag = ISNULL(override_flag,0)
	FROM 	glcurate_vw
	WHERE	from_currency = @nat_cur_code 
...
END
GO
GRANT EXECUTE ON  [dbo].[arincrh2_sp] TO [public]
GO
