...
Copyright (c) 1996 Platinum Software Corporation, 1996
                 All Rights Reserved 
 */
                                         CREATE PROC [dbo].[gl_gleslcalc_sp]  @home_ctry_code varchar(3), 
 @esl_period_id int AS BEGIN DECLARE  @return_code int,  @from_date int,  @to_date int, 
 @post_flag smallint,  @esl_ctrl_root varchar(16),  @esl_ctrl_num varchar(16),  @esl_err_num varchar(16), 
 @rpt_cur_code varchar(8)     SELECT @from_date = from_date,  @to_date = to_date, 
 @post_flag = post_flag,  @esl_ctrl_root = esl_ctrl_root,  @esl_ctrl_num = esl_ctrl_num, 
 @esl_err_num = esl_err_num,  @rpt_cur_code = rpt_cur_code  FROM gl_gleslhdr  WHERE home_ctry_code = @home_ctry_code AND esl_period_id = @esl_period_id 
 ...
 IF @@rowcount <> 1  BEGIN  ROLLBACK TRANSACTION  RETURN 8141  END     EXEC @return_code = gl_gleslclear_sp @home_ctry_code, @esl_period_id, @esl_ctrl_root, @esl_ctrl_num, @esl_err_num 
...
(
 ...
 vat_reg_num, rate_type  FROM gl_glinphdr  WHERE home_ctry_code = @home_ctry_code AND date_applied <= @to_date AND esl_ctrl_num = '' 
 ...
 BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     EXEC @return_code = gl_gleslcurcvt_sp @home_ctry_code, @rpt_cur_code 
 IF @return_code <> 0  BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     EXEC @return_code = gl_gleslagrdet_sp 
 @home_ctry_code, @esl_ctrl_root, @esl_ctrl_num, @esl_err_num, @from_date  IF @return_code <> 0 
 BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     EXEC @return_code = gl_gleslflush_sp 
 IF @return_code <> 0  BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     DROP TABLE #gl_glinpdet 
 DROP TABLE #gl_glinphdr  DROP TABLE #gl_glinterr     EXEC @return_code = gl_gleslrounddet_sp @home_ctry_code, @esl_ctrl_num, @esl_err_num 
 IF @return_code <> 0  BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     EXEC @return_code = gl_gleslupdhdr_sp @home_ctry_code, @esl_period_id, @esl_ctrl_num, @esl_err_num 
 IF @return_code <> 0  BEGIN  ROLLBACK TRANSACTION  RETURN @return_code  END     UPDATE gl_gleslhdr SET post_flag = 0 WHERE home_ctry_code = @home_ctry_code AND esl_period_id = @esl_period_id 
 IF @@error <> 0  BEGIN  ROLLBACK TRANSACTION  RETURN 8100  END  COMMIT TRANSACTION 
 RETURN 0 END 
GO
GRANT EXECUTE ON  [dbo].[gl_gleslcalc_sp] TO [public]
GO
