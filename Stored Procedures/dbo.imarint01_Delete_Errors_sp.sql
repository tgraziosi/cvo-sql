...
GO
    ...
    CREATE PROC
[dbo].[imarint01_Delete_Errors_sp] @imarint01_Delete_Errors_sp_Transaction_Type INT = 2031,
                           @userid INT = 0
    AS
    ...
    SET @Routine_Name = 'imarint01_Delete_Errors_sp'        
    --
    -- Obtain company_code
    --
    ...
    DELETE 
            FROM [imicmerr_vw]
            WHERE LTRIM(RTRIM([company_code])) = @company_code
-- Enable this line after adding Transaction Type to the error table
--                  AND trx_type = @imarint01_Delete_Errors_sp_Transaction_Type
                    AND ([User_ID] = @userid OR @userid = 0)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_DELETE + ' imicmerr_vw 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    DELETE [perror]
            FROM [perror] a
    ...
    DELETE [perror]
            FROM [perror] a
            INNER JOIN [imarhdr_vw] b
                    ON a.trx_ctrl_num = b.source_ctrl_num
            WHERE LTRIM(RTRIM([company_code])) = @company_code
                    AND trx_type = @imarint01_Delete_Errors_sp_Transaction_Type
                    AND (b.[User_ID] = @userid  OR @userid = 0)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_DELETE + ' perror 3' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    DELETE [perror]
            FROM [perror] a
            INNER JOIN [imarhdr_vw] b
            ...
            WHERE LTRIM(RTRIM([company_code])) = @company_code
                    AND trx_type = @imarint01_Delete_Errors_sp_Transaction_Type
                    AND (b.[User_ID] = @userid  OR @userid = 0)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_DELETE + ' perror 4' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    RETURN 0
Error_Return:
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarint01_Delete_Errors_sp] TO [public]
GO
