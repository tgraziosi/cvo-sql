...
CREATE PROC [dbo].[ARUPDatePriceSummary_SP] 
...
(	price_code 	varchar(8), 
	...
	AND	( LTRIM(age.price_code) IS NOT NULL AND LTRIM(age.price_code) != " " )
	GROUP BY age.price_code, prd.period_start_date, prd.period_end_date
	...
	AND	( LTRIM(a.price_code) IS NOT NULL AND LTRIM(a.price_code) != " " )
	GROUP BY a.price_code, prd.period_start_date, prd.period_end_date


	UPDATE	#price_tot
	SET	#price_tot.amt_inv = a.amt_inv,
		#price_tot.amt_cm = a.amt_cm,
		#price_tot.amt_adj = a.amt_adj,
		#price_tot.amt_fin_chg = a.amt_fin_chg,
		#price_tot.amt_late_chg = a.amt_late_chg,
		#price_tot.amt_inv_oper = a.amt_inv_oper,
		#price_tot.amt_cm_oper = a.amt_cm_oper,
		#price_tot.amt_adj_oper = a.amt_adj_oper,
		#price_tot.amt_fin_chg_oper = a.amt_fin_chg_oper,
		#price_tot.amt_late_chg_oper = a.amt_late_chg_oper,
		#price_tot.num_inv = a.num_inv,
		#price_tot.num_inv_paid = a.num_inv_paid,
		#price_tot.num_cm = a.num_cm,
		#price_tot.num_adj = a.num_adj,
		#price_tot.num_fin_chg = a.num_fin_chg,
		#price_tot.num_late_chg = a.num_late_chg,
		#price_tot.amt_tax = a.amt_tax,
		#price_tot.amt_tax_oper = a.amt_tax_oper,
		#price_tot.amt_freight = a.amt_freight,
		#price_tot.amt_freight_oper = a.amt_freight_oper,
		#price_tot.amt_disc_given = a.amt_disc_given,
		#price_tot.amt_disc_g_oper = a.amt_disc_g_oper,
		#price_tot.days_pay = a.days_pay,
		#price_tot.days_overdue = a.days_overdue
	FROM	#artrx_tot a
	WHERE	#price_tot.price_code = a.price_code
	AND	a.date_from = #price_tot.date_from
	AND	a.date_thru = #price_tot.date_thru
	...
	FROM	artrxage age, artrxpdt pdt, #price_tot p
	WHERE	age.price_code = p.price_code
	AND	pdt.doc_ctrl_num = age.doc_ctrl_num
	AND	pdt.payer_cust_code = age.payer_cust_code
	AND	pdt.sequence_id = age.ref_id
	AND	pdt.date_applied between p.date_from and p.date_thru 
	AND	( LTRIM(age.price_code) IS NOT NULL AND LTRIM(age.price_code) != " " )
	AND	pdt.trx_type = 2111
	AND	age.trx_type IN (2111, 2032)	
	AND	pdt.void_flag = 0	
	GROUP BY age.price_code, p.date_from, p.date_thru

	
	UPDATE	#price_tot
	SET	#price_tot.amt_pyt = a.amt_pyt,
		#price_tot.amt_pyt_oper = a.amt_pyt_oper
	FROM	#amt_pyt_tot a
	WHERE	#price_tot.price_code = a.price_code
	AND	a.date_from = #price_tot.date_from
	AND	a.date_thru = #price_tot.date_thru
	...
	SET	exists_flag = 1
	FROM	arsumprc
	WHERE	arsumprc.price_code = #price_tot.price_code
	AND	arsumprc.date_thru = #price_tot.date_thru
	...
	SET	date_from = tot.date_from,
		...
		last_trx_time = 1
	FROM	#price_tot tot
	WHERE	arsumprc.price_code = tot.price_code
	AND	arsumprc.date_thru = tot.date_thru
	AND	tot.exists_flag = 1

	
	INSERT	arsumprc (	price_code, 		date_from, 		date_thru,
	...
	SELECT	price_code, 	date_from, 		date_thru,
		num_inv, 		num_inv_paid, 	num_cm,
		num_adj,		num_wr_off,		num_pyt,
		num_overdue_pyt,	num_nsf,		num_fin_chg,
		num_late_chg,		amt_inv, 		amt_cm, 
	...
	DROP TABLE #price_tot

GO
GRANT EXECUTE ON  [dbo].[ARUPDatePriceSummary_SP] TO [public]
GO
