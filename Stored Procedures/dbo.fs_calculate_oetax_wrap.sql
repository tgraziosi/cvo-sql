...
CREATE PROCEDURE [dbo].[fs_calculate_oetax_wrap] @ord int, @ext int,
@debug int = 0, @batch_call int = 0, @doctype int = 0, 
@trx_ctrl_num varchar(16) = '', @err_msg varchar(255) = '' OUT
AS
BEGIN
set nocount on

if @batch_call = 2 -- get results
begin
  select reference_number, (amt_tax), (amt_tax_included),
    (calc_tax), (tax_included), (non_recoverable_tax), (non_rec_frt_tax), (non_rec_incl_tax)
  from #online_taxinfo
  where control_number = @ord and trx_type = 3 and reference_number = 0

  return 1
end

if @batch_call = 4 -- get results
begin
  select tax_code, unit_price, extended_price, amt_tax, amt_final_tax
  from #online_taxinfo
  where control_number = @ord and trx_type = -1 
    and (action_flag = @doctype or @doctype = -1)
...
create index c1 on #TXcents(cents_code,row_id)
	...
	DECLARE @err int
        ...
        if @batch_call  = 0
  	EXEC dbo.adm_calculate_oetax @ord, @ext, @err OUT, @doctype, @trx_ctrl_num, 1,
          @debug
	else
  	EXEC dbo.adm_calculate_oetax @ord, @ext, @err OUT, @doctype, @trx_ctrl_num, 0,
          @debug

        if @batch_call <= 0
		begin
		  exec adm_get_tax_detail @ord, @ext, @err
		end
        else
        begin
		  exec adm_get_tax_detail @ord, @ext, @err, 0, @err_msg out
  	      return @err
		end 
END 

GO
GRANT EXECUTE ON  [dbo].[fs_calculate_oetax_wrap] TO [public]
GO
