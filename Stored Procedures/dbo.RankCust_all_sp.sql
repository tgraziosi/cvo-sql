...
BEGIN
    ...
    SELECT -- t1.customer_code ,
        ...
        CASE WHEN contact_email IS NULL THEN ''
            ...
            WHEN contact_email LIKE '%refused%' THEN '' ELSE contact_email
        END AS contact_email,
        ISNULL(addr_sort1, '') AS CustType,
        CASE WHEN coop_eligible = '' THEN 'N' ELSE ISNULL(coop_eligible, 'N') END AS coop_eligible,
        ISNULL(AR30 + AR60 + AR90 + AR120 + AR150, 0) PastDueAmt,
        designations.desig Designations,
        p.code PriDesig,
        p.start_date,
        p.end_date,
        CASE WHEN t1.customer_code = art.parent THEN '' ELSE art.parent END AS Parent,
        CASE WHEN t1.status_type = 1 THEN 'A' ELSE 'I' END AS Status,
        t1.price_code
    FROM @terr_tbl t
        INNER JOIN armaster t1 (NOLOCK)
            ON t.terr = t1.territory_code
        INNER JOIN artierrl (NOLOCK) art
            ON art.rel_cust = t1.customer_code
        INNER JOIN @bg_tbl bg
            ON bg.bg = CASE WHEN art.rel_cust = art.parent THEN '' ELSE art.parent END
        LEFT OUTER JOIN CVO_armaster_all T2 (NOLOCK)
            ON t1.customer_code = T2.customer_code
               AND t1.ship_to_code = T2.ship_to
        LEFT OUTER JOIN SSRS_ARAging_Temp ar
            ON ar.CUST_CODE = t1.customer_code
        LEFT OUTER JOIN
        (
        ...
        FROM dbo.cvo_cust_designation_codes (NOLOCK) c
            ...
            ON designations.MergeCust = RIGHT(t1.customer_code, 5)
        LEFT OUTER JOIN
        (
        ...
        WHERE primary_flag = 1
            ...
            ON p.MergeCust = RIGHT(t1.customer_code, 5)
    WHERE (
          t1.address_type = 0
          AND @CustOpt = 'B'
          )
          OR (
             t1.address_type IN ( 0, 1 )
             AND @CustOpt = 'S'
             )
    ) custinfo
    GROUP BY MergeCust,
    ...
    CREATE CLUSTERED INDEX idx_rankcust3
    ON #RankCusts_S3
    (
    MergeCust,
    ship_to
    ...
    IF @ActiveDesig IS NOT NULL
       AND @ActiveDesig <> '*ALL*' -- only report customers in the active designations they want
    BEGIN
        DELETE FROM #RankCusts_S3
        WHERE MergeCust + ship_to NOT IN (
                                         SELECT rcs.MergeCust + rcs.ship_to cust
                                         FROM #RankCusts_S3 AS rcs (NOLOCK)
                                             JOIN dbo.cvo_cust_designation_codes AS cdc (NOLOCK)
                                                 ON MergeCust = RIGHT(cdc.customer_code, 5)
                                             JOIN @activedes_tbl AS a
                                                 ON a.code = cdc.code
    ...
    SELECT RIGHT(customer, 5) MergeCust,
           t2.customer,
           ship_to = CASE WHEN @CustOpt = 'B' THEN '' ELSE t2.ship_to END,
           t2.promo_id,
           t2.return_code,
           t2.user_category,
           t2.yyyymmdd,
    ...
    FROM @coll_tbl c
        INNER JOIN inv_master (NOLOCK) INV
            ON INV.category = c.collection
        INNER JOIN cvo_sbm_details (NOLOCK) t2
            ON t2.part_no = INV.part_no
        INNER JOIN armaster ar (NOLOCK)
            ON t2.customer = ar.customer_code
               AND t2.ship_to = ar.ship_to_code
        INNER JOIN @terr_tbl t
            ON t.terr = ar.territory_code

    WHERE (
          t2.yyyymmdd
          BETWEEN @DateFromTY AND @DateToTY
          OR t2.yyyymmdd
    ...
    GROUP BY RIGHT(t2.customer, 5),
             CASE WHEN @CustOpt = 'B' THEN '' ELSE t2.ship_to END,
             CASE WHEN t2.yyyymmdd BETWEEN @DateFromTY AND @DateToTY THEN 'TY' ELSE 'LY' END,
             t2.customer,
             t2.promo_id,
             t2.return_code,
             t2.user_category,
    ...
    CREATE NONCLUSTERED INDEX idx_sales ON #sales (MergeCust, ship_to);
    ...
    SET @PrYr1To = DATEADD(MILLISECOND, -3, DATEADD(YEAR, DATEDIFF(YEAR, 0, DATEADD(YEAR, -1, @DateToTY)) + 1, 0));
    ...
    BEGIN
        ...
        FROM cvo_sbm_details sbm (NOLOCK)
            JOIN inv_master inv (NOLOCK)
                ON sbm.part_no = inv.part_no
            JOIN @coll_tbl c
                ON inv.category = c.collection
        WHERE yyyymmdd
        BETWEEN DATEADD(yy, -2, @PrYr1From) AND @PrYr1To
    ...
    CREATE NONCLUSTERED INDEX idx_py3
    ON #p3yrdata
    (
    customer,
    ship_to
    ...
    SELECT RIGHT(customer, 5) Customer,
           ...
           END AS RetURaTY_R12,
           CASE WHEN yyyymmdd
                     BETWEEN DATEADD(YEAR, -2, DATEADD(dd, 1, DATEDIFF(dd, 0, @DateToTY))) AND DATEADD(
                                                                                                          YEAR,
                                                                                                          -1,
                                                                                                          @DateToTY
                                                                                                      ) THEN 'LY' ELSE
    ...
    FROM cvo_sbm_details t1 (NOLOCK)
        INNER JOIN inv_master t2 (NOLOCK)
            ON t1.part_no = t2.part_no
    WHERE yyyymmdd
    BETWEEN DATEADD(YEAR, -2, DATEADD(dd, 1, DATEDIFF(dd, 0, @DateToTY))) AND @DateToTY
    GROUP BY Customer,
             CASE WHEN @CustOpt = 'B' THEN '' ELSE t1.ship_to END,
             promo_id,
             return_code,
             type_code,
             yyyymmdd
    ...
    CREATE NONCLUSTERED INDEX idx_r12
    ON #R12
    (
    Customer,
    ship_to
    ...
    IF (OBJECT_ID('tempdb.dbo.#RXEFREIGHT') IS NOT NULL)
        DROP TABLE #RXEFREIGHT;
    ...
    SELECT t1.order_no,
           ...
           dbo.f_cvo_FreightRateLookup(t1.routing, LEFT(t1.ship_to_zip, 5), t4.cs_dim_weight) AS UnchargedRates,
           rxDesig = CASE WHEN t3.promo_level = '5' THEN 'rx5' ELSE 'rx3' END
    INTO #RXEFREIGHT
    FROM orders_all t1 (NOLOCK)
        INNER JOIN CVO_armaster_all T2 (NOLOCK)
            ON t1.cust_code = T2.customer_code
               AND t1.ship_to = T2.ship_to
        INNER JOIN CVO_orders_all t3 (NOLOCK)
            ON t1.order_no = t3.order_no
               AND t1.ext = t3.ext
        INNER JOIN tdc_carton_tx t4 (NOLOCK)
    ...
    GROUP BY t1.order_no,
             ...
             CASE WHEN @CustOpt = 'B' THEN '' ELSE t1.ship_to END,
             dbo.f_cvo_FreightRateLookup(t1.routing, LEFT(t1.ship_to_zip, 5), t4.cs_dim_weight),
    ...
    WITH salesdata
    AS (SELECT t1.MergeCust AS customer,
               t1.ship_to,
               -- SALES
    ...
    SELECT DISTINCT
           ...
           WHERE A.cust_code = B.cust_code
                 AND A.ship_to = CASE WHEN @CustOpt = 'B' THEN '' ELSE B.ship_to END
    ...
    -- 13,165
    CREATE INDEX IDX_LASTST
    ON #LastST2
    (
    Cust,
    ship_to
    )
    INCLUDE (last_st_ord_date);
    ...
    SELECT Status Stat,
           ...
           CASE WHEN EXISTS (SELECT 1 FROM c_quote cq WHERE RIGHT(cq.customer_key, 5) = T1.customer) THEN 'Y' ELSE 'N' END AS ContrPrcing,
           CASE WHEN goal1 > DesigNetSTY THEN goal1 - DesigNetSTY WHEN goal2 > DesigNetSTY THEN goal2 - DesigNetSTY ELSE
                                                                                                                        0 END AS DolToNextRebate,
           CASE WHEN DesigNetSTY >= goal2 THEN DesigNetSTY * rebatepct2
               WHEN DesigNetSTY >= goal1 THEN DesigNetSTY * rebatepct1 ELSE 0
           END AS RebateEarned,
           ...
           ar.m_Postal_code
    FROM #FinalData1 T1
        LEFT OUTER JOIN #RankCusts_S3 S3
            ON S3.MergeCust = T1.customer
               AND S3.ship_to = T1.ship_to
        LEFT OUTER JOIN cvo_designation_rebates T2
            ON T2.code = ISNULL(S3.PriDesig, '')
               AND progyear = DATEPART(YEAR, @DateToTY)
        LEFT OUTER JOIN
        (
        SELECT customer_code,
               SUM(coop_earned) coop_earned,
               SUM(coop_redeemed) coop_redeemed
        FROM #coopdata
        WHERE yyear = DATEPART(YEAR, @DateToTY)
        GROUP BY customer_code
        ) coop
            ON coop.customer_code = T1.customer
        -- LEFT OUTER JOIN #coopdata ON t1.customer = #coopdata.customer_code AND yyear = DATEPART(YEAR, @DateToTY)
        LEFT OUTER JOIN #LastST t3
            ON T1.customer = t3.Cust
               AND T1.ship_to = t3.ship_to
        LEFT OUTER JOIN #R12 r12_ty
            ON T1.customer = r12_ty.Customer
               AND T1.ship_to = r12_ty.ship_to
               AND r12_ty.Years = 'TY'
        LEFT OUTER JOIN #R12 r12_ly
            ON T1.customer = r12_ly.Customer
               AND T1.ship_to = r12_ly.ship_to
               AND r12_ly.Years = 'LY'
        LEFT OUTER JOIN
        (
        SELECT Cust_code,
        ...
        GROUP BY Cust_code,
            ...
            ON T1.customer = RX.Cust_code
               AND T1.ship_to = RX.ship_to
        LEFT OUTER JOIN
        (
        SELECT customer,
               ship_to,
        ...
        GROUP BY customer,
            ...
            ON T1.customer = PY.customer
               AND T1.ship_to = PY.ship_to
        LEFT OUTER JOIN
        (
        SELECT DISTINCT
