...
-- DECLARE	@custom_count int -- v10.1 v10.3
	...
	DECLARE @is_quoted smallint -- v12.2
	DECLARE @is_fixed smallint -- v12.2
	...
	SELECT	@invoice_note = ISNULL(REPLACE(invoice_note, CHAR(13) + CHAR(10), ' ') ,'')	
...
--	END
	...
	IF (ISNULL(@parent,'') <> '')
	BEGIN
		SELECT	@BG_Order = ISNULL(alt_location_code,0)
		FROM	arcust (NOLOCK)
	...
	BEGIN  
		...
		EXEC dbo.cvo_get_pack_list_details_sp @order_no, @order_ext, @Location, '0','1' -- v12.2
		...
		*/
		...
		SELECT	a.part_no,
				...
				a.line_no -- v12.3
		FROM	#detail a
		LEFT JOIN inv_master b (NOLOCK)
		ON		a.part_no = b.part_no
		LEFT JOIN #part_type c
		ON		b.type_code = c.part_type
		ORDER BY ISNULL(c.printorder,3), a.part_no
		-- v10.2 end 
		...
		*/
		...
		-- v10.1 End
			...
			-- v12.3 Start
			EXEC dbo.cvo_get_invoice_line_prices_sp	@order_no, @order_ext, @line_no, @cust_code, @sum_pack_qty, @c_gross_price OUTPUT, @c_discount_amount OUTPUT,
			...
			BEGIN
				...
				BEGIN		
					...
					BEGIN
						SELECT	@list_price	= CAST(@c_net_price AS DECIMAL (20,2)),				
								@order_ext_list = @c_net_price * @Sum_Line_Ship,
								@gross_price	= CAST(@c_ext_net_price AS DECIMAL (20,2)), 
								@net_price		= CAST(@c_net_price AS DECIMAL (20,2)), 
								@ext_net_price	= CAST(@c_net_price AS DECIMAL (20,2)), 
			...
			ELSE
			BEGIN
				SELECT 	@gross_price	= CAST(@c_ext_net_price AS DECIMAL(20,2)), 
						@net_price		= CAST(@c_net_price AS DECIMAL(20,2)),							
						...
						@list_price		= CAST(@c_gross_price AS DECIMAL(20,2))
