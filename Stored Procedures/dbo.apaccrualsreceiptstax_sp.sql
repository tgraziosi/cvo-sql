...
CREATE PROC [dbo].[apaccrualsreceiptstax_sp]  
as  
...
select @tmp_ctrl_num = MIN(tmp_ctrl_num) from #receipts_tmp  
...
BEGIN  
 ...
 SELECT r.receipt_no, @h_doc_type, @trx_type, tc_companycode, getdate(), '', '',  
   0,                    '', r.vendor,                 '',           3,     '',   
  v.addr1 ,        v.addr2 ,                v.addr3 , v.city ,  v.state ,  
  v.postal_code , v.country_code ,  
  p.ship_address1,    p.ship_address2,     p.ship_address3,    
  p.ship_city,  p.ship_state, p.ship_zip,  p.ship_country_cd  
 FROM   receipts r   
  left outer join purchase p (nolock) on p.po_no = r.po_no   
  JOIN apmaster_all v (nolock) ON (v.vendor_code = r.vendor)  
  JOIN Organization_all o (nolock) ON (o.organization_id = r.organization_id)  
 WHERE r.receipt_no = @tmp_ctrl_num  
 ...
 SELECT receipts.receipt_no, pur_list.line, receipts.nat_curr,  
   c.curr_precision, pur_list.tax_code, (receipts.quantity), pur_list.unit_cost,   
   (SIGN(pur_list.unit_cost * (receipts.quantity)) * ROUND(ABS(pur_list.unit_cost * (receipts.quantity)) + 0.0000001, c.curr_precision)), pur_list.line  
 FROM receipts  
  ...
  join glcurr_vw c (nolock) on (receipts.nat_curr = c.currency_code)  
 WHERE ( receipts.receipt_no = @tmp_ctrl_num )   
 ...
 FROM  #TXtaxcode tc  
  join    #TXtaxtyperec ttr on ttr.tc_row = tc.row_id    join    #TXtaxtype tt on tt.ttr_row = ttr.row_id  
 ...
 select @tmp_ctrl_num = MIN(tmp_ctrl_num)   
 from #receipts_tmp where tmp_ctrl_num > @tmp_ctrl_num  
  
END   
  
UPDATE  dtl  
SET dtl.account_code = cdt.account_no  
FROM #tmpinptaxdtl dtl   
 JOIN receipts cdt (nolock) ON (cdt.receipt_no = dtl.trx_ctrl_num)  
    
/**/                                                
GO
GRANT EXECUTE ON  [dbo].[apaccrualsreceiptstax_sp] TO [public]
GO
