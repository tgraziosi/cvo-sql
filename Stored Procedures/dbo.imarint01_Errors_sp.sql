...
GO
    ...
    CREATE PROC
[dbo].[imarint01_Errors_sp] (@imarint01_Errors_sp_Dummy_1 VARCHAR(128) = '',
                     @imarint01_Errors_sp_Table_Indicator VARCHAR(128) = '',
                     @imarint01_Errors_sp_Dummy_2 VARCHAR(128) = '',
                     @imarint01_Errors_sp_record_id_num INT = -1, 
                     @imarint01_Errors_sp_Transaction_Type INT,
                     @userid INT = 0)
    AS
    ...
    SET @Routine_Name = 'imarint01_Errors_sp'        
    --
    -- Obtain company_code
    --
    ...
    -- perror.trx_ctrl_num = staging.source_ctrl_num for "trial"
    ...
    IF @imarint01_Errors_sp_record_id_num = -1
        ...
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_CREATETABLE + ' #imarint01_Errors_sp_1 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END   
		...
		BEGIN
            ...
            --
            SET @SQL = 'IF EXISTS (SELECT [TABLE_NAME] FROM [' + @ewcomp_cursor_db_name + '].INFORMATION_SCHEMA.TABLES WHERE [TABLE_NAME] = ''imarhdr_vw'')'
                        + ' INSERT INTO [#imarint01_Errors_sp_1]'
                                + ' SELECT CAST(SUBSTRING(''['' + CAST(ISNULL(a.[err_code], 0) AS VARCHAR) + '']'' + SPACE(43), 1, 45) + ISNULL([err_desc], ISNULL(a.[err_code], '''')) AS VARCHAR(250)) AS ''Text'','
                                       ...
                                       + ' a.[extra]' -- For alternate style import report
                                        ...
                                        + ' INNER JOIN [CVO_Control]..[imarhdr] c'
                                                + ' ON (a.[trx_ctrl_num] = c.[trx_ctrl_num] OR a.[trx_ctrl_num] = c.[source_ctrl_num] OR a.[source_ctrl_num] = c.[source_ctrl_num])'
                                        + ' WHERE a.module_id = 2000'
                                                + ' AND (b.e_active = 1 OR b.e_active = -1)'
                                                + ' AND c.[trx_type] = ' + CAST(@imarint01_Errors_sp_Transaction_Type AS VARCHAR)
    ...
    ELSE
        BEGIN
        IF UPPER(@imarint01_Errors_sp_Table_Indicator) = 'H'
            BEGIN
            SELECT DISTINCT CAST(SUBSTRING('[' + CAST(ISNULL(a.[err_code], 0) AS VARCHAR) + ']' + SPACE(43), 1, 45) + ISNULL([err_desc], ISNULL(a.[err_code], '')) + ' ' + (CASE [infoint] WHEN 0 THEN '' ELSE CAST([infoint] AS VARCHAR) END) AS VARCHAR(250)) AS 'Text',
                   c.record_id_num,
                   CAST(ISNULL(c.[Import Identifier], 0) AS VARCHAR) AS 'Import Identifier'
                    FROM perror a 
                    INNER JOIN aredterr b 
                            ON a.err_code = b.e_code
                    INNER JOIN [imarhdr_vw] c 
                            ON (a.[trx_ctrl_num] = c.[trx_ctrl_num] OR a.[trx_ctrl_num] = c.[source_ctrl_num] OR a.[source_ctrl_num] = c.[source_ctrl_num]) 
                    WHERE a.module_id = 2000
                            AND (b.e_active = 1 OR b.e_active = -1)
                            AND c.record_id_num = @imarint01_Errors_sp_record_id_num
            ...
            SELECT CAST(SUBSTRING('[' + CAST(ISNULL(a.[err_code], 0) AS VARCHAR) + ']' + SPACE(43), 1, 45) + ISNULL([e_ldesc], ISNULL(a.[err_code], '')) AS VARCHAR(250)) AS 'Text',
                   ...
                   CAST(ISNULL(c.[Import Identifier], 0) AS VARCHAR) AS 'Import Identifier'
                    ...
                    WHERE a.module_id = 6000
                            ...
                            AND (a.[sequence_id] = 0 OR a.[sequence_id] IS NULL)
            END                
        IF UPPER(@imarint01_Errors_sp_Table_Indicator) = 'D'
            BEGIN
            SELECT CAST(SUBSTRING('[' + CAST(ISNULL(a.[err_code], 0) AS VARCHAR) + ']' + SPACE(43), 1, 45) + ISNULL([err_desc], ISNULL(a.[err_code], '')) AS VARCHAR(250)) AS 'Text',
                   c.record_id_num,
                   CAST(ISNULL(c.[Import Identifier], 0) AS VARCHAR) AS 'Import Identifier'
...
Error_Return:
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarint01_Errors_sp] TO [public]
GO
