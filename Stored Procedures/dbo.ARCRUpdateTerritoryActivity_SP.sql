...
CREATE PROC [dbo].[ARCRUpdateTerritoryActivity_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcruta.sp", 68, "Entering ARCRUpdateTerritoryActivity_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 71, 5 ) + " -- ENTRY: "
	...
	SELECT	DISTINCT artrx.territory_code
	...
	AND	( LTRIM(artrx.territory_code) IS NOT NULL AND LTRIM(artrx.territory_code) != " " )
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 92, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	amt_balance = ISNULL(amt_balance, 0.0) - 
		(
			...
			AND	pdt.db_action = 2
			AND	#aractter_work.territory_code = artrx.territory_code
			AND	pyt.non_ar_flag = 0),0.0)
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 120, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 147, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SELECT artrx.territory_code, trx.trx_type, MAX(trx.trx_ctrl_num) trx_ctrl_num
	INTO	#max_trx
	...
	GROUP BY artrx.territory_code, trx.trx_type	
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 173, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	AND	#aractter_work.territory_code = maxtrx.territory_code
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 193, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 216, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 242, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 266, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 281, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	GROUP BY artrx.territory_code
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 305, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	WHERE	#aractter_work.territory_code = a.territory_code		
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 323, 5 ) + " -- EXIT: "
		RETURN 34563
	END

	DROP TABLE #invoices_paid

	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcruta.sp" + ", line " + STR( 329, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCRUpdateTerritoryActivity_SP] TO [public]
GO
