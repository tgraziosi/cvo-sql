...
CREATE PROCEDURE [dbo].[cvo_process_approved_adhoc_adj_sp]
AS
...
BEGIN
	...
	UPDATE	dbo.CVO_adhoc_adjust_approval
	SET		approve = -5
	WHERE	approve = -1
	AND		direction <> -2
	...
	FROM	dbo.CVO_adhoc_adjust_approval (NOLOCK)
	WHERE	approve = -5
	ORDER BY id ASC	
...
--		AND		code = @reason_code
	...
	DELETE	dbo.CVO_adhoc_adjust_approval 
	WHERE	approve = -7


END
GO
GRANT EXECUTE ON  [dbo].[cvo_process_approved_adhoc_adj_sp] TO [public]
GO
