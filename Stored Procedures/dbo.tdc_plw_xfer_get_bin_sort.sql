...
ELSE IF (EXISTS (SELECT * FROM tdc_config   
	    	  WHERE [function] = 'alloc_bin_sort_xfer' 
	      	    AND active     = 'Y' 
...
BEGIN
	SELECT @xfer_inv_pick = value_str FROM tdc_config (NOLOCK) WHERE [function] = 'dist_xfer_pick'

	SELECT @bin_sort = CASE	@xfer_inv_pick				 
				WHEN '1' THEN 'LIFO' 
				WHEN '2' THEN 'FIFO'
...
RETURN						 
GO
GRANT EXECUTE ON  [dbo].[tdc_plw_xfer_get_bin_sort] TO [public]
GO
