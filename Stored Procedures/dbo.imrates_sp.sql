...
GO
    ...
    CREATE PROC 
[dbo].[imrates_sp] @company_code VARCHAR(8), 
    ...
    IF (@debug_level >= 3)  
        BEGIN
        SELECT @buf = '(3): ' + @Routine_Name + ': ' + CAST(@Row_Count AS VARCHAR) + ' rate_type_home records defaulted'
        SELECT @buf
        END
    UPDATE #imaphdr_vw
    ...
    IF (@debug_level >= 3)  
        BEGIN
        SELECT @buf = '(3): ' + @Routine_Name + ': ' + CAST(@Row_Count AS VARCHAR) + ' rate_type_oper records defaulted'
        SELECT @buf
        END
    UPDATE #imaphdr_vw
    ...
    IF (@debug_level >= 3)  
        BEGIN
        SELECT @buf = '(3): ' + @Routine_Name + ': ' + CAST(@Row_Count AS VARCHAR) + ' nat_cur_code records defaulted'
        SELECT @buf
        END
    ...
    UPDATE #imaphdr_vw
            ...
            WHERE a.nat_cur_code = b.from_currency
                    ...
                    AND RTRIM(LTRIM(ISNULL(a.company_code, ''))) = @company_code
                    AND (a.rate_home = 0.0 OR a.rate_home IS NULL)
    ...
    UPDATE #imaphdr_vw
            ...
            WHERE a.nat_cur_code = b.from_currency
                    ...
                    AND datediff(dd, @January_First_Nineteen_Eighty, CONVERT(datetime,a.date_applied))+722815 = b.date_applied
                    AND RTRIM(LTRIM(ISNULL(a.company_code, ''))) = @company_code
                    AND (a.rate_oper = 0.0 OR a.rate_oper IS NULL)
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imrates_sp] TO [public]
GO
