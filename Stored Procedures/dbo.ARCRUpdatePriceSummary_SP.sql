...
CREATE PROC [dbo].[ARCRUpdatePriceSummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcrups.sp", 72, "Entering ARCRUpdatePriceSummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 75, 5 ) + " -- ENTRY: "
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 116, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 133, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 159, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	num_inv_paid = 
		(
			...
			FROM	#invoices_paid
			WHERE	#invoices_paid.price_code = #arsumprc_work.price_code
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 176, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 183, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	sum_days_to_pay_off = 
		(
			...
			AND	artrxpdt.db_action = 2
			AND	( LTRIM(invoice.price_code) IS NOT NULL AND LTRIM(invoice.price_code) != " " )
			AND	arinppyt.date_applied > invoice.date_doc
			AND	#arsumprc_work.price_code = invoice.price_code
		)
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 205, 5 ) + " -- EXIT: "
		RETURN 34563
	END	
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 246, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 274, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcrups.sp" + ", line " + STR( 278, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCRUpdatePriceSummary_SP] TO [public]
GO
