...
CREATE PROCEDURE [dbo].[cvo_BG_order_tracker_sp]
-- @promo varchar(20), 
@startdate datetime, @enddate datetime, @parent varchar(10)
-- updates
-- 10/30 - make promo multi-value list - need to remove parameter
 /*
...
-- exec [cvo_BG_order_tracker_sp] '11/1/2014','11/26/2014' , '000507'
...
0 as return_qty,
o.source
, c.customer_name bg_name

into #temp

FROM  cvo_adord_vw AS o WITH (nolock) 
left outer join artierrl ar (nolock) on o.cust_code = ar.rel_cust
left outer join arcust c (nolock) on c.customer_code = ar.parent
where 1=1
and ar.parent = @parent
...
GO
GRANT EXECUTE ON  [dbo].[cvo_BG_order_tracker_sp] TO [public]
GO
