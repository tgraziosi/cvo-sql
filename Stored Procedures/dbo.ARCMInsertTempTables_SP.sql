...
CREATE PROC [dbo].[ARCMInsertTempTables_SP]	@process_ctrl_num	varchar(16),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 138, 'Entering ARCMInsertTempTables_SP', @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 141, 5 ) + ' -- ENTRY: '

		
	


	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 147, 'Start inserting unposted invoice headers into #arinpchg_work', @PERF_time_last OUTPUT
	...
	WHERE	batch_code = @batch_ctrl_num
    	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 212, 5 ) + ' -- EXIT: '
       	RETURN 34563
	END
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 224, 'Done inserting unposted invoice headers into #arinpchg_work', @PERF_time_last OUTPUT

	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 226, 'Start inserting unposted invoice detail into #arinpcdt_work', @PERF_time_last OUTPUT
	...
	SELECT
		...
		ISNULL(d.cust_po, ''),	d.org_id
	FROM	arinpcdt d, #arinpchg_work h
    	WHERE	d.trx_ctrl_num = h.trx_ctrl_num 
      	AND   	d.trx_type = h.trx_type
    	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 262, 5 ) + ' -- EXIT: '
		RETURN 34563
	END
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 274, 'Done inserting unposted invoice headers into #arinpcdt_work', @PERF_time_last OUTPUT

	


	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 279, 'Start inserting unposted invoice tax details into #arinptax_work', @PERF_time_last OUTPUT
    	...
    	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 297, 5 ) + ' -- EXIT: '
		RETURN 34563
	END
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 300, 'Done inserting unposted invoice tax details into #arinptax_work', @PERF_time_last OUTPUT
	
	



	EXEC @result = ARCMInsertDetailTables_SP	@batch_ctrl_num,
							@debug_level,
							@perf_level
	IF( @result != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 311, 5 ) + ' -- EXIT: '
		RETURN 34563
	END
	...
	IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, 'arcmitt.cpp', 322, 'Leaving ARCMInsertTempTables_SP', @PERF_time_last OUTPUT
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'arcmitt.cpp' + ', line ' + STR( 323, 5 ) + ' -- EXIT: '
	RETURN 0
END
GO
GRANT EXECUTE ON  [dbo].[ARCMInsertTempTables_SP] TO [public]
GO
