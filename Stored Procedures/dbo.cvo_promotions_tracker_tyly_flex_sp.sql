...
-- exec cvo_promotions_tracker_tyly_flex_sp '11/1/2016','01/30/2017', '11/1/2015','10/31/2016',  null , 'sunps', '1,2,3,NEW'
 -- exec cvo_promotions_tracker_tyly_flex_sp '11/1/2016', '2/17/2017', '11/1/2015', '2/17/2016',  null , 'SM', null
...
--				   EXEC cvo_promotions_tracker_tyly_sp '1/1/2016','8/22/2016', '50505','aspire','1,3,launch,new,vew'
        ...
        SELECT  ar.territory_code terr ,
                ...
                ship_to_code = CASE WHEN car.door = 0 THEN ''
                                    ELSE ar.ship_to_code
                               END ,
                SUM(CASE WHEN sbm.yyyymmdd >= @datefrom
                         THEN ISNULL(sbm.anet, 0)
                         ELSE 0
                    END) net_sales_ty ,
                SUM(CASE WHEN sbm.yyyymmdd <= @datetoly
                         THEN ISNULL(sbm.anet, 0)
                         ELSE 0
                    END) net_sales_ly
        INTO    #salesdata
        FROM    ( SELECT DISTINCT
                ...
                INNER JOIN CVO_armaster_all car ( NOLOCK ) ON car.customer_code = ar.customer_code
                                                              AND car.ship_to = ar.ship_to_code
                INNER JOIN cvo_sbm_details sbm ( NOLOCK ) ON sbm.customer = ar.customer_code
...
-- get rid of any rolled up customers not in this territory (i.e. 030774)
        ...
        FROM    #salesdata s
                INNER JOIN armaster ar ( NOLOCK ) ON ar.customer_code = s.customer
                                                     AND ar.ship_to_code = s.ship_to_code;

        DELETE  FROM #salesdata
        WHERE   NOT EXISTS ( SELECT 1
                             FROM   #temptable
                             WHERE  Territory = #salesdata.terr );	
 ...
 -- select * from #salesdata
        ...
        SELECT  t.salesperson ,
                t.Territory ,
                t.region ,
                CASE WHEN t.id = (SELECT min (t2.id) FROM #temptable AS t2 
								WHERE t.territory = t2.Territory) THEN ISNULL(door.Num_ActiveCust,0) ELSE 0 END AS ActiveCustCntLY ,
                t.promo_id ,
                t.promo_level ,
        ...
        FROM    ( SELECT    slp.salesperson_code salesperson ,
                ...
                LEFT OUTER JOIN ( SELECT    terr ,
                                            COUNT(DISTINCT active.customer
                                                  + active.ship_to_code) Num_ActiveCust
                                  FROM      ( SELECT    terr ,
