...
CREATE PROC [dbo].[ARUPDateTerrSummary_SP] 
...
DECLARE	@precision_oper	smallint,
	...
	AND	( LTRIM(age.territory_code) IS NOT NULL AND LTRIM(age.territory_code) != " " ) 
	GROUP BY age.territory_code, prd.period_start_date, prd.period_end_date
	...
	AND	( LTRIM(a.territory_code) IS NOT NULL AND LTRIM(a.territory_code) != " " )
	GROUP BY a.territory_code, prd.period_start_date, prd.period_end_date


	UPDATE	#terr_tot
	SET	#terr_tot.amt_inv = a.amt_inv,
		#terr_tot.amt_cm = a.amt_cm,
		#terr_tot.amt_adj = a.amt_adj,
		#terr_tot.amt_fin_chg = a.amt_fin_chg,
		#terr_tot.amt_late_chg = a.amt_late_chg,
		#terr_tot.amt_inv_oper = a.amt_inv_oper,
		#terr_tot.amt_cm_oper = a.amt_cm_oper,
		#terr_tot.amt_adj_oper = a.amt_adj_oper,
		#terr_tot.amt_fin_chg_oper = a.amt_fin_chg_oper,
		#terr_tot.amt_late_chg_oper = a.amt_late_chg_oper,
		#terr_tot.num_inv = a.num_inv,
		#terr_tot.num_inv_paid = a.num_inv_paid,
		#terr_tot.num_cm = a.num_cm,
		#terr_tot.num_adj = a.num_adj,
		#terr_tot.num_fin_chg = a.num_fin_chg,
		#terr_tot.num_late_chg = a.num_late_chg,
		#terr_tot.amt_tax = a.amt_tax,
		#terr_tot.amt_tax_oper = a.amt_tax_oper,
		#terr_tot.amt_freight = a.amt_freight,
		#terr_tot.amt_freight_oper = a.amt_freight_oper,
		#terr_tot.amt_disc_given = a.amt_disc_given,
		#terr_tot.amt_disc_g_oper = a.amt_disc_g_oper,
		#terr_tot.days_pay = a.days_pay,
		#terr_tot.days_overdue = a.days_overdue
	FROM	#artrx_tot a
	WHERE	#terr_tot.territory_code = a.territory_code
	AND	a.date_from = #terr_tot.date_from
	AND	a.date_thru = #terr_tot.date_thru
	...
	FROM	artrxage age, artrxpdt pdt, #terr_tot t
	WHERE	age.territory_code = t.territory_code
	AND	pdt.doc_ctrl_num = age.doc_ctrl_num
	AND	pdt.payer_cust_code = age.payer_cust_code
	AND	pdt.sequence_id = age.ref_id
	AND	pdt.date_applied between t.date_from and t.date_thru 
	AND	( LTRIM(age.territory_code) IS NOT NULL AND LTRIM(age.territory_code) != " " )
	AND	pdt.trx_type = 2111
	AND	age.trx_type IN (2111, 2032)	
	AND	pdt.void_flag = 0	
	GROUP BY age.territory_code, t.date_from, t.date_thru

	
	UPDATE	#terr_tot
	SET	#terr_tot.amt_pyt = a.amt_pyt,
		#terr_tot.amt_pyt_oper = a.amt_pyt_oper
	FROM	#amt_pyt_tot a
	WHERE	#terr_tot.territory_code = a.territory_code
	AND	a.date_from = #terr_tot.date_from
	AND	a.date_thru = #terr_tot.date_thru
	...
	SET	exists_flag = 1
	FROM	arsumter
	WHERE	arsumter.territory_code = #terr_tot.territory_code
	AND	arsumter.date_thru = #terr_tot.date_thru
	...
	SET	date_from = tot.date_from,
		...
		last_trx_time = 1
	FROM	#terr_tot tot
	WHERE	arsumter.territory_code = tot.territory_code
	AND	arsumter.date_thru = tot.date_thru
	AND	tot.exists_flag = 1

	
	INSERT	arsumter (	territory_code, 	date_from, 		date_thru,
	...
	SELECT	territory_code, 	date_from, 		date_thru,
		num_inv, 		num_inv_paid, 	num_cm,
		num_adj,		num_wr_off,		num_pyt,
		num_overdue_pyt,	num_nsf,		num_fin_chg,
		num_late_chg,		amt_inv, 		amt_cm, 
...
GO
GRANT EXECUTE ON  [dbo].[ARUPDateTerrSummary_SP] TO [public]
GO
