...
-- EXEC dbo.cvo_email_ship_confirmation_sp 2818917, 0, '045700', '', 'tgraziosi@epicor.com'
...
BEGIN
	...
	BEGIN
		...
		--SELECT	TOP (1) @body_text = @body_text + '<a href="' + @tracking_url + 
		--	(CASE WHEN ISNULL(cs_tracking_no,'') <> '' THEN CAST(cs_tracking_no AS VARCHAR(20)) ELSE CAST(carton_no AS VARCHAR(20)) END) 
		--	 + '">' + 
		--	(CASE WHEN ISNULL(cs_tracking_no,'') <> '' THEN CAST(cs_tracking_no AS VARCHAR(20)) ELSE CAST(carton_no AS VARCHAR(20)) END) 
		--	 + '</a><BR>'
		--FROM	tdc_carton_tx (NOLOCK)
