...
CREATE PROCEDURE [dbo].[adm_loc_so_access_view] @typ int, @loc varchar(10), @user varchar(255) AS
set nocount on
...
if @@rowcount <> 0
begin
  select @user_create_sec = write from smperm where app_id = 18000 and user_id = @user_id and company_id = @comp_id and form_id = 16765
  select @user_use_sec = write from smperm where app_id = 18000 and user_id = @user_id and company_id = @comp_id and form_id = 16762
end
...
from adm_so_locorgrel_vw r
left outer join Organization o (nolock) on r.related_org_id = o.organization_id 
where r.location = @loc and r.create_ind = 1
and (related_org_id not in (select organization_id from adm_organization where io_create_so_ind = 0 )
...
from adm_so_locorgrel_vw r 
left outer join Organization o (nolock) on r.related_org_id = o.organization_id 
where r.location = @loc and r.use_ind = 1
and (related_org_id not in (select organization_id from adm_organization where io_use_so_ind = 0)
or related_org_id in (select organization_id from locations_all where location = @loc))
...
GO
GRANT EXECUTE ON  [dbo].[adm_loc_so_access_view] TO [public]
GO
