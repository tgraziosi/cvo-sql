...
CREATE PROCEDURE [dbo].[cvo_dc_dash_recv_sp]
    @recalc_usage INT = 0,
    @det INT = 0,
    @days_out INT = 5
AS
...
BEGIN
    SELECT DISTINCT
           @lastusgupdate = asofdate
    FROM ##cvo_usage;
    IF DATEDIFF(DAY, @lastusgupdate, @asofdate) > 1
        SELECT @recalc_usage = 1;
END;

IF @recalc_usage = 1
   OR (OBJECT_ID('tempdb.dbo.##cvo_usage') IS NULL)
BEGIN

    IF (OBJECT_ID('tempdb.dbo.##cvo_usage') IS NOT NULL)
        DROP TABLE ##cvo_usage;
    ...
    FROM dbo.f_cvo_calc_weekly_usage_coll('o', NULL) AS fccwu
        LEFT JOIN dbo.cvo_item_avail_vw AS iav
            ON iav.location = fccwu.location
               AND iav.part_no = fccwu.part_no;
...
IF @det = -1
BEGIN
    SELECT DATEDIFF(d, @asofdate, ISNULL(r.inhouse_date, r.confirm_date)) due_days,
           ...
           MAX(   CASE
                      WHEN ia.field_26 > @asofdate THEN
                          'New Release'
                      WHEN ISNULL(drp.qty_avl, 0) < 0
                           AND ISNULL(drp.open_ord, 0) > 0 THEN
                          'Backorders'
                      ELSE
                          ''
                  END
           ...
           qty_open = CASE
                          WHEN SUM(qty_ordered - qty_received) < 0 THEN
                              0
                          ELSE
    ...
    FROM pur_list p (NOLOCK)
        ...
        INNER JOIN dbo.apmaster_all AS aa (NOLOCK)
            ON aa.vendor_code = pa.vendor_no
        LEFT JOIN releases r (NOLOCK)
            ON p.po_key = r.po_key
               AND p.line = r.po_line
        LEFT JOIN inv_master_add ia (NOLOCK)
            ON p.part_no = ia.part_no
        LEFT JOIN ##cvo_usage AS drp (NOLOCK)
            ON p.part_no = drp.part_no
               AND p.location = drp.location
        LEFT OUTER JOIN
        (
            SELECT DISTINCT
                   ppp.po_key,
            ...
            ON b.po_key = pa.po_key
    WHERE p.po_key = p.po_no
          AND pa.po_no = pa.po_key
          AND r.po_key = r.po_no
          AND p.status = 'o'
          AND p.void <> 'v'
          AND DATEDIFF(d, @asofdate, ISNULL(r.inhouse_date, r.confirm_date))
          BETWEEN -@days_out AND @days_out
...
BEGIN
    SELECT DATEDIFF(d, @asofdate, ISNULL(r.inhouse_date, r.confirm_date)) due_days,
           ...
           (
               ...
               FROM dbo.receipts_all AS ra2 (NOLOCK)
               WHERE ra2.po_key = pa.po_key
           ) Last_receipt_date,
           CASE
               WHEN ISNULL(p.plrecd, 0) = 1 THEN
                   'Yes-L'
               WHEN ISNULL(pa.expedite_flag, 0) = 1 THEN
                   'Yes-H'
               ELSE
                   'No'
           END AS Pk_lst
...
BEGIN
    ...
    SELECT DATEDIFF(d, @asofdate, ISNULL(r.inhouse_date, r.confirm_date)) due_days,
           ...
           qty_received,
           qty_open = CASE
                          WHEN qty_ordered - qty_received < 0 THEN
                              0
                          ELSE
           ...
           DATEDIFF(
                       d,
                       ISNULL(r.inhouse_date, r.confirm_date),
                       ISNULL(ra.max_recv_date, ISNULL(r.inhouse_date, r.confirm_date))
                   ) recv_days,
           CASE
               WHEN ISNULL(p.plrecd, 0) = 1 THEN
                   'Yes-L'
           ...
           END AS Pk_lst,
           p.status,
           status_desc = CASE p.status + p.void
                             WHEN 'ON' THEN
                                 'Open'
                             WHEN 'OO' THEN
                                 'Open'
           ...
           CASE
               ...
               WHEN p.ship_via_method IN ( 0, 2 ) THEN
                   'Ocean'
               WHEN p.ship_via_method IS NULL
                    AND pa.ship_via_method = 0 THEN
                   'Ocean'
               WHEN p.ship_via_method IS NULL
                    AND pa.ship_via_method = 1 THEN
                   'Air'
               WHEN p.ship_via_method IS NULL
                    AND pa.ship_via_method = 2 THEN
                   'Ocean'
               ELSE
                   'Ocean'
           END AS Method,
    ...
    FROM pur_list p (NOLOCK)
        ...
        (
            SELECT po_key,
                   po_line,
                   MAX(rr.receipt_no) max_receipt,
                   MAX(rr.recv_date) max_recv_date
            FROM dbo.receipts_all AS rr (NOLOCK)
            GROUP BY rr.po_key,
                     rr.po_line
        ) ra
            ON ra.po_key = p.po_key
               AND ra.po_line = r.po_line
    WHERE p.po_key = p.po_no
          AND pa.po_no = pa.po_key
          AND r.po_key = r.po_no
          AND
          (
              ...
              OR
              (
                  ra.max_recv_date IS NOT NULL
                  AND ra.max_recv_date >= DATEADD(dd, DATEDIFF(dd, 0, @asofdate), 0)
              )
          )
          AND
          (
              DATEDIFF(d, @asofdate, ISNULL(r.inhouse_date, r.confirm_date))
          BETWEEN -@days_out AND @days_out
              OR
              (
                  ra.max_recv_date IS NOT NULL
