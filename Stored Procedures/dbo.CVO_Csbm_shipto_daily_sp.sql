...
-- =============================================
-- exec [dbo].[CVO_csbm_shipto_daily_sp]
-- select * from cvo.dbo.cvo_csbm_shipto order by customer, ship_to, month 
-- select * From tempdb.dbo.#cvo_csbm_det
/* 9/5/2012 - make qty's frames and suns only, and add unposted AR */


CREATE PROCEDURE [dbo].[CVO_Csbm_shipto_daily_sp]
AS
BEGIN
...
if (object_id('cvo.dbo.cvo_csbm_shipto_daily') is not null)
	drop table cvo.dbo.cvo_csbm_shipto_daily

if (object_id('cvo.dbo.cvo_csbm_shipto_daily') is null)
 begin
...
(
 ...
 CREATE NONCLUSTERED INDEX [idx_cvo_csbm_shipto] ON [dbo].[cvo_csbm_shipto_daily] 
 (
	[Customer] ASC,
	[ship_to] asc,
	[month] ASC,
...
--
--case when x.trx_type = '2031' then isnull(x.qty_shipped,0) else 0 end as qsales,
--case when x.trx_type = '2032' then isnull(x.qty_returned,0) else 0 end as qreturns
...
-- cast ((cast([x_month] as varchar(2))+'/01/'+cast([year] as varchar(4))) as datetime) as yyyymmdd 
 ...
 from #cvo_csbm_det
 group by customer, ship_to, year, X_MONTH, [month], dateshipped
