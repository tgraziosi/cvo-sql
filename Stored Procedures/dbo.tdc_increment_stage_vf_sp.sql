...
CREATE PROCEDURE [dbo].[tdc_increment_stage_vf_sp]
@date     DATETIME, 
...
@ctr		INT 
	...
	SELECT @tmp_date_str=convert(char,@date,112)

	--Build the counter string eg: '001'
	SELECT @ctr = 1
	SELECT @counter_str = CAST(@ctr AS varchar(3))
	WHILE LEN(@counter_str) < 2
		SELECT @counter_str = '0' + @counter_str

	--Build the first stage number
	SELECT @tmp_stage_no = LTRIM(RTRIM(@tmp_date_str)) + 'V' + LTRIM(RTRIM(@counter_str))
...
GO
GRANT EXECUTE ON  [dbo].[tdc_increment_stage_vf_sp] TO [public]
GO
