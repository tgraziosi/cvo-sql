...
--v2.1	TM	Highbay Replen was picking up Custom Frame moves.  Added check on Priority

CREATE PROCEDURE [dbo].[CVO_replenish_list]
...
-- replaced line feeds, carriage returns and tabs in attempt to eliminate gaps in report DMoon 1/31/2012
-- changed sort order for Highbay replenishment -- DMoon 03/5/2012
	If @group_code = 'HIGHBAY' --DM HIGHBAY LOOP - PRIORITY THEN BIN
	Begin 
		DECLARE detail_cursor CURSOR FOR 
			SELECT  a.tran_id,
					REPLACE(REPLACE(REPLACE(a.part_no, CHAR(13), ''),CHAR(10),''),CHAR(9),''), 
					REPLACE(REPLACE(REPLACE(a.bin_no, CHAR(13), ''),CHAR(10),''),CHAR(9),''),  
					REPLACE(REPLACE(REPLACE(a.qty_to_process, CHAR(13), ''),CHAR(10),''),CHAR(9),''), 
					REPLACE(REPLACE(REPLACE(a.next_op, CHAR(13), ''),CHAR(10),''),CHAR(9),''), 
					a.priority 
				FROM  tdc_pick_queue a (nolock), tdc_bin_master b (nolock) 
				where a.next_op = b.bin_no 
				and trans = 'MGTB2B'
				and a.trans_type_no = 0  -- take out custom frame moves
				and b.group_code =  'hIGHBAY'
				and a.priority < 9					--v2.1
				order by a.priority, a.bin_no
	END
	ELSE
	BEGIN
		DECLARE detail_cursor CURSOR FOR 
	...
	BEGIN
		...
		INSERT INTO #tdc_print_ticket (print_value) SELECT 'LP_FROM_BIN_' + RTRIM(CAST(@printed_on_the_page AS char(4))) + ',' + 	 ISNULL(@from_bin_no, '')
...
GO
GRANT EXECUTE ON  [dbo].[CVO_replenish_list] TO [public]
GO
