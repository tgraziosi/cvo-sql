...
CREATE PROCEDURE [dbo].[tdc_print_ctn_pack_list_sp](  
   @user_id    varchar(50),  
...
--GET THE ORDER NO AND EXT FOR THE (SINGLE) ORDER CARTON_NO THAT WAS PASSED IN AS A PARAMETER  
...
EXEC @return_value = tdc_print_label_sp 'PPS', 'CTNPACKTKT', 'VB', @station_id  
  
-- IF label hasn't been set up for the station id, try finding a record for the user id  
IF @return_value != 0  
BEGIN  
 EXEC @return_value = tdc_print_label_sp 'PPS', 'CTNPACKTKT', 'VB', @user_id  
END  
...
BEGIN  
 ...
 BEGIN  
  ...
  ELSE  
  BEGIN  
   SELECT TOP 1 @item_description = [description] -- SCR 35090  
  ...
  -------------- Now let's insert the Details into the output table -----------------  
	...
	--JVM 09/14/2010 
	  ...
	  DECLARE @gross_price VARCHAR (40), @net_price VARCHAR (40), @discount_amount VARCHAR (40)
	  ...
	  ELSE
			...
			SELECT @gross_price     = CAST((shipped * curr_price) AS DECIMAL (20,2)),                                       --as Line_1_Gross,
				   @net_price       = CAST(((shipped * curr_price * ((100-discount)/100)) + total_tax)AS DECIMAL (20,2)),    --as Line_1_Net, 
				   @discount_amount = CAST((shipped * curr_price * (discount/100)) AS DECIMAL (20,2))                       --as Line_1_DiscAmt
			FROM   ord_list
...
RETURN  
GO
GRANT EXECUTE ON  [dbo].[tdc_print_ctn_pack_list_sp] TO [public]
GO
