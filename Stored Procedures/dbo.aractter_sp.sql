...
CREATE PROC [dbo].[aractter_sp]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/aractter.sp" + ", line " + STR( 46, 5 ) + " -- ENTRY: "

IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 48, "entry aractter_sp", @PERF_time_last OUTPUT
...
BEGIN
	UPDATE	#aractter_work
	SET	update_flag = 1
	FROM	#aractter_work a, aractter b
	WHERE	a.territory_code = b.territory_code

	SELECT	@status = @@error

	IF ( @perf_level >= 3 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 65, "update #aractter_work", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 124, "insert aractter", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	FROM	aractter p, #aractter_work t
	WHERE	p.territory_code = t.territory_code

	SELECT	@status = @@error

	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 191, "update aractter: delta values", @PERF_time_last OUTPUT
...
BEGIN	
	...
	WHERE	p.num_inv_paid != 0 AND
		p.territory_code = t.territory_code

	SELECT @status = @@error

	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 241, "update aractter: avg_days_overdue", @PERF_time_last OUTPUT
END
...
BEGIN
	...
	IF ( @perf_level >= 2 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 256, "update aractter: avg_days_pay", @PERF_time_last OUTPUT


END

IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/aractter.sp", 261, "exit aractter_sp", @PERF_time_last OUTPUT

RETURN @status

GO
GRANT EXECUTE ON  [dbo].[aractter_sp] TO [public]
GO
