...
CREATE PROC [dbo].[ARCAUpdateShipToSummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcausts.sp", 55, "Entering ARCAUpdateShipToSummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcausts.sp" + ", line " + STR( 58, 5 ) + " -- ENTRY: "
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcausts.sp" + ", line " + STR( 106, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	num_inv_paid = - invoice_pre.num_inv_paid + ISNULL(( 	SELECT SUM(invoice_post.paid_flag)	
										...
										WHERE	#arsumshp_work.ship_to_code = invoice_post.ship_to_code
										AND	#arsumshp_work.customer_code = invoice_post.customer_code 
										AND	invoice_post.trx_type <= 2031 ),0.0)
	FROM	#arsumshp_pre	invoice_pre
	WHERE	#arsumshp_work.ship_to_code = invoice_pre.ship_to_code
	AND	#arsumshp_work.customer_code = invoice_pre.customer_code

	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcausts.sp" + ", line " + STR( 129, 5 ) + " -- EXIT: "
		RETURN 34563
	END	
	...
	SET	num_nsf = (	SELECT count(b.doc_ctrl_num) 	 
				...
				AND	#arsumshp_work.customer_code = c.customer_code
				AND	#arsumshp_work.ship_to_code = c.ship_to_code
				AND	b.trx_type = 2121
			 )
	...
	SET	amt_nsf = (	SELECT SUM(ROUND(a.amt_applied * ( SIGN(1 + SIGN(c.rate_home))*(c.rate_home) + (SIGN(ABS(SIGN(ROUND(c.rate_home,6))))/(c.rate_home + SIGN(1 - ABS(SIGN(ROUND(c.rate_home,6)))))) * SIGN(SIGN(c.rate_home) - 1) ), @home_precision)) 	 
				...
				AND	#arsumshp_work.customer_code = b.customer_code
				AND	#arsumshp_work.ship_to_code = b.ship_to_code
				AND	a.trx_type = 2121
			 )
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcausts.sp" + ", line " + STR( 176, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCAUpdateShipToSummary_SP] TO [public]
GO
