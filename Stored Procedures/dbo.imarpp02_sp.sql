...
GO
    ...
    CREATE PROCEDURE 
[dbo].[imarpp02_sp] @trx_ctrl_num varchar(16),
    ...
    SET @Routine_Name = 'imarpp02_sp'        
    INSERT #arinptmp (trx_ctrl_num, doc_ctrl_num,  trx_desc,
                      date_doc,     customer_code, payment_code,
                      amt_payment,  prompt1_inp,   prompt2_inp,
                      prompt3_inp,  prompt4_inp,   amt_disc_taken,
                      cash_acct_code)
            VALUES (@trx_ctrl_num, @prepay_doc_num, @trx_desc,
                    @date_doc,     @customer_code,  @payment_code,
                    @prepay_amt,   @prompt1_inp,    @prompt2_inp,
                    @prompt3_inp,  @prompt4_inp,    @prepay_discount,
                    @cash_acct_code)
    SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_INSERT + ' #arinptmp 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
    RETURN 0
Error_Return:
    IF @debug_level >= 3
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarpp02_sp] TO [public]
GO
