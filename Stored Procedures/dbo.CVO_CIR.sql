...
FROM    dbo.inv_master_add a JOIN dbo.inv_master b ON b.part_no = a.part_no
WHERE   a.field_17 <> 0
...
CREATE TABLE #part_info ( [part_no] varchar(30), [style] varchar(40), [category] varchar(10), [part_no2] nvarchar(max), [POM_date] datetime, [obsolete] smallint )
...
declare @bg TABLE ( customer_code varchar(8), discount varchar(8), buying varchar(80) ) 

INSERT INTO @bg (customer_code, discount, buying)
...
LEFT OUTER JOIN dbo.arnarel B ( NOLOCK ) ON ar.customer_code = B.child;

--
IF ( OBJECT_ID('tempdb.dbo.#cust_info') IS NOT NULL )
DROP TABLE #cust_info;

CREATE TABLE #cust_info ( [cust_code] varchar(8), [ship_to] varchar(8), [address_name] varchar(40), [addr2] varchar(40), [addr3] varchar(40), [addr4] varchar(40), [contact_phone] varchar(30), [city] varchar(40), [postal_code] varchar(15), [customer_short_name] varchar(8000), [date_opened] varchar(23), [territory] varchar(8) )
...
'Date Opened '
+ SUBSTRING(CAST(( CAST(FLOOR(CAST(b.added_by_date AS FLOAT)) AS DATETIME) ) AS VARCHAR (25)),
1, 11) date_opened ,
b.territory_code territory -- v5.3 082813 - tag
FROM    dbo.armaster b ( NOLOCK );

CREATE NONCLUSTERED INDEX idx_cust_info_1 ON #cust_info (territory, cust_code, ship_to);
...
INNER JOIN dbo.artrx t5 ( NOLOCK ) ON oi.trx_ctrl_num = t5.trx_ctrl_num 
--v3.1
...
CREATE TABLE #last_st ( [cust_code] varchar(10), [ship_to] varchar(8), [last_st_ord_date] datetime, [last_st_order_no] varchar(10), [territory] varchar(8) )

CREATE NONCLUSTERED INDEX idx_ord ON #vsordList (territory, cust_code, ship_to, 
...
CREATE NONCLUSTERED INDEX idx_ls ON #last_st (territory, cust_code, ship_to,  last_st_ord_date);
...
ELSE cs.ship_to
END AS ship_to ,
SUM(CASE WHEN cs.yyyymmdd BETWEEN @F12 AND @last THEN cs.anet
ELSE 0
END) AS Custnetsales ,
SUM(CASE WHEN cs.yyyymmdd BETWEEN @f24 AND DATEADD(yy, -1, @last)
THEN cs.anet
ELSE 0
END) AS NetSalesLY ,
SUM(CASE WHEN cs.yyyymmdd BETWEEN @TYstart AND @last THEN cs.anet
ELSE 0
END) AS YTDTY ,
SUM(CASE WHEN cs.yyyymmdd BETWEEN @LYstart AND DATEADD(yy, -1, @last)
THEN cs.anet
ELSE 0
END) AS YTDLY

FROM    dbo.cvo_sbm_details AS cs ( NOLOCK )
INNER JOIN dbo.armaster ar ( NOLOCK ) ON cs.customer = ar.customer_code
AND cs.ship_to = ar.ship_to_code
WHERE   cs.yyyymmdd BETWEEN @f24 AND @last
...
CREATE NONCLUSTERED INDEX idx_cs ON #cs (territory_code, customer, ship_to);
...
END OTH36,
-- v4.4
ISNULL(( SELECT   SUM(cs.anet)
FROM     dbo.cvo_sbm_details AS cs (nolock)
JOIN dbo.armaster ar (NOLOCK) ON ar.customer_code = cs.customer
...
DECLARE @cust_swap table ( [from_cust] varchar(8), [shipto] varchar(8), [to_cust] varchar(8), [territory] varchar(8) )
...
INNER JOIN dbo.armaster b ( NOLOCK ) ON a.affiliated_cust_code = b.customer_code
...
CREATE TABLE #new ( [territory] varchar(8), [cust_code] varchar(10), [ship_to] varchar(8), [part_no] varchar(30), [TimeEntered] datetime, [custnetsales] real, [netsalesly] real, [ytdty] real, [ytdly] real, [ST12] decimal(22,8), [ST36] decimal(22,8), [RX12] decimal(22,8), [RX36] decimal(22,8), [RET12] decimal(22,8), [RET36] decimal(22,8), [NET12] decimal(22,8), [net36] decimal(22,8), [OTH12] decimal(22,8), [OTH36] decimal(22,8), [TotAcctNetSales] real )
INSERT INTO #new
...
new.TotAcctNetSales = ISNULL(( SELECT   SUM(cs.anet)
...
FROM    @cust_swap cs
INNER JOIN dbo.cvo_carbi cir ON cs.to_cust = cir.cust_code;
