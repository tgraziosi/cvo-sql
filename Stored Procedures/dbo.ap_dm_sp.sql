...
CREATE PROC [dbo].[ap_dm_sp]  @batch_ctrl_num varchar( 16 )
AS
BEGIN
...
DECLARE
	...
	BEGIN TRANSACTION
		...
		SELECT @key = STR(4092, 4) + CONVERT(char(16), h.trx_ctrl_num),
		...
		FROM apdmhdr h
			INNER JOIN apvend v ON h.vendor_code = v.vendor_code
			INNER JOIN gl_glctry c ON v.country_code = c.country_code
		...
		BEGIN
			...
			EXEC @return_code = ap__setaphdr_sp
				@trx_ctrl_num,
				@po_ctrl_num,
				@vend_order_num,
				0,
			...
			BEGIN
				...
				OR (d.location_code = l.location
					...
					AND l.country_code IN
						(SELECT country_code FROM gl__glctry)))
			...
			BEGIN
				DELETE FROM gl_glinphdr WHERE
					(@trx_ctrl_num = src_doc_num AND src_trx_id = "4092") OR
			...
			WHERE h.batch_code = @batch_ctrl_num   
			AND @prev_key < STR(4092, 4) + CONVERT(char(16), h.trx_ctrl_num)
			ORDER BY h.trx_ctrl_num
		END
	COMMIT TRANSACTION
...
END
GO
GRANT EXECUTE ON  [dbo].[ap_dm_sp] TO [public]
GO
