...
-- START v5.1 
 ...
 BEGIN
	...
	*/
	...
	SELECT @cursor_statement =	'DECLARE detail_cursor CURSOR FOR  
								...
								pick_qty, part_no, lot_ser, bin_no, item_note, CAST(tran_id AS varchar(10)), carton_id 
								FROM #pick_ticket   
	 ...
	 SELECT @non_qty_statement =	'UNION      
									...
									pick_qty = ordered, part_no, NULL AS lot_ser, NULL AS bin_no, note, NULL AS tran_id, ' + CAST(@max_carton_id + 1 AS VARCHAR(10)) + ' AS carton_id     
									FROM ord_list (NOLOCK)  
									WHERE order_no  =   ' + cast(@order_no as varchar(30)) +  
									' AND order_ext =   ' + cast(@order_ext as varchar(4)) +  
									' AND location  = ''' +      @location                 + '''  
 ...
 BEGIN
	...
	SELECT @cursor_statement =	'DECLARE detail_cursor CURSOR FOR  
								SELECT line_no, part_type,  uom, [description], ord_qty, dest_bin,  
								pick_qty, part_no, lot_ser, bin_no, item_note, CAST(tran_id AS varchar(10)),1 AS carton_id 
								FROM #so_pick_ticket   
								WHERE order_no  =   ' + cast(@order_no as varchar(30)) +  
								' AND order_ext =   ' + cast(@order_ext as varchar(4)) +  
								' AND location  = ''' +      @location                 + ''''  
	...
	SELECT @non_qty_statement =	'UNION      
								SELECT line_no, part_type,  uom, [description], ordered, NULL AS dest_bin,  
								pick_qty = ordered, part_no, NULL AS lot_ser, NULL AS bin_no, note, NULL AS tran_id,1 AS carton_id   
								FROM ord_list (NOLOCK)  
								WHERE order_no  =   ' + cast(@order_no as varchar(30)) +  
								' AND order_ext =   ' + cast(@order_ext as varchar(4)) +  
								' AND location  = ''' +      @location                 + '''  
