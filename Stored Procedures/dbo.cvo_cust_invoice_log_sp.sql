...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cvo_cust_invoice_log_sp] @cust_code VARCHAR(12), @sdate DATETIME, @edate DATETIME 
AS
BEGIN
...
DECLARE @cust VARCHAR(12), @ssdate DATETIME, @eedate DATETIME
SELECT @cust = @cust_code
SELECT @ssdate = @sdate
SELECT @eedate = @edate

SELECT @cust = '055502', @ssdate = '12/1/2018', @eedate = '12/31/2018'
...
WHERE ipa.cust_code LIKE @cust
      ...
      AND ((ipa.source <>'open' AND ipa.Shipped <> 0)
             OR (ipa.source = 'open' AND ipa.who_entered = 'backordr'));
