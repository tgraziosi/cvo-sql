...
GO
    ...
    CREATE procedure 
[dbo].[imarpyt_Validate_sp] @imarpyt_Validate_sp_Batch_Number INT = 0,
                    @imarpyt_Validate_sp_Starting_Record_Number INT = 0,
                    @imarpyt_Validate_sp_Ending_Record_Number INT = 0,
                    @imarpyt_Validate_sp_Dummy_1 INT = 0,
                    @debug_level INT = 0,
                    @imarpyt_Validate_sp_Dummy_2 INT = 0,
                    @imarpyt_Validate_sp_Import_Identifier INT = 0,
                    @userid INT = 0,
                    @imarpyt_Validate_sp_Application_Name VARCHAR(30) = 'Import Manager',
                    @imarpyt_Validate_sp_Override_User_Name VARCHAR(30) = ''
    AS
    ...
    SET @Routine_Name = 'imarpyt_Validate_sp'        
    IF @debug_level >= 3
    ...
    IF @imarpyt_Validate_sp_Import_Identifier = 0    
            OR @imarpyt_Validate_sp_Import_Identifier IS NULL
        BEGIN
        EXEC @SP_Result = imarpyt_sp @company_code = @company_code,
                                     ...
                                     @imarpyt_sp_Import_Identifier = @imarpyt_Validate_sp_Import_Identifier OUTPUT,
                                     @imarpyt_sp_Override_User_Name = @imarpyt_Validate_sp_Override_User_Name
    ...
    ELSE    
        ...
        EXEC @SP_Result = imarpyt_sp @company_code = @company_code,
                                     ...
                                     @imarpyt_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                     @imarpyt_sp_dummy = @Dummy OUTPUT,
                                     @imarpyt_sp_Import_Identifier = @imarpyt_Validate_sp_Import_Identifier,
                                     @imarpyt_sp_Override_User_Name = @imarpyt_Validate_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imarpyt_sp 2' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
        BEGIN
    ...
    SET @Import_Identifier = @imarpyt_Validate_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarpyt_Validate_sp] TO [public]
GO
