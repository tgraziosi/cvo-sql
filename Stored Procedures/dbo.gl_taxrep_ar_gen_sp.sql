...
Copyright (c) 1996 Platinum Software Corporation, 1996
                 All Rights Reserved 
 */
                             CREATE PROCEDURE [dbo].[gl_taxrep_ar_gen_sp] @start_date int, 
@end_date int, @report_cur smallint, @ret_val int OUTPUT AS BEGIN SELECT @ret_val = 0 
 IF ((@start_date IS NULL) OR (@end_date IS NULL) OR  (@report_cur IS NULL)) RETURN 2 
SET ROWCOUNT 0 INSERT  #gl_taxrep 
(
...
SELECT  artrx.trx_ctrl_num,  artrx.doc_ctrl_num,  artrx.trx_type,  gltaxboxdet.tax_box_code, 
 ...
 artrx.date_doc,  0 FROM  artrx,  artrxtax,  gltaxboxdet,  artxtype WHERE  artrxtax.tax_type_code = gltaxboxdet.tax_type_code AND 
 artrxtax.tax_type_code = artxtype.tax_type_code AND  artrxtax.trx_type = artrx.trx_type AND 
 artrxtax.doc_ctrl_num = artrx.doc_ctrl_num AND  artrx.date_applied BETWEEN @start_date AND @end_date 
...
FROM  #gl_taxrep tr JOIN artxtype tt  ON tr.tax_type_code = tt.tax_type_code IF (@@error <>0) 
 BEGIN  SELECT @ret_val = 1  ROLLBACK TRAN  RETURN @ret_val  END END 
GO
GRANT EXECUTE ON  [dbo].[gl_taxrep_ar_gen_sp] TO [public]
GO
