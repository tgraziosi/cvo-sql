...
BEGIN
    ...
    (
        ...
        WHERE CHARINDEX('@', contact_email) > 0
              AND CHARINDEX('cvoptical.com', contact_email) = 0
        UNION
        SELECT DISTINCT
               RIGHT(customer_code, 5) mergecust,
               contact_email
    ...
    UPDATE #email
    SET contact_email = REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(contact_email)), ' ', ','), ';', ','), ',,', ',');
    ...
    AS (SELECT DISTINCT
               ...
               dbo.f_cvo_get_buying_group(ar.customer_code, GETDATE()) current_bg,
               dbo.f_cvo_get_buying_group_name(dbo.f_cvo_get_buying_group(ar.customer_code, GETDATE())) bg_parent,
               ISNULL(Pri_desig.desig, '') Primary_Designation,
               ISNULL(Pri_desig.desig_code, '') desig_code,
    ...
    SELECT report.territory_code,
           ...
           report.desig_code,
           CASE WHEN report.bg_parent IS NOT NULL THEN dbo.f_check_bg_pri_mismatch(report.bg_parent,report.desig_code) ELSE '' END PRI_BG_mismatch,
    ...
    WHERE (
              report.Primary_Designation <> ''
              OR report.Active_Rebateable_Designations <> ''
          );
