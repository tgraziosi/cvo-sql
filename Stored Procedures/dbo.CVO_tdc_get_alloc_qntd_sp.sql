...
CREATE PROCEDURE  [dbo].[CVO_tdc_get_alloc_qntd_sp]  @location VARCHAR(10), @part_no VARCHAR(30), @bin_group VARCHAR(30) AS
BEGIN
...
BEGIN  
 ...
 IF EXISTS (SELECT * FROM tdc_soft_alloc_tbl (nolock) WHERE location = @location AND part_no = @part_no AND (order_no <> 0 OR (order_no = 0 AND dest_bin = @custom_bin))) -- v1.1 
 BEGIN  
