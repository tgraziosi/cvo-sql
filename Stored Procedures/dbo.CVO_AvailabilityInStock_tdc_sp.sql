...
CREATE PROCEDURE  [dbo].[CVO_AvailabilityInStock_tdc_sp]   @part_no VARCHAR(30), @location VARCHAR(10), @available_out DECIMAL(20,8) OUTPUT
AS  
BEGIN  
 SET NOCOUNT ON  
 ...
 EXEC tdc_get_alloc_qntd_sp @location, @part_no  
	...
	-- v1.3
	SELECT @in_stock = case when (m.status='C' or m.status='V') then 0 else (l.in_stock + l.issued_mtd + p.produced_mtd - p.usage_mtd - s.sales_qty_mtd + r.recv_mtd + x.xfer_mtd - isnull(@exc_bin_qty,0))end	-- v1.4 & v1.5
...
END  
GO
GRANT EXECUTE ON  [dbo].[CVO_AvailabilityInStock_tdc_sp] TO [public]
GO
