...
SELECT @cart_order_no = MIN(cpp.order_no)
FROM dbo.cvo_cart_parts_processed AS cpp (NOLOCK)
    JOIN dbo.tdc_pick_queue AS tpq (NOLOCK)
        ON tpq.tran_id = cpp.tran_id
WHERE 1 = 1
      AND cpp.scanned = 1
      AND cpp.isPicked <> 'Y'
...
BEGIN
    ...
    -- )
    EXEC dbo.cvo_pick_cart_process_sp @cart_no = 0,
                                      @order_no = @order,
                                      @order_ext = @ext,
                                      @proc_option = 1;
    ...
    WHERE 1 = 1
          AND cpp.scanned = 1
          AND cpp.isPicked <> 'Y'
          AND cpp.order_no > @cart_order_no;
