...
CREATE PROCEDURE [dbo].[fs_calc_receipt_tax_wrap] @po varchar(16),@po_line int,@txcode varchar(8),
@qty decimal(20,8),@price decimal(20,8), @batch_call int = 0,
@tax_included decimal(20,8) = 0 OUT, @nonrec_tax_out decimal(20,8) = 0 OUT
AS

BEGIN

DECLARE @tax decimal(20,8), @nonrec_tax decimal(20,8), @rc int
...
exec  @rc = fs_calc_receipt_tax @po, @po_line, @txcode, @qty, @price, @tax  OUT , @nonrec_tax OUT

if @batch_call = 0
  select @tax, @nonrec_tax, @rc
else
  select @tax_included = @tax, @nonrec_tax_out = @nonrec_tax
...
GO
GRANT EXECUTE ON  [dbo].[fs_calc_receipt_tax_wrap] TO [public]
GO
