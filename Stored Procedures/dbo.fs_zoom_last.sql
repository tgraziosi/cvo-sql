...
CREATE PROCEDURE [dbo].[fs_zoom_last] @sqltext varchar(1000), @key_col varchar(255), @void_col varchar(255)
AS
...
begin
  ...
  begin
    ...
    if @from_pos > 0
      select @sqltext = 'select max(' + @key_col + ') ' + substring(@sqltext,charindex('from',@sqltext),1000)
    else
...
begin
  if @key_col != ''
    select @sqltext = @sqltext + ' , "last","' + @key_col + '",""'
end 
...
GO
GRANT EXECUTE ON  [dbo].[fs_zoom_last] TO [public]
GO
