...
CREATE PROC [dbo].[ARUPDateCustActivity_SP]        
...
BEGIN
        ...
        (       customer_code   varchar(8) NULL, 
                amt_balance_home        float NULL,
                amt_balance_oper        float NULL,
                amt_on_acct_home        float NULL,
                amt_on_acct_oper        float NULL,
                amt_inv_unp_home        float NULL,
                amt_inv_unp_oper        float NULL, 
                amt_on_order_home       float NULL,
                amt_on_order_oper       float NULL, 
                num_inv         int NULL,
                num_inv_paid            int NULL,
                num_overdue_pyt int NULL,
                days_overdue            int NULL,
                days_pay                int NULL,
                avg_days_overdue        int NULL,
                avg_days_pay            int NULL,
                exists_flag             int NULL
        )       
...
(
	...
	exists_flag		smallint
        ...
        EXEC    ARUPDateActvityAmtonOrder_SP    1        

        UPDATE  #existing_orders
        SET     exists_flag = 1
        FROM    #customer_bal
        WHERE   #customer_bal.customer_code = #existing_orders.key_code1
        
        INSERT #customer_bal
        SELECT  key_code1, 0.0, 0.0, 0.0, 0.0,  /* amt_balance & amt_on_acct */
        ...
        BEGIN
		   ...
		   CREATE TABLE #unposted_trx
                ...
                INSERT #customer_bal
                SELECT  customer_code, 0.0, 0.0, 0.0, 0.0,  /* amt_balance & amt_on_acct */
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[ARUPDateCustActivity_SP] TO [public]
GO
