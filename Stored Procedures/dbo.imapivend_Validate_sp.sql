...
GO
    ...
    CREATE procedure 
[dbo].[imapivend_Validate_sp] @imapivend_Validate_sp_Batch_Number INT = 0,
                      @imapivend_Validate_sp_Starting_Record_Number INT = 0,
                      @imapivend_Validate_sp_Ending_Record_Number INT = 0,
                      @imapivend_Validate_sp_Dummy_1 INT = 0,
                      @debug_level INT = 0,
                      @imapivend_Validate_sp_Record_Type INT = 0,
                      @imapivend_Validate_sp_Import_Identifier INT = 0,
                      @userid INT = 0,
                      @imapivend_Validate_sp_Application_Name VARCHAR(30) ='Import Manager',
                      @imapivend_Validate_sp_Override_User_Name VARCHAR(30) =''
    AS
    ...
    SET @Routine_Name = 'imapivend_Validate_sp'        
    IF @debug_level >= 3
    ...
    IF @imapivend_Validate_sp_Import_Identifier = 0     
            OR @imapivend_Validate_sp_Import_Identifier IS NULL
        BEGIN         
        EXEC @SP_Result = imapivend_sp @company_code = @company_code,
                                       @address_type = @imapivend_Validate_sp_Record_Type,
                                       @trial_flag = 1,
                                       @debug_level = @debug_level,
                                       @userid = @userid,
                                       @imapivend_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                       @imapivend_sp_Dummy_1 = @Dummy OUTPUT,
                                       @imapivend_sp_Import_Identifier = @imapivend_Validate_sp_Import_Identifier OUTPUT,
                                       @imapivend_sp_Override_User_Name = @imapivend_Validate_sp_Override_User_Name
    ...
    ELSE    
        ...
        EXEC @SP_Result = imapivend_sp @company_code = @company_code,
                                       ...
                                       @imapivend_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                       @imapivend_sp_Dummy_1 = @Dummy OUTPUT,
                                       @imapivend_sp_Import_Identifier = @imapivend_Validate_sp_Import_Identifier,
                                       @imapivend_sp_Override_User_Name = @imapivend_Validate_sp_Override_User_Name
    ...
    SET @Import_Identifier = @imapivend_Validate_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imapivend_Validate_sp] TO [public]
GO
