...
CREATE PROCEDURE [dbo].[fs_calculate_matchtax_wrap] @match_id int  , @online_call int = 0 ,  
@doctype int = 2, @trx_ctrl_num varchar(16) = '',  
@err_msg varchar(255)='' OUT,  
@debug int = 0 AS  
  
BEGIN  
-- doctype = 2 - purchaseOrder  
-- doctype = 3 - purchaseInvoice - called with 3 when posting to AP  
  
if @online_call = 2 -- get results  
begin  
  ...
  from #online_taxinfo  
  where control_number = @match_id and trx_type = 3 and reference_number = 0  
  
  return 1  
end  
  
if @online_call = 4 -- get results  
begin  
...
exec  fs_calculate_matchtax @match_id, @err1  out, @online_call, @doctype,   
  @trx_ctrl_num, @debug  
  
      if @online_call  in ( 1,3)  
  begin  
    exec adm_get_tax_detail @match_id, 0, @err1  
  end  
        else  
        begin  
    exec adm_get_tax_detail @match_id, 0, @err1, 0, @err_msg out  
...
GO
GRANT EXECUTE ON  [dbo].[fs_calculate_matchtax_wrap] TO [public]
GO
