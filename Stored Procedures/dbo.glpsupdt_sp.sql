...
CREATE PROCEDURE        [dbo].[glpsupdt_sp] 
...
BEGIN
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'glpsupdt.cpp' + ', line ' + STR( 188, 5 ) + ' -- ENTRY: '
	...
	IF ( @debug_level > 2 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 265, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Initialize #drcr for existing balances'
	...
	IF ( @debug_level > 2 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 308, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + '******************* Beginning SQL Transaction ********************'
	



	EXEC    @result = glpsupd2_sp @batch_code, @debug_level
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 374, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'GLBAL insert of new records'
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 428, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Updating all balance records'
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 460, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Time to generate #baluntil table'
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 597, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Updating balance_until column'
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 611, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Updating posted flag for transaction group'
	...
	IF ( @debug_level > 3 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 625, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Updating posted flag for transaction details'
	...
	BEGIN
		...
		END
                ...
                SELECT  journal_ctrl_num,    date_applied,
                        recurring_flag,      repeating_flag,
                        reversing_flag,      mark_flag,
                        interbranch_flag
                FROM    #gldtrx
	...
	IF ( @debug_level > 2 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 692, 5 ) + ' -- MSG: ' + CONVERT(char,@start_time,100) + '******************  End SQL Transaction ******************'
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + '  ' + 'glpsupdt.cpp' + ', line ' + STR( 703, 5 ) + ' -- EXIT: '
	RETURN 0
	...
	IF ( @debug_level > 0 ) SELECT 'glpsupdt.cpp' + ', line ' + STR( 719, 5 ) + ' -- MSG: ' + CONVERT(char,@work_time,100) + 'Exiting - ERROR'
	RETURN 1039
...
END
GO
GRANT EXECUTE ON  [dbo].[glpsupdt_sp] TO [public]
GO
