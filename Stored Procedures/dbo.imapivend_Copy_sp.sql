...
GO
    ...
    CREATE procedure 
[dbo].[imapivend_Copy_sp] @imapivend_Copy_sp_Batch_Number INT = 0,
                  @imapivend_Copy_sp_Starting_Record_Number INT = 0,
                  @imapivend_Copy_sp_Ending_Record_Number INT = 0,
                  @imapivend_Copy_sp_Dummy_1 INT = 0,
                  @debug_level INT = 0,
                  @imapivend_Copy_sp_Record_Type INT = 0,
                  @imapivend_Copy_sp_Import_Identifier INT = 0,
                  @userid INT = 0,
                  @imapivend_Copy_sp_Dummy_3 VARCHAR(40),
                  @imapivend_Copy_sp_Dummy_4 VARCHAR(40),   
                  @imapivend_Copy_sp_Application_Name VARCHAR(30) ='Import Manager',
                  @imapivend_Copy_sp_Override_User_Name VARCHAR(30) ='',
                  @imapivend_Copy_sp_Dummy_5 INT = NULL
    AS
    ...
    SET @Routine_Name = 'imapivend_Copy_sp'        
    IF @debug_level >= 3
    ...
    --
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imapivend_sp.'
    IF @imapivend_Copy_sp_Import_Identifier = 0    
            OR @imapivend_Copy_sp_Import_Identifier IS NULL
        BEGIN        
        EXEC @SP_Result = imapivend_sp @company_code = @company_code,
                                       @address_type = @imapivend_Copy_sp_Record_Type,
                                       @trial_flag = 0,
                                       @debug_level = @debug_level,
                                       @userid = @userid,
                                       @imapivend_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                       @imapivend_sp_Dummy_1 = @Dummy OUTPUT,
                                       @imapivend_sp_Import_Identifier = @imapivend_Copy_sp_Import_Identifier OUTPUT,
                                       @imapivend_sp_Override_User_Name = @imapivend_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imapivend_sp 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    ELSE    
        BEGIN        
        EXEC @SP_Result = imapivend_sp @company_code = @company_code,
                                       ...
                                       @imapivend_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                       @imapivend_sp_Dummy_1 = @Dummy OUTPUT,
                                       @imapivend_sp_Import_Identifier = @imapivend_Copy_sp_Import_Identifier,
                                       @imapivend_sp_Override_User_Name = @imapivend_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imapivend_sp 2' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
        BEGIN
        EXEC im_log_sp_error_sp @ILSE_Error_Code = @SP_Result,
                                @ILSE_SP_Name = 'imapivend_sp',
                                @ILSE_String = '',
        ...
        END    
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': After imapivend_sp.'
    SET @Import_Identifier = @imapivend_Copy_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imapivend_Copy_sp] TO [public]
GO
