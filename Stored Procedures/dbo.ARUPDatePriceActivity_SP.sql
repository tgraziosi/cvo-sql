...
CREATE PROC [dbo].[ARUPDatePriceActivity_SP] 
...
BEGIN
	...
	SELECT price_code, 
		...
		0, 0, 0
	FROM	artrxage
	WHERE 	( LTRIM(price_code) IS NOT NULL AND LTRIM(price_code) != " " )
	GROUP BY price_code	
		
	CREATE CLUSTERED INDEX #cust_bal_ind ON #price_bal(price_code)
...
(
	...
	EXEC	ARUPDateActvityAmtonOrder_SP	4 

	
	UPDATE	#existing_orders
	SET	exists_flag = 1
	FROM	#price_bal
	WHERE	#price_bal.price_code = #existing_orders.key_code1
	...
	BEGIN
		...
		SET	exists_flag = 1
		FROM	#price_bal
		WHERE	#price_bal.price_code = #unposted_trx.price_code
		
		
		INSERT #price_bal
		SELECT	price_code, 0.0, 0.0, 
	...
	SET	exists_flag = 1
	FROM	aractprc
	WHERE	aractprc.price_code = #price_bal.price_code
	...
	SET	amt_balance 		= bal.amt_balance_home,
		...
		last_trx_time		 = 1
	FROM	#price_bal	bal
	WHERE	bal.price_code = aractprc.price_code
	AND	exists_flag = 1
...
/**/                                              
GO
GRANT EXECUTE ON  [dbo].[ARUPDatePriceActivity_SP] TO [public]
GO
