...
SET ANSI_NULLS OFF
GO
CREATE PROC [dbo].[mbbmspSheetSecList]
	@UserID         mbbmudtUser,
	@SheetType	int,
...
************************************************************************************/
------------------------------------------------------------
--NOTE: this is a duplicate routine to mbbmspSheetSecGrid,--
...
------------------------------------------------------------
		...
		BEGIN
			...
			BEGIN
				...
				WHERE   rc.SheetID = ps.Parent AND
					rc.SheetLevel = @CurLevel AND
					ps.HostCompany = @HostCompany
	...
	----------------------------------
	INSERT  #mbbmLookUp
	SELECT DISTINCT SheetID, 0, '', '', '', 0, '', 0, '' FROM #mbbmFinal
	...
	SET 	PlanID = p.PlanID,
		...
		Status = p.Status
	FROM 	#mbbmLookUp l, mbbmPlanSheet74 p
	WHERE 	l.SheetID = p.SheetID AND
	      	p.HostCompany = @HostCompany

	UPDATE 	#mbbmLookUp 
	SET 	PlanKey = p.PlanKey
	FROM 	#mbbmLookUp l, mbbmPlan74 p
	WHERE 	l.PlanID = p.PlanID AND       
		p.HostCompany = @HostCompany       

END
GO
GRANT EXECUTE ON  [dbo].[mbbmspSheetSecList] TO [Analytics]
GO
GRANT EXECUTE ON  [dbo].[mbbmspSheetSecList] TO [public]
GO
