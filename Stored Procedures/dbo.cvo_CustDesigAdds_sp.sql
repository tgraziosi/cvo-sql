...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cvo_CustDesigAdds_sp] @start DATETIME, @end DATETIME
...
SELECT  t2.territory_code AS Terr ,
        ...
        t2.address_name ,
        t2.addr2 ,
        CASE WHEN t2.addr3 LIKE '%, __ %' THEN ''
             ELSE t2.addr3
        END AS addr3 ,
        t2.city ,
        t2.state ,
        t2.postal_code AS zip ,
        CASE WHEN t2.contact_email NOT LIKE '%@%.%' THEN ''
             ELSE LOWER(t2.contact_email)
        END AS Email ,
        t1.code ,
        t1.description ,
        t1.start_date ,
...
INTO    #tbl1
FROM    cvo_cust_designation_codes t1 ( NOLOCK )
        JOIN armaster t2 ( NOLOCK ) ON t2.customer_code = t1.customer_code
        LEFT OUTER JOIN ( SELECT distinct
                        ...
                        ) pri_desig ON pri_desig.customer_code = t1.customer_code
WHERE   
		--( t1.start_date > DATEADD(d, -7, GETDATE())
  --        OR t1.end_date BETWEEN DATEADD(d, -7, GETDATE()) AND GETDATE()
  --      )
		(t1.START_DATE >= @start OR end_date BETWEEN @start AND @end)
        AND t2.address_type = 0
        AND ( t1.code IN ( 'Opt-1', 'PRI', 'VT', 'FEC', 'TSO', 'IVA', 'RXE',
                           'RX3', 'RX5' )
              OR t1.code LIKE 'BB%'
              OR t1.code LIKE 'I-%'
            );
...
SELECT DISTINCT
        ...
        ( SELECT TOP 1
                    Item
          FROM      cvo_cust_designation_codes_audit t3
        ...
        CASE WHEN ( SELECT TOP 1
             ...
             WHEN ( SELECT TOP 1
                    ...
                    WHERE   t1.customer_code = t3.customer_code
                            AND t1.code = t3.code
                    ORDER BY Audit_Date DESC
                  ) = 'NT AUTHORITY\SYSTEM' THEN 'SYSTEM ADMIN'
             ELSE ( SELECT TOP 1
                            User_ID
                    FROM    cvo_cust_designation_codes_audit t3
                    WHERE   t1.customer_code = t3.customer_code
