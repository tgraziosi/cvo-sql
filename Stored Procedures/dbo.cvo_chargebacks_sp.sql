...
-- EXEC CVO_CHARGEBACKS_SP "cutoff date = 734801 and create_doc = 'N'"
...
DECLARE pd02 CURSOR FOR Select distinct customer_code from #Temp Order By customer_code
OPEN pd02
FETCH NEXT from pd02 INTO @customer_code
...
DECLARE pd01 CURSOR FOR Select distinct customer_code from #Temp /*where trx_type = 2031*/
group by customer_code having (sum(disc_given)<=0 ) Order By customer_code
OPEN pd01
FETCH NEXT from pd01 INTO @customer_code
WHILE (@@fetch_status=0)
BEGIN
	delete from #temp where customer_code = @customer_code 
	FETCH NEXT from pd01 INTO @customer_code
END 
CLOSE pd01 
DEALLOCATE pd01
...
DECLARE pdcb CURSOR FOR 
	Select distinct customer_code from #Temp where trx_type = 2031
	group by customer_code having (sum(disc_given-disc_nochg)<=0 ) Order By customer_code
OPEN pdcb
FETCH NEXT from pdcb INTO @customer_code
WHILE (@@fetch_status=0)
BEGIN
	delete from #temp where customer_code = @customer_code 
	FETCH NEXT from pdcb INTO @customer_code
...
SELECT *, @cutoff_date as cutoff_date,@Create_doc as create_doc 
