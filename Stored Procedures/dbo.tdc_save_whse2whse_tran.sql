...
CREATE PROCEDURE [dbo].[tdc_save_whse2whse_tran](
@tran_id int, 
@location varchar(10), 
@part_no varchar(30), 
@lot varchar(25),
...
AS
/**************************************************************************
tdc_save_whse2whse_tran - This stored procedure is used to update existing
...
IF @tran_id > 0 
  BEGIN
    IF NOT EXISTS(select null from tdc_pick_queue (NOLOCK) where tran_id = @tran_id)
      BEGIN
	-- Invalid Tran_id ''%d''
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -101 AND language = @language
    ...
    IF NOT EXISTS(select null from locations (NOLOCK) where location = @location)
      BEGIN
	-- Invalid Location ''%s''
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -102 AND language = @language
	...
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -103 AND language = @language
	...
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -104 AND language = @language
	...
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -105 AND language = @language
        raiserror(@msg, 16, -1)
        return -1
      END
    ...
    SELECT @qty_held_on_queue = isnull(sum(qty_to_process),0) from tdc_pick_queue (NOLOCK)
                WHERE trans = 'MGTW2W' AND location = @location 
	...
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -106 AND language = @language
        raiserror(@msg, 16, -1, @part_no, @from_bin,@qty_avail_in_lbs, @qty_held_on_queue, @qty_to_process)
        return -1
      END

    IF NOT EXISTS(select null from locations (NOLOCK) 
                     WHERE location = @to_loc)
      BEGIN
	-- Invalid Destination Location ''%s''
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -107 AND language = @language
        raiserror(@msg, 16, -1, @to_loc)
        return -1
      END
    ...
    IF @assign_type = 'G'
      BEGIN
        IF NOT EXISTS(SELECT null FROM tdc_group (NOLOCK) WHERE Group_Id = @assign_user_or_group)
          BEGIN
	    -- Invalid Group ''%s''
	    SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -108 AND language = @language
    ...
    IF @assign_type = 'U'
      BEGIN
        IF NOT EXISTS(SELECT null FROM tdc_sec (NOLOCK) WHERE userid = @assign_user_or_group)
          BEGIN
	    -- Invalid User ''%s''
	    SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -109 AND language = @language
            raiserror(@msg, 16, -1, @assign_user_or_group)
            return -1
          END
    ...
    UPDATE tdc_pick_queue SET location = @location, part_no = @part_no, lot = @lot,
        bin_no = @from_bin, next_op = @to_loc, qty_to_process = @qty_to_process,
...
ELSE
  BEGIN
    ...
    IF NOT EXISTS(select null from locations (NOLOCK) 
      ...
      BEGIN
	-- Invalid Destination Location ''%s''
	SELECT @msg = err_msg FROM tdc_lookup_error WHERE module = 'SPR' AND trans = 'tdc_save_whse2whse_tran' AND err_no = -110 AND language = @language
        raiserror(@msg, 16, -1, @to_loc)
        return -1
      END
    ...
    Values(@location,@part_no,@lot,
        @from_bin, @to_loc, @qty_to_process,
        @AssignGroup, @AssignUser, @priority,
        @new_seq_num, @user_id, CURRENT_TIMESTAMP,
        'M', 'H', 'VB', 'MGTW2W')
  END
GO
GRANT EXECUTE ON  [dbo].[tdc_save_whse2whse_tran] TO [public]
GO
