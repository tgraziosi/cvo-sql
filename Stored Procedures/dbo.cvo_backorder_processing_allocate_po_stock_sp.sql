...
EXEC cvo_backorder_processing_allocate_po_stock_sp 
...
BEGIN
	...
	*/
	...
	--DELETE FROM dbo.CVO_backorder_processing_orders_po_xref WHERE [status] = 2

	-- If any previously processed records still aren't fully processed then reset to status of 0
	UPDATE 
		dbo.CVO_backorder_processing_orders_po_xref 
	SET
		[status] = 0
	WHERE
		qty_processed < qty_received
		AND [status] = 2
	...
	WHERE 
		[status] <= 0
		AND qty_processed < qty_received
	...
	BEGIN
		...
		SELECT TOP 1
			...
			@line_no = line_no,
			@part_no = part_no,
			@ringfenced = qty_received - qty_processed 
		FROM
		...
		-- Write log
		SET @msg = 'Processing PO - Template: ' + @template_code 
		...
		SET @retval = 0
		SET @err_msg = ''
		EXEC @retval = dbo.cvo_backorder_processing_allocate_po_stock_line_sp	@rec_id, @order_no, @ext, @line_no, @part_no, @location, @ringfenced, 
																				@is_transfer, @template_code, @err_msg OUTPUT
		...
		SET
			...
			qty_processed = CASE ISNULL(@retval,-1) WHEN 0 THEN qty_processed + @ringfenced ELSE qty_processed END
		WHERE	
			rec_id = @rec_id
...
GO
GRANT EXECUTE ON  [dbo].[cvo_backorder_processing_allocate_po_stock_sp] TO [public]
GO
