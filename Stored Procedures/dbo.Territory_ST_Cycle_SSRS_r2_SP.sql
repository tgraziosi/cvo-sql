...
BEGIN
    ...
    SELECT CASE WHEN status_type = 1 THEN 'Open' ELSE 'Closed' END AS status_type,
           ...
           CASE WHEN od.date_entered >= @M1 THEN 1
           WHEN od.date_entered >= @M2 THEN 2
           WHEN od.date_entered >= @M3 THEN 3
           WHEN od.date_entered >= @M4 THEN 4
           WHEN od.date_entered >= @M5 THEN 5
           WHEN od.date_entered >= @M6 THEN 6
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -28, @DateTo)) THEN 7
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -32, @DateTo)) THEN 8
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -36, @DateTo)) THEN 9
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -42, @DateTo)) THEN 10
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -48, @DateTo)) THEN 11
           WHEN od.date_entered >= DATEADD(SECOND, 1, DATEADD(WEEK, -52, @DateTo)) THEN 12 ELSE 13
           END AS M,
           CASE WHEN od.date_entered >= @M2 THEN 1
           WHEN od.date_entered >= @M4 THEN 2
           WHEN od.date_entered >= @M6 THEN 3
           ELSE 4
           END AS R
    INTO #STCYC
    FROM #territory AS t
