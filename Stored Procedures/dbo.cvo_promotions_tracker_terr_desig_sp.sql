...
-- exec cvo_promotions_tracker_terr_desig_sp '1/1/2016','10/09/2016', '30310' , 've-revo'
AS
    SET NOCOUNT ON;
        ...
        BEGIN -- get enrollment info for the reporting period
            INSERT  INTO #temp
                    ...
                    WHERE   code LIKE 'rx%'
                            ...
                            AND ccdc.start_date BETWEEN @startdate
                                                AND     DATEADD(ms, -3,
                                                              DATEADD(dd,
                                                              DATEDIFF(dd, 0,
                                                              @enddate) + 1, 0));
            ...
            Qual_order = CASE WHEN source = 'T' THEN 0
                              ...
                              WHEN ISNULL(reason, '') = ''
                                   AND NOT EXISTS ( SELECT  1
                                                    ...
                                                    WHERE   poa.order_no = #temp.order_no
                                                            AND poa.order_ext = #temp.ext )
                                   AND #temp.ext = ( SELECT MIN(ext)
                                                     FROM   orders o
                                                     WHERE  o.order_no = #temp.order_no
													 AND o.status <> 'v'
                                                   ) -- don't qualify split orders
                                   THEN 1
                              ELSE 0
                         END ,
            ( SELECT TOP 1
