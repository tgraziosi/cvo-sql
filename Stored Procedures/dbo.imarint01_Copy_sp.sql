...
GO
    ...
    CREATE PROC 
[dbo].[imarint01_Copy_sp] @imarint01_Copy_sp_Batch_Number INT = 0,
                  @imarint01_Copy_sp_Starting_Record_Number INT = 0,
                  @imarint01_Copy_sp_Ending_Record_Number INT = 0,
                  @imarint01_Copy_sp_Flag_Bits INT = 0,
                  @debug_level INT = 0,
                  @imarint01_Copy_sp_Record_Type INT = 2031,
                  @imarint01_Copy_sp_Import_Identifier INT = 0,
                  @imarint01_Copy_sp_db_userid VARCHAR(40) = 'ABSENT',
                  @imarint01_Copy_sp_db_password VARCHAR(40) = '',
                  @userid INT = 0,
                  @imarint01_Copy_sp_Application_Name VARCHAR(30) = 'Import Manager',
                  @imarint01_Copy_sp_Override_User_Name VARCHAR(30) = '',
                  @imarint01_Copy_sp_TPS_int_value INT = NULL
    AS
    ...
    SET @Routine_Name = 'imarint01_Copy_sp'        
    IF @debug_level >= 3
    ...
    IF (@imarint01_Copy_sp_Flag_Bits & 0x1) > 0
        SET @close_batch_flag = 1
    SET @post_flag = 0
    IF (@imarint01_Copy_sp_Flag_Bits & 0x2) > 0
        SET @post_flag = 1
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imarint01_sp.'
    IF @imarint01_Copy_sp_Import_Identifier = 0    
            OR @imarint01_Copy_sp_Import_Identifier IS NULL
        BEGIN        
        EXEC @SP_Result = imarint01_sp @company_code = @company_code,
                                       @method_flag = 2,
                                       @print_flag = 0,
                                       @total_flag = 0,
                                       @post_flag = @post_flag,
                                       @invoice_flag = @imarint01_Copy_sp_Record_Type,
                                       @validation_flag = 1,
                                       @close_batch_flag = @close_batch_flag,
                                       @db_userid = @imarint01_Copy_sp_db_userid,
                                       @db_password = @imarint01_Copy_sp_db_password,
                                       @debug_level = @debug_level,
                                       @perf_level = 0,
                                       @userid = @userid,
                                       @imarint01_sp_process_ctrl_num_Validation = @process_ctrl_num OUTPUT,
                                       @imarint01_sp_process_ctrl_num_Posting = @Dummy OUTPUT,
                                       @imarint01_sp_Import_Identifier = @imarint01_Copy_sp_Import_Identifier OUTPUT,
                                       @imarint01_sp_Application_Name = @imarint01_Copy_sp_Application_Name,
                                       @imarint01_sp_Override_User_Name = @imarint01_Copy_sp_Override_User_Name,
                                       @imarint01_sp_TPS_int_value = @imarint01_Copy_sp_TPS_int_value
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imarint01_sp 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    ELSE    
        BEGIN        
        EXEC @SP_Result = imarint01_sp @company_code = @company_code,
                                       ...
                                       @imarint01_sp_process_ctrl_num_Validation = @process_ctrl_num OUTPUT,
                                       @imarint01_sp_process_ctrl_num_Posting = @Dummy OUTPUT,
                                       @imarint01_sp_Import_Identifier = @imarint01_Copy_sp_Import_Identifier,
                                       @imarint01_sp_Application_Name = @imarint01_Copy_sp_Application_Name,
                                       @imarint01_sp_Override_User_Name = @imarint01_Copy_sp_Override_User_Name,
                                       @imarint01_sp_TPS_int_value = @imarint01_Copy_sp_TPS_int_value
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imarint01_sp 2' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
        BEGIN
        EXEC im_log_sp_error_sp @ILSE_Error_Code = @SP_Result,
                                @ILSE_SP_Name = 'imarint01_sp',
                                @ILSE_String = '',
        ...
        END    
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': After imarint01_sp.'
    SET @Import_Identifier = @imarint01_Copy_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarint01_Copy_sp] TO [public]
GO
