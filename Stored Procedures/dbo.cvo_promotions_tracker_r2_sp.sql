...
CREATE PROCEDURE [dbo].[cvo_promotions_tracker_r2_sp]
-- @promo varchar(20), 
@startdate datetime, @enddate datetime
-- updates
-- 10/30 - make promo multi-value list - need to repmove parameter
-- 1/5/15 - tag - qualify returns on amount AND qty
 /*
...
-- exec cvo_promotions_tracker_r2_sp '07/01/2014','07/30/2014' 
...
--select * from #r
--select * From #temp where promo_id = 'izod clear'
...
-- select * From #temp where order_no = 2216899
...
when /* fix once 2014 numbers are finalized - */
/* isnull(return_qty,0) = 0 and isnull(return_date,'1/1/1949') <> '1/1/1949' and -- */ 
isnull(return_amt,0) = 0 and not exists (select 1 from cvo_promo_override_audit poa where poa.order_no = #temp.order_no and poa.order_ext = #temp.ext) then 1 
else 0 end,	
...
GO
GRANT EXECUTE ON  [dbo].[cvo_promotions_tracker_r2_sp] TO [public]
GO
