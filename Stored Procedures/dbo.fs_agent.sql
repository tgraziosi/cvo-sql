...
CREATE PROCEDURE [dbo].[fs_agent]	@part varchar(30), 
								@src char(1),   
								@src_no int,       
								@src_date datetime,  
								@who varchar(20),  
								@src_qty decimal(20,8) 
AS  
BEGIN  
	...
	DECLARE	@temp_part_no	varchar(30) -- v1.0
	  
	select @lb = 'N'  
  
	SELECT @mode = CASE WHEN @src = 'B' THEN @src   
					WHEN @src = 'R' THEN @src ELSE 'Z' END  
	...
	begin  
		...
		-- v1.0 Start
		SET @temp_part_no = LEFT(@part,CHARINDEX('-',@part)-1) + '-MAKE'
		SET @part = @temp_part_no
		SELECT	@loc2 = vendor
	...
	begin  
        ...
        where	r.po_no=p.po_no 
		and		r.receipt_no=@src_no  
	...
	begin  
		...
		if @verb = 'XFER' or @verb = 'AUTOXFER' or @verb = 'OUTSOURCE' 
		begin  
			select @xmode = ''  
			select @xpart = ''  
			select @tpart = @obj  
			...
			if substring(@obj,1,10) = ':BUILDPLAN' or @verb = 'OUTSOURCE' 
			begin  
			...
			if substring(@obj,1,11) = ':BUILDPLAN=' 
			begin  
