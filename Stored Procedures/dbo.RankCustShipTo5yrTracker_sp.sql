...
-- Description:	5 Year Customer SHIP TO Sales Tracker
-- EXEC RankCustShipTo5yrTracker_sp
-- =============================================
CREATE PROCEDURE [dbo].[RankCustShipTo5yrTracker_sp]
...
right(t1.customer_code,5)MCust, t1.customer_code, ship_to_code, territory_code as Terr, Address_name, addr1, 
case when addr2 like '%, __ %' then '' else addr2 end as addr2, 
...
drop table dbo.#DATA
select distinct t2.Status, t2.Door, t2.Terr, t1.MCust as Customer, ship_to, t2.address_name, addr2, addr3, City, ISNULL(State,'')State, ISNULL(Postal_code,'')Postal_Code, Country,
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)),0)'CY',
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)-1),0)'PY',
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)-2),0)'PYL1',
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)-3),0)'PYL2',
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)-4),0)'PYL3',
ISNULL((select sum(anet) from #Source t11 where t1.MCust=t11.MCust and t1.ship_to=t11.ship_to and Yr = DATEPART(YEAR,@PTo)-5),0)'PYL4'
INTO #DATA
 from #Source t1 
 join #Custs t2 on t1.MCust=t2.MCust and t1.ship_to=t2.ship_to_code
group by t2.Status, t2.Door, t2.Terr, t1.MCust, t1.ship_to, t2.address_name, addr2, addr3, City, State, Postal_code, Country
--
--
IF(OBJECT_ID('tempdb.dbo.#FINAL') is not null)
drop table dbo.#FINAL
Select Status AS Status_type, Door, Terr, Customer, ship_to, address_name, addr2, addr3, City, State, Postal_Code, Country, CY, PY, PYL1, PYL2, PYL3, PYL4,
...
DATEPART(YEAR,@PTo)-5 as 'DPYL4'
 INTO #FINAL
 FROM #Data
 GROUP BY Status, Door, Terr, Customer, ship_to, address_name, addr2, addr3, City, State, Postal_Code, Country, CY, PY, PYL1, PYL2, PYL3, PYL4
 ORDER BY Terr, Customer, Ship_TO

select 
Status_type, Door, Terr, Customer, ship_to, address_name, City, State, Postal_Code, Country, round(CY,2)CY, round(PY,2)PY, round(PYL1,2)PYL1, round(PYL2,2)PYL2, round(PYL3,2)PYL3, round(PYL4,2)PYL4, CYDiff, LYDiff, 
...
--INTO CVO_RankCustShipTo5yrTracker_EL
from #FINAL
