...
create procedure [dbo].[adm_rpt_invinv] @range varchar(8000) = '0=0',
@costs varchar(8000) = 'NULL',
@stat varchar(255) = '',
@order varchar(1000) = ' i.part_no',
@asof_date int = 733132
 as
...
i.min_stock, 
i.min_order, 
i.commit_ed + i.sch_alloc + i.xfer_commit_ed, 
i.recv_mtd, 
i.issued_mtd, 
i.usage_mtd,  
i.sales_qty_mtd, 
...
i.labor,  
i.std_labor,  
i.hold_qty + i.transit,  
i.hold_mfg,  
i.hold_ord,  
i.hold_rcv,  
i.hold_xfr,
i.hold_ord + i.hold_xfr, 
i.in_stock - (i.commit_ed + i.sch_alloc + i.xfer_commit_ed), '
else
select @sql = 'SELECT distinct
i.part_no,
i.description,  
...
where curr_date < ''' + convert(varchar(20),dbo.adm_format_pltdate_f((@asof_date + 1)),110) + ''' group by part_no, location) as tr(part_no, location,tran_id)
on tr.part_no = i.part_no and tr.location = i.location
join inv_tran t on t.tran_id = tr.tran_id'

select @sql = @sql + '
WHERE 1=1 and ' + @stat + @range + '
ORDER BY ' + @order
...
GO
GRANT EXECUTE ON  [dbo].[adm_rpt_invinv] TO [public]
GO
