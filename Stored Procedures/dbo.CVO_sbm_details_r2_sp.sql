...
-- =============================================
-- exec [dbo].[CVO_sbm_details_r2_sp]
...
-- load live data - use artrx to capture validated sales #
        ...
        INSERT  INTO #cvo_sbm_det
                SELECT  customer = ISNULL(xx.customer_code, '') ,
                        ...
                        CASE WHEN ol.return_code = '05-24'
                                  AND co.promo_id = 'BEP' THEN ''
                             ELSE ISNULL(co.promo_id, '')
                        END AS promo_id ,
                        CASE WHEN ol.return_code = '05-24'
                                  AND co.promo_id = 'BEP' THEN ''
                             ELSE ISNULL(co.promo_level, '')
                        END AS promo_level ,
...
--end as asales,
                        ...
                        CASE o.type
                          WHEN 'c'
                          THEN ROUND(ol.cr_shipped * ol.curr_price, 2)
                               - ROUND(ol.cr_shipped * ( ol.curr_price
                                                         * ( ol.discount
                                                             / 100.00 ) ), 2)
                          ELSE 0
                        END AS areturns ,
                        CASE WHEN o.type = 'i' THEN ol.shipped
                             ELSE 0
                        END AS qsales ,
                        CASE WHEN o.type = 'c' THEN ol.cr_shipped
                             ELSE 0
                        END AS qreturns ,
-- add cost and list 11/12/13
                        ROUND(( ol.shipped - ol.cr_shipped ) * ( ol.cost
                                                              + ol.ovhd_dolrs
                                                              + ol.util_dolrs ),
                              2) AS csales ,
...
--
                        ...
                        0 -- isCL
                        ,
                        CASE WHEN o.who_entered = 'BACKORDR' THEN 1
                             ELSE 0
                        END  -- isBO
                        ,
                ...
                FROM                    
                        artrx xx ( NOLOCK ) 
						INNER JOIN orders_invoice oi ( NOLOCK ) ON oi.trx_ctrl_num = xx.trx_ctrl_num    
						INNER JOIN orders o ( NOLOCK ) ON o.order_no = oi.order_no
        ...
        INSERT  INTO #cvo_sbm_det
                SELECT  customer = ISNULL(xx.customer_code, '') ,
                        ...
                        CASE o.type
                          ...
                          THEN CASE ISNULL(cl.is_amt_disc, 'n')
                                 WHEN 'y'
                                 THEN ROUND(ol.shipped * ol.curr_price, 2)
                                      - ROUND(ol.shipped * ISNULL(cl.amt_disc,
                                                              0), 2)
                                 ELSE ROUND(ol.shipped * ol.curr_price, 2)
                                      - ROUND(ol.shipped * ( ol.curr_price
                                                             * ( ol.discount
...
-- add cost and list 11/12/13
                        ...
                        ROUND(( ol.shipped - ol.cr_shipped ) * cl.list_price,
                              2) AS lsales ,
--
                        CONVERT(VARCHAR, DATEADD(d, xx.date_applied - 711858,
                                                 '1/1/1950'), 101) DateShipped ,
                        ISNULL(DATEADD(dd, DATEDIFF(dd, 0, oo.date_entered), 0),
                               CONVERT(VARCHAR, DATEADD(d,
                                                        xx.date_applied
                                                        - 711858, '1/1/1950'), 101)) AS dateOrdered ,
                        0 ,
                        CASE WHEN o.who_entered = 'BACKORDR' THEN 1
                             ELSE 0
                        END ,
                ...
                FROM    
						arinpchg xx ( NOLOCK ) 
						INNER JOIN orders_invoice oi ( NOLOCK ) ON oi.trx_ctrl_num = xx.trx_ctrl_num
						INNER JOIN orders o (nolock) ON oi.order_no = o.order_no
...
*/
                        CASE WHEN CHARINDEX('-', h.order_ctrl_num) <= 1
                             THEN ''
                             ELSE ISNULL(( SELECT TOP 1
                                                    promo_id
                                           FROM     CVO_orders_all co
                                           WHERE    co.order_no = LEFT(h.order_ctrl_num,
                                                              CHARINDEX('-',
                                                              h.order_ctrl_num)
                                                              - 1)
                                                    AND co.ext = 0
                        ...
                        END AS promo_level ,
                        CASE WHEN h.trx_type = 2032 THEN '06-13'
                             ELSE ''
                        END AS return_code ,
                        CASE WHEN CHARINDEX('-', h.order_ctrl_num) <= 1
                             THEN ''
                        ...
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(d.extended_price, 2)
                             ELSE 0
                        END AS asales ,
                        CASE WHEN h.trx_type = 2032
                             THEN ROUND(d.extended_price, 2)
                             ELSE 0
                        END AS areturns ,
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(( d.qty_shipped ), 2)
                             ELSE 0
                        END AS qsales ,
                        CASE WHEN h.trx_type = 2032
                             THEN ROUND(( d.qty_returned ), 2)
                             ELSE 0
                        END AS qreturns ,
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(d.amt_cost * d.qty_shipped, 2)
                             WHEN h.trx_type = 2032
                             THEN ROUND(d.amt_cost * d.qty_returned * -1, 2)
                        END AS csales ,
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(d.extended_price, 2)
                             WHEN h.trx_type = 2032
                             THEN ROUND(d.extended_price * -1, 2)
                        END AS lsales ,
                        CONVERT(VARCHAR, DATEADD(d, h.date_applied - 711858,
                                                 '1/1/1950'), 101) DateShipped ,
                        CONVERT(VARCHAR, DATEADD(d, h.date_applied - 711858,
                                                 '1/1/1950'), 101) AS dateOrdered ,
                        0 ,
                        0 ,
...
-- d.item_code part_no,
                        ...
                        END AS qreturns ,
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(d.unit_cost * d.qty_shipped, 2)
                             WHEN h.trx_type = 2032
                             THEN ROUND(d.unit_cost * d.qty_returned * -1, 2)
                        END AS csales ,
                        CASE WHEN h.trx_type = 2031
                             THEN ROUND(d.extended_price, 2)
                             WHEN h.trx_type = 2032
...
-- load history data
        ...
        IF @first <= '2012-01-12 00:00:00.000'
            BEGIN
                ...
                INSERT  INTO #cvo_sbm_det
                        SELECT  customer = ISNULL(oa.cust_code, '') ,
                                ...
                                END AS year ,
                                CASE WHEN type = 'i'
                                     THEN ISNULL(o.shipped * o.price, 0)
                                     ELSE 0
                                END AS asales ,
                                CASE WHEN type = 'c'
                                     THEN ISNULL(o.cr_shipped * o.price, 0)
                                     ELSE 0
                                END AS areturns ,
                                CASE WHEN type = 'I' THEN ISNULL(o.shipped, 0)
                                     ELSE 0
                                END AS qsales ,
                                CASE WHEN type = 'C'
                                     THEN ISNULL(o.cr_shipped, 0)
                                     ELSE 0
                                END AS qreturns ,
	-- 11/12/13
                                ISNULL(( o.shipped - o.cr_shipped ) * o.cost,
                                       0) AS csales ,
                                ISNULL(( o.shipped - o.cr_shipped )
                                       * pp.price_a, 0) AS lsales ,
	...
	--
                        FROM    CVO_orders_all_Hist oa ( NOLOCK )
                                INNER JOIN cvo_ord_list_hist o ( NOLOCK ) ON oa.order_no = o.order_no
                                ...
                                LEFT OUTER JOIN ( SELECT    order_no ,
                                                  ...
                                                  GROUP BY  ooo.order_no
                                                          ...
                                                          date_entered ) ON oo.order_no = oa.order_no
                        WHERE   1 = 1
                                AND oa.date_shipped BETWEEN @first AND @last;

            END; -- need to regen history ?
...
-- 4/25/2014 - classify closeouts
        ...
        SET     isCL = 1
        WHERE   lsales <> 0
                AND ( 1 - ( asales - areturns ) / lsales ) BETWEEN .8 AND .99; 

        IF ( OBJECT_ID('cvo.dbo.cvo_sbm_details_r2') IS NOT NULL )
	-- drop table cvo.dbo.cvo_sbm_details_r2
            DELETE  FROM dbo.cvo_sbm_details_r2
            WHERE   yyyymmdd BETWEEN @first AND @last;


        IF ( OBJECT_ID('cvo.dbo.cvo_sbm_details_r2') IS NULL )
            BEGIN
		...
		-- new
                ...
                ON dbo.cvo_sbm_details_r2 (asales,areturns,qsales,qreturns,lsales);

                CREATE NONCLUSTERED INDEX idx_cvo_sbm_yyyymmdd
                ON dbo.cvo_sbm_details_r2 (yyyymmdd)
 ...
 -- 030514 - tag dont' mark sales as exc returns -- oops
                             WHEN return_code NOT IN ( '06-13', '06-13B',
                                                       '06-27', '06-32', '' )
                             THEN 'EXC'
                        ...
                        slp
                FROM    #cvo_sbm_det
                        LEFT OUTER JOIN armaster (NOLOCK) ar ON ar.customer_code = customer
