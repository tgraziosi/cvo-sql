...
GO
    ...
    CREATE procedure 
[dbo].[imglintsp_Copy_sp] @imglintsp_Copy_sp_Batch_Number INT = 0,
                  @imglintsp_Copy_sp_Starting_Record_Number INT = 0,
                  @imglintsp_Copy_sp_Ending_Record_Number INT = 0,
                  @imglintsp_Copy_sp_Flag_Bits INT = 0,
                  @debug_level INT = 0,
                  @imglintsp_Copy_sp_Dummy_2 INT = 0,
                  @imglintsp_Copy_sp_Import_Identifier INT = 0,
                  @userid INT = 0,
                  @imglintsp_Copy_sp_Dummy_3 VARCHAR(40),
                  @imglintsp_Copy_sp_Dummy_4 VARCHAR(40),
                  @imglintsp_Copy_sp_Application_Name VARCHAR(30) = 'Import Manager',
                  @imglintsp_Copy_sp_Override_User_Name VARCHAR(30) = '',
                  @imglintsp_Copy_sp_Dummy_5 INT = NULL
    AS
    ...
    SET @Routine_Name = 'imglintsp_Copy_sp'        
    IF @debug_level >= 3
    ...
    IF (@imglintsp_Copy_sp_Flag_Bits & 0x1) > 0
        SET @close_batch_flag = 1
    SET @post_flag = 0
    IF (@imglintsp_Copy_sp_Flag_Bits & 0x2) > 0
        SET @post_flag = 1
    SET @init_mode = @close_batch_flag + @post_flag    
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imglintsp_sp.'
    IF @imglintsp_Copy_sp_Import_Identifier = 0    
            OR @imglintsp_Copy_sp_Import_Identifier IS NULL
        BEGIN        
        EXEC @SP_Result = imglintsp_sp @company_code = @company_code,
                                       ...
                                       @imglintsp_sp_Import_Identifier = @imglintsp_Copy_sp_Import_Identifier OUTPUT,
                                       @imglintsp_sp_Override_User_Name = @imglintsp_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imglintsp_sp' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    ELSE    
        BEGIN        
        EXEC @SP_Result = imglintsp_sp @company_code = @company_code,
                                       ...
                                       @imglintsp_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                       @imglintsp_sp_dummy = @Dummy OUTPUT,
                                       @imglintsp_sp_Import_Identifier = @imglintsp_Copy_sp_Import_Identifier,
                                       @imglintsp_sp_Override_User_Name = @imglintsp_Copy_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imglintsp_sp' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
    ...
    SET @Import_Identifier = @imglintsp_Copy_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imglintsp_Copy_sp] TO [public]
GO
