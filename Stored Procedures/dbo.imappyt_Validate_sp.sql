...
GO
    ...
    CREATE procedure 
[dbo].[imappyt_Validate_sp] (@imappyt_Validate_sp_Batch_Number INT = 0,
                     @imappyt_Validate_sp_Starting_Record_Number INT = 0,
                     @imappyt_Validate_sp_Ending_Record_Number INT = 0,
                     @imappyt_Validate_sp_Dummy_1 INT = 0,
                     @debug_level INT = 0,
                     @imappyt_Validate_sp_Dummy_2 INT = 0,
                     @imappyt_Validate_sp_Import_Identifier INT = 0,
                     @userid INT = 0,
                     @imappyt_Validate_sp_Application_Name VARCHAR(30) = 'Import Manager',
                     @imappyt_Validate_sp_Override_User_Name VARCHAR(30) = '')
    AS
    ...
    SET @Routine_Name = 'imappyt_Validate_sp'        
    IF @debug_level >= 3
    ...
    --
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Initial @imappyt_Validate_sp_Import_Identifier = ' + CAST(@imappyt_Validate_sp_Import_Identifier AS VARCHAR)
    IF @imappyt_Validate_sp_Import_Identifier = 0    
            OR @imappyt_Validate_sp_Import_Identifier IS NULL
        BEGIN        
        IF @debug_level >= 3
            SELECT '(3): ' + @Routine_Name + ': Before imappyt_sp, Import_Identifier OUTPUT.'
        EXEC @SP_Result = imappyt_sp @company_code = @company_code,
                                     @trial_flag = 1,
                                     @debug_level = @debug_level,
                                     @userid = @userid,
                                     @imappyt_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                     @imappyt_sp_dummy = @Dummy OUTPUT,
                                     @imappyt_sp_Import_Identifier = @imappyt_Validate_sp_Import_Identifier OUTPUT,
                                     @imappyt_sp_Override_User_Name = @imappyt_Validate_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imappyt_sp 1' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    ELSE    
        BEGIN        
        IF @debug_level >= 3
            SELECT '(3): ' + @Routine_Name + ': Before imappyt_sp, Import_Identifier.'
        EXEC @SP_Result = imappyt_sp @company_code = @company_code,
                                     @trial_flag = 1,
                                     @debug_level = @debug_level,
                                     @userid = @userid,
                                     @imappyt_sp_process_ctrl_num = @process_ctrl_num OUTPUT,
                                     @imappyt_sp_dummy = @Dummy OUTPUT,
                                     @imappyt_sp_Import_Identifier = @imappyt_Validate_sp_Import_Identifier,
                                     @imappyt_sp_Override_User_Name = @imappyt_Validate_sp_Override_User_Name
        SELECT @Error_Code = @@ERROR, @Row_Count = @@ROWCOUNT IF NOT @Error_Code = 0 AND NOT @Error_Code = 8153 BEGIN IF @ROLLBACK_On_Error = 'YES' BEGIN ROLLBACK TRANSACTION SET @ROLLBACK_On_Error = 'NO' END SET @CSS_Intermediate_String = @External_String_EXEC + ' imappyt_sp 2' EXEC im_log_sql_error_sp @ILSE_Error_Code = @Error_Code, @ILSE_String = @CSS_Intermediate_String, @ILSE_Procedure_Name = @Routine_Name, @ILSE_Log_Activity = 'YES', @im_log_sql_error_sp_User_ID = @userid IF @debug_level >= 3 BEGIN SELECT '(3): ERROR: ' + @CSS_Intermediate_String END GOTO Error_Return END
        END
    IF NOT @SP_Result = 0
        BEGIN
        EXEC im_log_sp_error_sp @ILSE_Error_Code = @SP_Result,
                                @ILSE_SP_Name = 'imappyt_sp',
                                @ILSE_String = '',
    ...
    IF @debug_level >= 3
        BEGIN
        SELECT '(3): ' + @Routine_Name + ': After imappyt_sp.'
        SELECT '(3): ' + @Routine_Name + ': Post-imappyt_sp @imappyt_Validate_sp_Import_Identifier = ' + CAST(@imappyt_Validate_sp_Import_Identifier AS VARCHAR)
        END
    SET @Import_Identifier = @imappyt_Validate_sp_Import_Identifier 
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imappyt_Validate_sp] TO [public]
GO
