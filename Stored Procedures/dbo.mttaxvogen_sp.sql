...
CREATE PROC [dbo].[mttaxvogen_sp] @match_ctrl_num varchar(16),   
...
create table #TXtaxtyperec (  
  ...
  seq_id int,  
  base_id int,  
  cur_amt decimal(20,8),    old_tax decimal(20,8),  
  tax_type varchar(8)  
)  
...
IF((@tax_connect_flag = 1) AND (@tax_code_istax_connect = 1 ))  
BEGIN  
 ...
 SELECT @tc_currency_code = nat_cur_code   
 FROM epmchhdr (nolock) WHERE match_ctrl_num = @match_ctrl_num  
   
 SELECT @curr_precision = curr_precision from glcurr_vw where currency_code = @tc_currency_code  
 ...
 INSERT #txconnhdrinput  
    (doccode, doctype, trx_type, companycode, docdate, exemptionno, salespersoncode,  
     ...
     currCode, currRate)  
 SELECT @trx_ctrl_num, @h_doc_type, 4091, @tax_companycode, getdate(), '', '',  
      0,                    '', @vendor_code,                 '',           3,     '',   
     ...
     @tc_currency_code, @curr_precision  
 ...
 SELECT @match_ctrl_num = #epmchdtl.match_ctrl_num,   
 @sequence_id = #epmchdtl.sequence_id,  
 @unit_price = #epmchdtl.invoice_unit_price,   
 @item_code = epinvdtl.item_code,   
 @qty_invoiced = #epmchdtl.qty_invoiced,   
 @tax_code = #epmchdtl.tax_code,  
 @receipt_ctrl_num = #epmchdtl.receipt_ctrl_num   
 FROM  #epmchdtl, epinvdtl (nolock)  
 WHERE  #epmchdtl.match_ctrl_num = @match_ctrl_num  
 AND  #epmchdtl.receipt_dtl_key = epinvdtl.receipt_detail_key  
 AND    #epmchdtl.po_sequence_id = epinvdtl.po_sequence_id  
 ...
 BEGIN  
  ...
  SELECT @ext_price = Round( @unit_price * @qty_invoiced, @curr_precision )  
   ...
   select @trx_ctrl_num, @sequence_id, @tc_currency_code,   
     @curr_precision, @tax_code, @qty_invoiced,@unit_price,   
     @ext_price, @sequence_id  
 ...
 begin  
  ...
  AND a.doccode = @trx_ctrl_num  
  AND b.sequence_id = a.no  
  AND b.amt_tax != b.calc_tax  
    
  INSERT INTO gltcDocTaxOverride (trx_ctrl_num, trx_type, line, TaxOverrided)  
  select @trx_ctrl_num, @trx_type, sequence_id, 1   
  from #epmchdtl where match_ctrl_num = @match_ctrl_num  
  and amt_tax != calc_tax  
 ...
 select @total_tax=0, @non_included_tax=0, @included_tax=0  
   ...
   INSERT #mtinptaxdtl ( match_ctrl_num,   
    ...
    recoverable_flag,  
          ...
          SELECT @match_ctrl_num ,  
                 ...
                 1031,    
                 ti.d_index + 1,   
                 CASE WHEN ti.reference_number < 0 THEN ti.reference_number ELSE ti.t_index + 1 END,    
                 tt.tax_type,  
                 ti.taxable,  
  ...
  FROM #epmchdtl epm,  #TXLineInput_ex TXL   
   WHERE   epm.match_ctrl_num = @match_ctrl_num  
       AND  TXL.seqid  = epm.sequence_id   
       AND TXL.control_number = @trx_ctrl_num   
...
GO
GRANT EXECUTE ON  [dbo].[mttaxvogen_sp] TO [public]
GO
