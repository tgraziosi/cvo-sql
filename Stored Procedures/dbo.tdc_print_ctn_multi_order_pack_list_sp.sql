...
CREATE PROCEDURE [dbo].[tdc_print_ctn_multi_order_pack_list_sp](  
   @user_id    varchar(50),  
...
DECLARE  
 ...
 @Special_Instr   varchar (275), @header_add_note varchar (255),  
 -- START v1.1
...
EXEC @return_value = tdc_print_label_sp 'PPS', 'CTNMULTIPACKTKT', 'VB', @station_id  
  
-- IF label hasn't been set up for the station id, try finding a record for the user id  
IF @return_value != 0  
BEGIN  
 EXEC @return_value = tdc_print_label_sp 'PPS', 'CTNMULTIPACKTKT', 'VB', @user_id  
END  
  ...
  BEGIN  
   ...
   -- Get Total Packed Qty for the Item on the Order / Carton  
    SELECT @Sum_Pack_Qty = ISNULL(  
            (SELECT CAST(SUM(pack_qty) AS varchar (20))  
          FROM tdc_carton_detail_tx   
            WHERE order_no  = @order_no  
            AND order_ext = @order_ext   
            AND part_no   = @part_no  
   ...
   SELECT @Order_Plus_Ext     = (CAST(@order_no  AS varchar(10)) + '-' + CAST  (@order_ext  AS varchar(10))), --detail  
    @Cust_PO            = orders.cust_po,    --detail  
    @Note      = REPLACE(orders.note, CHAR(13)+CHAR(10), '/'), --detail  
    @Req_Ship_Date      = orders.req_ship_date,      --detail        
    @Sch_Ship_Date      = orders.sch_ship_date,  --detail  
    @Special_Instr      = REPLACE(orders.special_instr, CHAR(13)+CHAR(10), '/'),    --detail   
    @salesperson        = orders.salesperson,       --detail  
    @dest_zone_code     = orders.dest_zone_code,  --detail  
    @back_ord_flag      = orders.back_ord_flag,  --detail  
    @ship_to_no         = orders.ship_to,   --detail  
    @terms              = orders.terms   --detail  
     FROM  tdc_order   (NOLOCK)  
    INNER  JOIN orders (NOLOCK) ON  
           tdc_order.order_no  = orders.order_no  
      AND  tdc_order.order_ext = orders.ext  
   ...
   --New Detail line items  
   INSERT INTO #tdc_print_ticket (print_value) VALUES ('LP_CUST_PO_'     + RTRIM(CAST(@printed_on_the_page AS char(4))) + ',' + ISNULL(@cust_po,          ''))  
   ...
   INSERT INTO #tdc_print_ticket (print_value) VALUES('LP_ORDER_PLUS_EXT_'    + RTRIM(CAST(@printed_on_the_page AS char(4))) + ',' + @order_plus_ext)  
...
RETURN  
GO
GRANT EXECUTE ON  [dbo].[tdc_print_ctn_multi_order_pack_list_sp] TO [public]
GO
