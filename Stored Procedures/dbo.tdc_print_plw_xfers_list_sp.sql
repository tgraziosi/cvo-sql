...
CREATE PROCEDURE [dbo].[tdc_print_plw_xfers_list_sp]
			@user_id    varchar(50),
...
EXEC @return_value = tdc_print_label_sp 'PLW', 'XFERLIST', 'VB', @station_id

-- IF label hasn't been set up for the station id, try finding a record for the user id
IF @return_value != 0
BEGIN
	EXEC @return_value = tdc_print_label_sp 'PLW', 'XFERLIST', 'VB', @user_id
END
...
BEGIN
	...
	SELECT @details_count = COUNT(*) FROM #pick_ticket_details WHERE sel_flg <> 0
	...
	BEGIN
		...
		SELECT @max_details_on_page = ISNULL((SELECT value_str FROM tdc_config (NOLOCK) WHERE [function] = 'Xfer_List_Detl_Count'), 4) 
	END	
	...
	DECLARE detail_cursor CURSOR FOR 
		SELECT CAST(xfer_no AS varchar(10)) FROM #pick_ticket_details WHERE sel_flg <> 0
		 ORDER BY xfer_no

	OPEN detail_cursor

	FETCH NEXT FROM detail_cursor INTO @xfer_no

	WHILE (@@FETCH_STATUS <> -1)
	BEGIN
		-------------- Now let's insert the Details into the output table -----------------			
		INSERT INTO #tdc_print_ticket (print_value) SELECT 'LP_XFER_NO_'     + RTRIM(CAST(@printed_on_the_page AS char(4))) + ',' + ISNULL(@xfer_no, '')
...
RETURN
GO
GRANT EXECUTE ON  [dbo].[tdc_print_plw_xfers_list_sp] TO [public]
GO
