...
*/
    ...
    SELECT @reason_code = CASE
                              WHEN @reason = 'WRITEOFF' THEN
                                  'WRITE-OFF'
                              ELSE
                                  'ADJ-ADHOC'
                          END;
    ...
    IF
    (
        SELECT OBJECT_ID('tempdb..#adm_inv_adj_log')
    ) IS NOT NULL
    BEGIN
    ...
    IF
    (
        SELECT OBJECT_ID('tempdb..#cvo_inv_adj')
    ) IS NOT NULL
    BEGIN
    ...
    BEGIN
        ...
        VALUES
        (@loc, '', 0, @direction, 'Error', @code, 'Nothing to process', -1);
    -- RETURN;
    END;
    ...
    BEGIN
        ...
        VALUES
        (@loc, @bin, '', 0, @direction, 'Error', @code, 'Invalid Location or Bin', -1);
    ...
    BEGIN
        ...
        VALUES
        (@loc, @bin, '', 0, @direction, 'Error', @code, 'Invalid Direction.  Must be 1 or -1', -1);
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUES
            (@location, @part_no, @bin_no, '1', GETDATE() + 365, @qty_to_adj, @direction, 'Inv Adj', @reason_code,
             @code);
    ...
    -- 3/12/2019
    IF @bin = 'rr putaway'
        EXECUTE dbo.cvo_clean_up_S_assignments; -- remove unused assignments in the SA area
