...
CREATE PROC [dbo].[glbalprdcurr_eu_sp]          @start_date             int,    
...
BEGIN
	...
	SELECT  b.account_code,
		...
		(1-ABS(SIGN(@first_balance_date-b.balance_date))),
        	...
        	(1-ABS(SIGN(@first_balance_date-b.balance_date)))
	FROM    glbal b, #balances c, #range_currency rc
	...
	AND	b.currency_code = rc.currency_code
	GROUP BY b.account_code, b.balance_date


	UPDATE  #balances
	SET     beginning_balance = t.home_current_balance - t.home_net_change,
		ending_balance = t.home_current_balance,
		oper_beginning_balance = t.current_balance_oper - t.net_change_oper,
...
ubal1	float
	...
	FROM            gltrx h, gltrxdet d, glchart a, #range_currency rc
	WHERE           h.journal_ctrl_num = d.journal_ctrl_num
...
GO
GRANT EXECUTE ON  [dbo].[glbalprdcurr_eu_sp] TO [public]
GO
