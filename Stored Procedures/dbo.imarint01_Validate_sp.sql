...
GO
    ...
    CREATE PROC 
[dbo].[imarint01_Validate_sp] @imarint01_Validate_sp_Batch_Number INT = 0,
                      @imarint01_Validate_sp_Starting_Record_Number INT = 0,
                      @imarint01_Validate_sp_Ending_Record_Number INT = 0,
                      @imarint01_Validate_sp_Flag_Bits INT = 0,
                      @debug_level INT = 0,
                      @imarint01_Validate_sp_Record_Type INT = 2031,
                      @imarint01_Validate_sp_Import_Identifier INT = 0,
                      @userid INT = 0,
                      @imarint01_Validate_sp_Application_Name VARCHAR(30) = 'Import Manager',
                      @imarint01_Validate_sp_Override_User_Name VARCHAR(30) = ''
    AS
    ...
    SET @Routine_Name = 'imarint01_Validate_sp'        
    IF @debug_level >= 3
    ...
    IF (@imarint01_Validate_sp_Flag_Bits & 0x1) > 0
        SET @close_batch_flag = 1
    IF @debug_level >= 3
        SELECT '(3): ' + @Routine_Name + ': Before imarint01_sp.'
    IF @imarint01_Validate_sp_Import_Identifier = 0     
            OR @imarint01_Validate_sp_Import_Identifier IS NULL
        BEGIN         
        EXEC @SP_Result = imarint01_sp @company_code = @company_code,
                                       ...
                                       @total_flag = 0,
                                       @post_flag = 0,
                                       @invoice_flag = @imarint01_Validate_sp_Record_Type,
                                       @validation_flag = 1,
                                       ...
                                       @imarint01_sp_Import_Identifier = @imarint01_Validate_sp_Import_Identifier OUTPUT,
                                       @imarint01_sp_Override_User_Name = @imarint01_Validate_sp_Override_User_Name
    ...
    ELSE    
        ...
        EXEC @SP_Result = imarint01_sp @company_code = @company_code,
                                       ...
                                       @imarint01_sp_process_ctrl_num_Validation = @process_ctrl_num OUTPUT,
                                       @imarint01_sp_process_ctrl_num_Posting = @Dummy OUTPUT,
                                       @imarint01_sp_Import_Identifier = @imarint01_Validate_sp_Import_Identifier,
                                       @imarint01_sp_Override_User_Name = @imarint01_Validate_sp_Override_User_Name
    ...
    SET @Import_Identifier = @imarint01_Validate_sp_Import_Identifier
    --
...
Error_Return:
    ...
    RETURN -1    
GO
GRANT EXECUTE ON  [dbo].[imarint01_Validate_sp] TO [public]
GO
