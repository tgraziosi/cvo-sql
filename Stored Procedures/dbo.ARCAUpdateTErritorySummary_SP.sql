...
CREATE PROC [dbo].[ARCAUpdateTErritorySummary_SP]	@batch_ctrl_num	varchar( 16 ),
...
IF ( @perf_level >= 1 ) EXEC perf_sp @batch_ctrl_num, "tmp/arcautes.sp", 55, "Entering ARCAUpdateTErritorySummary_SP", @PERF_time_last OUTPUT

BEGIN
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcautes.sp" + ", line " + STR( 58, 5 ) + " -- ENTRY: "
	...
	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcautes.sp" + ", line " + STR( 104, 5 ) + " -- EXIT: "
		RETURN 34563
	END
	...
	SET	num_inv_paid = - invoice_pre.num_inv_paid + ISNULL(( 	SELECT SUM(invoice_post.paid_flag)	
										...
										WHERE	#arsumter_work.territory_code = invoice_post.territory_code 
										AND	invoice_post.trx_type <= 2031 ),0.0)
	FROM	#arsumter_pre	invoice_pre
	WHERE	#arsumter_work.territory_code = invoice_pre.territory_code

	IF( @@error != 0 )
	BEGIN
		IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcautes.sp" + ", line " + STR( 125, 5 ) + " -- EXIT: "
		RETURN 34563
	END	
	...
	SET	num_nsf = (	SELECT count(b.doc_ctrl_num) 	 
				...
				AND	#arsumter_work.territory_code = c.territory_code
				AND	b.trx_type = 2121
			 )
	...
	SET	amt_nsf = (	SELECT SUM(ROUND(a.amt_applied * ( SIGN(1 + SIGN(c.rate_home))*(c.rate_home) + (SIGN(ABS(SIGN(ROUND(c.rate_home,6))))/(c.rate_home + SIGN(1 - ABS(SIGN(ROUND(c.rate_home,6)))))) * SIGN(SIGN(c.rate_home) - 1) ), @home_precision)) 	 
				...
				AND	#arsumter_work.territory_code = b.territory_code
				AND	a.trx_type = 2121
			 )
	...
	IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "tmp/arcautes.sp" + ", line " + STR( 169, 5 ) + " -- EXIT: "
	RETURN 0 
END
GO
GRANT EXECUTE ON  [dbo].[ARCAUpdateTErritorySummary_SP] TO [public]
GO
