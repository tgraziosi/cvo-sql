...
Copyright (c) 1996 Platinum Software Corporation, 1996
                 All Rights Reserved 
 */
                               CREATE PROCEDURE [dbo].[gl_opentaxrep_ap_gen_sp] @report_date int, 
@report_cur smallint, @ret_val int OUTPUT AS BEGIN DECLARE @today int  IF ((@report_date IS NULL) OR 
 (@report_cur IS NULL)) RETURN 2 SELECT @ret_val = 0 SET ROWCOUNT 0  INSERT #gl_opentaxrep 
(
...
SELECT  apvohdr.trx_ctrl_num,  apvohdr.doc_ctrl_num,  aptrxtax.trx_type,  gltaxboxdet.tax_box_code, 
 ...
 aptrxtax.trx_ctrl_num = apvohdr.trx_ctrl_num AND  aptrxtax.tax_type_code = aptxtype.tax_type_code AND 
 aptrxtax.trx_type = 4091 AND  aptrxtax.tax_type_code = gltaxboxdet.tax_type_code AND 
 apvohdr.date_applied < @report_date IF (@@error <>0)  BEGIN  SELECT @ret_val = 1 
 ROLLBACK TRAN  RETURN @ret_val  END  INSERT #gl_opentaxrep 
(
 trx_ctrl_num,  doc_ctrl_num,  trx_type,  tax_box_code,  tax_box_rep_seq,  tax_type_code, 
 tax_type_desc,  description,  nat_cur_code,  amt_net,  amt_tax,  rate_home,  rate_oper, 
 date_applied,  date_doc,  tax_type 
...
SELECT  apdmhdr.trx_ctrl_num,  apdmhdr.doc_ctrl_num,  aptrxtax.trx_type,  gltaxboxdet.tax_box_code, 
 ...
 ELSE 1  END FROM  apdmhdr,  aptrxtax,  gltaxboxdet,  aptxtype WHERE  aptrxtax.trx_ctrl_num = apdmhdr.trx_ctrl_num AND 
 aptrxtax.tax_type_code = aptxtype.tax_type_code AND  aptrxtax.trx_type = 4092 AND 
 aptrxtax.tax_type_code = gltaxboxdet.tax_type_code AND  apdmhdr.date_applied < @report_date 
IF (@@error <>0)  BEGIN  SELECT @ret_val = 1  ROLLBACK TRAN  RETURN @ret_val  END 
 INSERT #gl_opentaxrep 
(
 trx_ctrl_num,  doc_ctrl_num,  trx_type,  tax_box_code,  tax_box_rep_seq,  tax_type_code, 
 tax_type_desc,  description,  nat_cur_code,  amt_net,  amt_tax,  rate_home,  rate_oper, 
 date_applied,  date_doc,  tax_type 
...
SELECT  appyhdr.trx_ctrl_num,  appyhdr.doc_ctrl_num,  aptrxtax.trx_type,  gltaxboxdet.tax_box_code, 
 ...
 ELSE 1  END FROM  appyhdr,  aptrxtax,  gltaxboxdet WHERE  aptrxtax.trx_ctrl_num = appyhdr.trx_ctrl_num AND 
 aptrxtax.trx_type = 4111 AND  aptrxtax.tax_type_code = gltaxboxdet.tax_type_code AND 
 appyhdr.date_applied < @report_date IF (@@error <>0)  BEGIN  SELECT @ret_val = 1 
 ROLLBACK TRAN  RETURN @ret_val  END DELETE #gl_opentaxrep WHERE  EXISTS  (SELECT * FROM gl_taxrep_dtl td WHERE td.trx_ctrl_num = #gl_opentaxrep.trx_ctrl_num 
...
FROM  #gl_opentaxrep tr JOIN aptxtype tt  ON tr.tax_type_code = tt.tax_type_code 
END 
GO
GRANT EXECUTE ON  [dbo].[gl_opentaxrep_ap_gen_sp] TO [public]
GO
