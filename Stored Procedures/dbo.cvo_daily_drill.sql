...
SET ANSI_NULLS ON
GO
--  cvo_daily_drill '1/17/2012','Brendeda','Brendeda'    
CREATE procedure [dbo].[cvo_daily_drill] @d datetime, @FTerr nvarchar(25), @tTerr nvarchar(25)                           
AS                                    
...
set @friday=DATEADD(day,6,@monday)         
        
        

selecT t6.sales_mgr_code,t5.salesperson_code,t2.order_no,t2.ext,t2.cust_code,t5.address_name,t2.ship_to,t2.date_entered,t2.user_category      
...
inner join armaster_all t5 (nolock) on t5.customer_code=t2.cust_code    
inner join arsalesp t6 (nolock) on t6.salesperson_code=t5.salesperson_code
where t2.date_entered between @monday and @friday        
and t2.salesperson between @Fterr and @tterr                                           
and t4.type_code in ('SUN','FRAME')      
and t2.user_category like 'ST%' and t2.ext=0 and type='I'      
group by t2.order_no,t2.ext,t2.cust_code,t2.ship_to,t2.date_entered,t2.user_category,t5.address_name,t5.salesperson_code,t6.sales_mgr_code     
GO
