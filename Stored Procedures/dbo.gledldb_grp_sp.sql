...
CREATE PROCEDURE [dbo].[gledldb_grp_sp] 
		 @db_name varchar(128),
...
IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "gledldb_grp.cpp" + ", line " + STR( 66, 5 ) + " -- ENTRY: "
...
BEGIN
  ...
  BEGIN
	...
	CREATE INDEX gltrxedt_ic_ind_0 ON #gltrxedt_ic (journal_ctrl_num)
    ...
    FROM  #gltrxedt_ic edt
		INNER JOIN #glcocodt coco ON edt.company_code = coco.org_code AND edt.rec_company_code = coco.rec_code AND edt.account_code LIKE coco.account_mask
    WHERE    edt.mark_flag = 0 
       AND edt.db_name = @db_name


    SELECT  edt.* INTO #gltrxedt_fail
    FROM  #gltrxedt_ic edt
		INNER JOIN #glcocodt coco ON edt.company_code = coco.org_code AND edt.rec_company_code = coco.rec_code AND edt.account_code NOT LIKE coco.account_mask
    WHERE edt.mark_flag = 0 
		AND edt.db_name = @db_name
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
		LEFT JOIN glchart ch ON ed.account_code = ch.account_code
  WHERE   ed.sequence_id > -1
  AND     db_name = @db_name
  AND ch.account_type IS NULL
END 
...
BEGIN
  ...
  SET     temp_flag = 2
  FROM    #gltrxedt1 ed
		INNER JOIN glprd prd ON ed.date_applied BETWEEN prd.period_start_date AND prd.period_end_date
  WHERE   db_name = @db_name
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
		INNER JOIN glprd prd ON ed.date_applied < prd.period_start_date AND prd.period_end_date = @current_period_end_date
  WHERE   db_name = @db_name
  AND     temp_flag = 2
END
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
	INNER JOIN glprd prd ON ed.date_applied > prd.period_end_date AND prd.period_end_date = @current_period_end_date
  WHERE   db_name = @db_name
  AND     temp_flag = 2
END
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
		INNER JOIN glprd prd ON ed.date_applied BETWEEN prd.period_start_date AND prd.period_end_date
		INNER JOIN glbal bal ON prd.period_end_date = bal.balance_date AND bal.account_code = ed.account_code
  WHERE   db_name = @db_name
  AND     temp_flag = 2
...
BEGIN
  ...
  SELECT DISTINCT journal_ctrl_num, 2009, 0  
  FROM    #gltrxedt1 ed
		INNER JOIN glincsum i ON ed.account_code LIKE i.account_pattern AND i.is_acct_code NOT IN (SELECT account_code FROM glchart)
  WHERE   temp_flag > 0
  AND     db_name = @db_name
END
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
		INNER JOIN glincsum i ON ed.account_code LIKE i.account_pattern AND i.re_acct_code NOT IN (SELECT account_code FROM glchart)
  WHERE   temp_flag > 0
  AND     db_name = @db_name
END
...
BEGIN
  ...
  FROM    #gltrxedt1 ed
	...
	INNER JOIN glcomp_vw b ON ed.company_code = b.company_code AND b.company_id = a.parent_comp_id
	INNER JOIN glcomp_vw c ON ed.rec_company_code = c.company_code AND c.company_id = a.sub_comp_id
  WHERE   ed.db_name = @db_name
...
IF ( @debug_level > 1 ) SELECT CONVERT(char,getdate(),109) + "  " + "gledldb_grp.cpp" + ", line " + STR( 930, 5 ) + " -- EXIT: "


RETURN
GO
GRANT EXECUTE ON  [dbo].[gledldb_grp_sp] TO [public]
GO
