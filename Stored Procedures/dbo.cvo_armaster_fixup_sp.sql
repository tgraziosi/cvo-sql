...
-- FIX CONSISTENCY ON CASE
    ...
    SET     addr_sort1 = CASE WHEN addr_sort1 = 'Buying Group'
                              THEN 'Buying Group'
                              WHEN addr_sort1 = 'Customer' THEN 'Customer'
                              WHEN addr_sort1 = 'Distributor'
                              THEN 'Distributor'
                              WHEN addr_sort1 = 'Employee' THEN 'Employee'
                              WHEN addr_sort1 = 'Intl Retailer'
                              THEN 'Intl Retailer'
                              WHEN addr_sort1 = 'Key Account'
                              THEN 'Key Account'
                              ELSE addr_sort1
                         END;
...
-- UPDATE TO CHECK WHERE MAIN ACCOUNT IS CLOSED OR NONEWBUSINESS AND SET SHIPTO'S TO MATCH
    UPDATE  t1
    SET     status_type = ( SELECT  t11.status_type
                            FROM    armaster t11
    ...
    WHERE   address_type = 1
            AND ( SELECT    t11.status_type
                  FROM      armaster t11 ( NOLOCK )
                  WHERE     t1.customer_code = t11.customer_code
                            AND address_type = 0
                ) <> 1
            AND t1.status_type <> ( SELECT  t11.status_type
                                    FROM    armaster t11 ( NOLOCK )
                                    WHERE   t1.customer_code = t11.customer_code
                                            AND address_type = 0
                                  );
...
-- UPDATE WHERE STATE IS IN US AND COUNTRY CODE IS BLANK
    ...
    SET     country_code = 'US'
    WHERE   country_code <> 'us'
            AND state IN ( 'AK', 'AL', 'AR', 'AZ', 'CA', 'CO', 'CT', 'DC',
...
-- select ar.customer_code, ar.ship_to_code, added_by_date, dateadd(dd, datediff(dd, 0 , FirstOrder), 0)
    FROM    armaster ar ( ROWLOCK )
            JOIN ( SELECT   cust_code ,
                            ship_to ,
                            MIN(date_entered) FirstOrder
                   FROM     orders_all t2 ( NOLOCK )
                   GROUP BY cust_code ,
                            ship_to
                 ) AS o ON ar.customer_code = o.cust_code
