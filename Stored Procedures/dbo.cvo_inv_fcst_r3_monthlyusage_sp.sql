...
CREATE PROCEDURE [dbo].[cvo_inv_fcst_r3_monthlyusage_sp]
...
-- exec cvo_inv_fcst_r3_monthlyusage_sp
...
*/
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT location,
                   ...
                   gross_w4,
                   gross_w12
            FROM dbo.f_cvo_calc_weekly_monthly_usage_loc(@usg_option, @t, @co, @lo);
    ...
    SELECT cte.brand,
           ...
           ISNULL(drp.s_e1_mu, 0) s_e1_mu,
           ISNULL(drp.s_e3_mu, 0) s_e3_mu,
           ISNULL(drp.s_e12_mu, 0) s_e12_mu,
    ...
    FROM #cte cte
        ...
        SELECT -- usage info
            ...
            SUM(ISNULL(drp.e1_mu, 0)) s_e1_mu,
            SUM(ISNULL(drp.e3_mu, 0)) s_e3_mu,
            SUM(ISNULL(drp.e12_mu, 0)) s_e12_mu,
    ...
    SELECT s.brand,
           ...
           ISNULL(s.s_e1_mu, 0) s_e1_mu,
           ISNULL(s.s_e3_mu, 0) s_e3_mu,
           ISNULL(s.s_e12_mu, 0) s_e12_mu,
    ...
    ,
           ...
           ISNULL(drp.p_e1_mu, 0) p_e1_mu,
           ISNULL(drp.p_e3_mu, 0) p_e3_mu,
           ISNULL(drp.p_e12_mu, 0) p_e12_mu,
    ...
    ,
           ...
           ROUND(
                    (CASE WHEN s.mth_since_rel <= 3 THEN ISNULL(s.s_e1_mu, 0) ELSE ISNULL(s.s_e3_mu, 0) END
                    ),
                    0,
                    1
                ) s_mth_usg,
           ROUND(
                    (CASE WHEN s.mth_since_rel <= 3 THEN ISNULL(drp.p_e1_mu, 0) ELSE ISNULL(drp.p_e3_mu, 0) END
                    ),
                    0,
                    1
                ) p_mth_usg,
           ROUND(
                    ((CASE WHEN s.mth_since_rel <= 3 THEN ISNULL(s.s_e1_mu, 0) ELSE ISNULL(s.s_e12_wu, 0) END ) * dmd.mult
                    ),
                    0,
                    1
                ) s_mth_usg_mult,
           ROUND(
                    ((CASE WHEN s.mth_since_rel <= 3 THEN ISNULL(drp.p_e1_mu, 0) ELSE ISNULL(drp.p_e3_mu, 0) END ) * dmd.mult
                    ),
                    0,
                    1
                ) p_mth_usg_mult,
           ROUND(
                    (CASE WHEN ISNULL(CAST(s.s_e3_mu AS DECIMAL), 0.00) <> 0.00 THEN
                              ISNULL(CAST(drp.p_e3_mu AS DECIMAL), 0.00) / ISNULL(CAST(s.s_e3_mu AS DECIMAL), 0) ELSE
                                                                                                                       0.00
                     END
                    ),
                    4
    ...
    FROM dbo.inv_master i (NOLOCK)
        ...
        SELECT -- drp info by part
            ...
            SUM(ISNULL(drp.e1_mu, 0)) p_e1_mu,
            SUM(ISNULL(drp.e3_mu, 0)) p_e3_mu,
            SUM(ISNULL(drp.e12_mu, 0)) p_e12_mu,
    ...
    SELECT 'DRP' AS LINE_TYPE,
           ...
           --     ) quantity,
            ROUND(
                    dmd.mult * dmd.s_mult
                    * (CASE WHEN DATEDIFF(mm, ia.field_26, @asofdate) > 3 THEN ISNULL(t.p_e3_mu, 0) ELSE ISNULL(t.p_e1_mu,0) END ), 0, 1) quantity,
           t.mult,
           t.s_mult,
           t.sort_seq,
           0 alloc_qty,
...
GO
GRANT EXECUTE ON  [dbo].[cvo_inv_fcst_r3_monthlyusage_sp] TO [public]
GO
