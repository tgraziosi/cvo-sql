...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[cvo_hello_customer_intro_sp]
    @cust VARCHAR(12) ,
...
exec cvo_hello_customer_intro_sp '011111', null, null, 1
exec cvo_hello_customer_sp '011111', null, null, 1

exec cvo_hello_customer_sp '054421' , '41673985399', null, 1
...
*/
    ...
    BEGIN
        ...
        FROM   cvo_date_range_vw
        WHERE  Period = 'today';
        SELECT @r12start = BeginDate ,
               @r12end = EndDate
        ...
        INSERT #info ( Info ,
               ...
               WHERE  ar.address_type <> 9
                      ...
                      AND ar.ship_to_code = ISNULL(
                                                @ship_to_code ,ar.ship_to_code)
        ...
        FROM   #info AS i
               ...
               JOIN dbo.CVO_armaster_all AS car ( NOLOCK ) ON car.customer_code = ar.customer_code
                                                              AND car.ship_to = ar.ship_to_code
               JOIN arsalesp slp ( NOLOCK ) ON slp.salesperson_code = ar.salesperson_code
               LEFT OUTER JOIN (   SELECT DISTINCT customer_code ,
                                          code
                                   FROM   dbo.cvo_cust_designation_codes AS cdc
                                   WHERE  code IN ( 'rx3', 'rx5', 'rx1', 'rx10' )
                                          AND @today
                                          BETWEEN ISNULL(cdc.start_date, @today) AND ISNULL(
                                                                                         cdc.end_date ,
                                                                                         @today)) rxe ON rxe.customer_code = ar.customer_code
               ...
               LEFT OUTER JOIN (   SELECT customer_code ,
                                          CC_Status = ISNULL(status_code, '')
                                   FROM   cc_cust_status_hist ( NOLOCK )
                                   WHERE  clear_date IS NULL
                                          AND customer_code = @cust ) cc ON cc.customer_code = ar.customer_code
        --LEFT OUTER JOIN
        ...
        --LEFT OUTER JOIN
        --         (SELECT 	customer, RAretpct = CASE WHEN facts.grosssales_r12 = 0 THEN 0 ELSE facts.rareturns_r12/facts.grosssales_r12 END,
        --RAretpct_ytd = CASE WHEN facts.grosssales_ytd = 0 THEN 0 ELSE facts.rareturns_ytd/facts.grosssales_ytd END
        --	from	
        --	(
        --	SELECT customer,
