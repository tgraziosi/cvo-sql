...
-- v1.4 CB 17/03/2016 - #1580 - Credit/Rebill Enhancements
/*======================================================================*/  
...
BEGIN  
	...
	DECLARE @user_category varchar(20) -- v1.4
  
	SET NOCOUNT ON  
  
	SELECT @language = @@language -- Get system language  
  
	SELECT  @language = CASE WHEN @language = 'Español' THEN 'Spanish' ELSE 'us_english' END  
	...
	BEGIN  
		...
		-- v1.4 Start
		SELECT	@user_category = user_category
		FROM	orders_all (NOLOCK)
		WHERE	order_no = @xlp
		AND		ext = @xle

		IF (UPPER(RIGHT(@user_category,2)) = 'RB')
			RETURN 0 
		-- v1.4 End
