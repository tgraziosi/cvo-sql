...
-- v3.0	CT 13/02/13 - Change to how gender is stored against a promo
-- v3.1 CT 13/02/13 - Additional field of max number of pieces
-- v3.2	CT 15/02/13 - Promo designation code logic
-- v3.3 CT 13/05/13 - Issue #1266 - Promo frequency - only check orders created between promo start and end date
-- v3.4 CT 30/05/13 - Issue #1281 - For Review At Ship To promos, still review at ship to level if ship to is blank
-- v3.5 CT 13/08/13 - Issue #1353 - When calculating frequency, only include orders with an extension of 0
-- v3.6 CT 11/11/13 - Issue #1412 - Check primary only setting when checking for designation code
-- v3.7 CT 12/02/14 - Issue #1426 - Check frequency type
...
CREATE PROCEDURE [dbo].[CVO_verify_customer_quali_sp]	@promo_id	VARCHAR(20), @promo_level VARCHAR(30), @customer VARCHAR(30), 
...
BEGIN
		DECLARE	@id					INT,
				...
				@retval				INT,	
				-- END v2.9
				@max_no_of_pieces	DECIMAL(20,8), -- v3.1
				@promo_designation_code	VARCHAR(10), -- v3.2
				-- START v3.3
				@promo_start_date	DATETIME,
				@promo_end_date		DATETIME,
				-- END v3.3
				@primary_only		SMALLINT, -- v3.6
				-- START v3.7
				@frequency_type		CHAR(1), 
				@date_entered		DATETIME,
				-- END v3.7
				@pp_not_purchased	SMALLINT, -- v3.8
		...
		SELECT
			@promo_designation_code = promo_designation_code,
			@primary_only = ISNULL(promo_designation_code_primary_only,0) -- v3.6
		FROM
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					IF @sub_check = 0
					BEGIN
						SELECT 0 as code, 'Customer does not qualify for promotion - Primary Designation Code.' as reason 
						RETURN	
					END
					ELSE
					BEGIN
			...
			BEGIN
				...
				BEGIN
					IF @sub_check = 0
					BEGIN
						SELECT 0 as code, 'Customer does not qualify for promotion - Designation Code.' as reason 
						RETURN	
					END
					ELSE
					BEGIN
		...
		-- START v2.9
		-- If promo is set to Review at Ship To and order has a ship to then check for ship to instead of customer
		-- START v3.4 - always call ship to review even if ship to is blank
		/*
		IF ISNULL(@ship_to,'') <> ''
		BEGIN
		*/
			IF EXISTS (SELECT 1 FROM dbo.cvo_promotions (NOLOCK) WHERE promo_id = @promo_id	AND promo_level = @promo_level AND ISNULL(review_ship_to,0) = 1)
			BEGIN
				EXEC @retval = dbo.CVO_verify_customer_shipto_quali_sp @promo_id = @promo_id, @promo_level = @promo_level, @customer = @customer,
																	   @ship_to = @ship_to, @order_no = @order_no, @ext = @ext, @sub_check = @sub_check
				RETURN @retval
			END 
		/*
		END	
		...
		*/
		...
		BEGIN
			...
			-- Get number of orders for this customer and promo
			SELECT 
			...
			BEGIN
				...
				IF @sub_check = 0
				BEGIN
					SELECT 0 as code, 'Customer does not qualify for promotion - Frequency.' as reason 
					RETURN	
				END
				ELSE
				BEGIN
		...
		INSERT INTO #cvo_customer_qualifications(
				...
				-- END v2.9
				gender_check,		-- v3.0
				max_no_of_pieces,	-- v3.1
				pp_not_purchased,	-- v3.8
				rolling_period, -- v3.9
				min_stock_orders, max_stock_orders, stock_orders_pieces, min_rx_orders, max_rx_orders -- v4.0
		)
...
--select * from #cvo_customer_qualifications
		...
		BEGIN
			SELECT	@min_sales = IsNull(min_sales,-99999999), @max_sales = IsNull(max_sales,999999999), -- v2.4
					...
					-- END v3.0
					-- END v2.9
					@max_no_of_pieces = max_no_of_pieces, -- v3.1
					@pp_not_purchased = pp_not_purchased,  -- v3.8
					@rolling_period = rolling_period, -- v3.9
					@min_stock_orders = min_stock_orders, @max_stock_orders = max_stock_orders, -- v4.0
					@stock_orders_pieces = stock_orders_pieces, @min_rx_orders = min_rx_orders, @max_rx_orders = max_rx_orders -- v4.0
			FROM 	#cvo_customer_qualifications
			WHERE	id = @id
			...
			BEGIN
				...
				IF @fail_reason = ''
				BEGIN
					SET @fail_reason = 'Customer does not qualify for promotion - prior Sales Orders do not fall within the minimum/maximum sales values.' -- v1.1
				END
				-- END v2.9
			END
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						IF @fail_reason = ''
						BEGIN
							SET @fail_reason = 'Customer does not qualify for promotion - customer has ordered the past promotion within the set dates.' 
						END
					END
				END
			END
			...
			BEGIN
				...
				--	Has the customer ordered any of the past programs within the selected dates
				...
				BEGIN
					...
					IF @fail_reason = ''
					BEGIN
						SET @fail_reason = 'Customer does not qualify for promotion - customer has not ordered any of the past promotions within the required dates.' -- v1.1
					END
					-- END v1.1
				END
			END
			...
			--	Does the customer have at least the minimum/maximum Rx orders withint the selected dates.
			IF (@achived = 1)
			BEGIN
				...
				BEGIN
					...
					BEGIN
						SELECT @achived = 0
						SET @fail_reason = 'Customer does not qualify for promotion - minimum/maximum RX orders requirement.' -- v1.1
					END
				END
			END

			--  Does the customer have no more than the max Returns % withint the selected dates.
			IF (@achived = 1)
			BEGIN
				-- START v1.3
				SET @returns = 0
				...
				BEGIN
					...
					BEGIN
						SELECT @achived = 0
						SET @fail_reason = 'Customer does not qualify for promotion - customer exceeds the maximum returns percentage.' -- v1.1
					END
				END
			END

			--   Has the customer order or not ordered a given brand or category of product within the selected dates
			IF (@achived = 1)
			BEGIN
				-- START v1.3
				...
				BEGIN
					...
					BEGIN
						IF (LEN(RTRIM(@brand))>0 AND LEN(RTRIM(@category))>0)
							...
							BEGIN
								SELECT @achived = 0
								SET @fail_reason = 'Customer does not qualify for promotion - excluded brand or category.' -- v1.1
							END
						ELSE
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN 
							SELECT @achived = 0
							SET @fail_reason = 'Customer does not qualify for promotion - Stock Orders do not fall within the minimum/maximum number for the pieces specified.' 					
						END
					END
				END
			END
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN 
							SELECT @achived = 0
							SET @fail_reason = 'Customer does not qualify for promotion - RX Re-Orders do not fall within the minimum/maximum number specified.' 					
						END
					END
				END
			END
		...
		-- END v2.9
		DROP TABLE #cvo_customer_qualifications_gender -- v3.0
...
GO
GRANT EXECUTE ON  [dbo].[CVO_verify_customer_quali_sp] TO [public]
GO
