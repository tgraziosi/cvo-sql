...
CREATE view [dbo].[cvo_sales_by_Customer_vw] as
...
--	ROUND((a.curr_price - (a.curr_price * (a.discount / 100))) ,2) as UnitPrice,
	isnull(a.Shipped * ROUND((a.curr_price - (a.curr_price * (a.discount / 100))) ,2), 0) as ExtendedAmt,
	a.return_code,
	reason = isnull((select return_desc from po_retcode p (nolock) where a.return_code = p.return_code),''), 
...
and b.type = 'I' and b.status='T' and a.shipped > 0
...
and ( exists (select * from inv_tran (nolock) where a.order_no = tran_no and
		...
		or right(b.user_category,2) in ('tb','rb') )
