...
-- v1.0 CB 29/03/2011 18.RDOCK Inventory - Excluded specified bins from available stock
-- v1.1 CT 02/07/2012 673. Add column for qty on replenishment
-- v1.2 CB 26/07/2012 - Add cvo_in_stock column - in_stock without any exclusions
-- v1.3 CT 06/08/2012 - Add in_stock_inc_non_allocating - in stock figure with qty in non-allocating bins not excluded
-- v1.4 CB 16/06/2014 - Change to use views instead of function

-- mls 8/2/01 SCR 27329 - pull void from inv_master instead of inv_list.  It is not set on inv_list.
...
loc.organization_id,
isnull(replen.qty,0) replen_qty,	-- v1.1
case when (m.status='C' or m.status='V') then 0 else (l.in_stock + l.issued_mtd + p.produced_mtd - p.usage_mtd - s.sales_qty_mtd + r.recv_mtd + x.xfer_mtd)end cvo_in_stock, -- v1.2
case when (m.status='C' or m.status='V') then 0 else (l.in_stock + l.issued_mtd + p.produced_mtd - p.usage_mtd - s.sales_qty_mtd + r.recv_mtd + x.xfer_mtd - isnull(z1.qty,0)) end in_stock_inc_non_allocating	-- v1.3
...
LEFT JOIN dbo.f_get_excluded_bins_1_vw z on l.part_no = z.part_no and l.location = z.location -- v1.4
-- v1.1 Join cvo_replenishment_qty
LEFT JOIN dbo.cvo_replenishment_qty replen on l.location = replen.location AND l.part_no = replen.part_no 
-- v1.3 Call function to get qty in bins in config setting
LEFT JOIN dbo.f_get_excluded_bins_4_vw z1 on l.part_no = z1.part_no and l.location = z1.location -- v1.4
GO
GRANT REFERENCES ON  [dbo].[inventory] TO [public]
GO
GRANT SELECT ON  [dbo].[inventory] TO [public]
GO
GRANT INSERT ON  [dbo].[inventory] TO [public]
GO
GRANT DELETE ON  [dbo].[inventory] TO [public]
GO
GRANT UPDATE ON  [dbo].[inventory] TO [public]
GO
