...
AS
	...
	sa_stock = ISNULL((CASE WHEN a.deleted = 1 THEN (a.quantity * -1) ELSE a.quantity END),0) 
	--  - CASE WHEN a.change >= 1 THEN ISNULL((b.qty),0) ELSE 0 END
	, 0 as alloc_stock,
	bo_stock = case when sh.bo_hold = 1 then isnull((case when a.deleted=1 then a.quantity*-1 else a.quantity end),0) else 0 end
