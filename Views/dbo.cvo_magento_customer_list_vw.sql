...
-- RXExpress accounts
SELECT ar.customer_code, ar.ship_to_code, ar.address_name, 'rxe' AS List_name
	FROM dbo.cvo_cust_designation_codes AS ccdc (NOLOCK)
	JOIN armaster ar (NOLOCK) ON ar.customer_code = ccdc.customer_code
	WHERE ccdc.code LIKE 'rx%' AND GETDATE() BETWEEN ISNULL(start_date,GETDATE()) AND ISNULL(end_date, GETDATE())
UNION ALL
	SELECT ar.customer_code, ar.ship_to_code, ar.address_name, 'key' AS List_name
