...
-- select * From cvo_adord_vw where cust_code = '047212'
-- 2/5/2013 - tag - updated order totals on open orders to properly include discounts

CREATE view [dbo].[cvo_adord_TAG_vw] as
SELECT         
...
ISNULL( (select sum(ordered) - sum(cr_ordered)
...
WHERE 
-- orders.type = 'I' and 
orders.status<>'V'  and orders.status<>'X'  
...
-- tag - 5/21/2012 - add qty ordered and shipped per KM request
	qty_ordered = (select sum(isnull(ordered,0))-sum(isnull(cr_ordered,0)) from cvo_ord_list_hist ol (nolock) where
		t1.order_no = ol.order_no and t1.ext = ol.order_ext),
	qty_shipped = (select sum(isnull(shipped,0))-sum(isnull(cr_shipped,0)) from cvo_ord_list_hist ol (nolock) where
		t1.order_no = ol.order_no and t1.ext = ol.order_ext),
