...
/*
SELECT dbo.f_recalc_coop_dollars ('010581') 
SELECT dbo.f_recalc_coop_dollars ('012709') 
SELECT dbo.f_recalc_coop_dollars ('020294') 
SELECT dbo.f_recalc_coop_dollars ('024092') 
SELECT dbo.f_recalc_coop_dollars ('043337') 
*/
          
CREATE FUNCTION [dbo].[f_recalc_coop_dollars_hist](@cust_code VARCHAR(8))          
...
BEGIN
	...
	declare @total_sales decimal(20,8) -- tag
...
--***********Validations*******************************************************************
	...
	Select @total_sales = 
			(select sum(CASE WHEN type = 'C' THEN ((ISNULL(gross_sales,0) - ISNULL(total_discount,0)) * -1) ELSE ISNULL(gross_sales,0) - ISNULL(total_discount,0) END) from orders (NOLOCK)	-- v1.4												-- TLM : Fix
	...
	select @total_sales = @total_sales + (select sum(CASE WHEN type = 'C' THEN ((ISNULL(total_amt_order,0) 
				- ISNULL(tot_ord_disc,0)) * -1) 
				ELSE ISNULL(total_amt_order,0) - ISNULL(tot_ord_disc,0) END) 
				from cvo_orders_all_hist (NOLOCK)	-- v1.4												-- TLM : Fix
				where cust_code = @cust_code  
				and status > 'S' and status <> 'V'												 
				and invoice_date	
				between (SELECT DATEADD(yy, DATEDIFF(yy,0,getdate()), 0))   
				and  (SELECT dateadd(year, 1, dateadd(ms,-3,DATEADD(yy, DATEDIFF(yy,0,getdate() ), 0)))))
...
--***********End Validations****************************************************************??
	...
	DECLARE coop_order CURSOR FOR
			...
			ELSE ISNULL(gross_sales,0) - ISNULL(total_discount,0) END,0) -- v1.4
			from orders (NOLOCK)
			where cust_code = @cust_code
			...
			ELSE ISNULL(total_amt_order,0) - ISNULL(tot_ord_disc,0) END,0) -- v1.4
...
GO
GRANT REFERENCES ON  [dbo].[f_recalc_coop_dollars_hist] TO [public]
GO
GRANT EXECUTE ON  [dbo].[f_recalc_coop_dollars_hist] TO [public]
GO
