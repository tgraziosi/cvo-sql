...
AS
            ...
            BEGIN
                ...
                WHILE 1 = 1 --find the innermost object or list...
                    BEGIN
                        ...
                        SELECT @NextCloseDelimiter = CHARINDEX(
                    ...
                    BEGIN
                            ...
                            BEGIN
                                ...
                                SELECT @token = SUBSTRING(
                                       ...
                                       @EndOfName = PATINDEX(
                                                        '%[0-9]%' ,
                                                        @token COLLATE SQL_Latin1_General_CP850_BIN) ,
                                       @param = RIGHT(@token, LEN(@token)
                                                              - @EndOfName
                                                              + 1);
                                SELECT @token = LEFT(@token, @EndOfName - 1) ,
                                       @Contents = RIGHT(' ' + @Contents, LEN(' '
                                                                              + @Contents
                                                                              + '|')
                                                                          - @end
                                                                          - 1);
                        ...
                        SELECT @value = RTRIM(
                                            SUBSTRING(
                                                ...
                                                @Start ,
                                                @end - @Start)) ,
                               @Contents = RIGHT(@Contents + ' ', LEN(@Contents
                                                                      + '|')
                                                                  - @end);
                        IF SUBSTRING(@value, 1, 7) = '@object'
                            INSERT INTO @hierarchy ( NAME ,
                                                     sequenceNo ,
                                                     parent_ID ,
                                                     StringValue ,
                                                     Object_ID ,
    ...
    END;
GO
GRANT REFERENCES ON  [dbo].[parseJSON] TO [public]
GO
GRANT SELECT ON  [dbo].[parseJSON] TO [public]
GO
