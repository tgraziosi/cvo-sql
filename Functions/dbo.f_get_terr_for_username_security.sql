...
-- select * from dbo.f_get_terr_for_username_security('cvoptical\leeb','0000')
...
if (exists (select * from cvo_work_day_cal
	...
	where x.user_name = @username) not in ('Internal','SS','CS','KA','IS')
...
Begin               
	...
	where x.user_name = @username) in ('SS') -- gets all territories except corp and internal
	BEGIN
		INSERT INTO @rettab 
	...
	if (select distinct x.Salesperson_code from cvo_territoryxref x (nolock) 
		where x.user_name = @username) in ('Internal','CS','KA','IS') -- gets all territories
	BEGIN
		INSERT INTO @rettab 
			select  distinct t.territory_code 
